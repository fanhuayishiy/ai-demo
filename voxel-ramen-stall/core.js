...
scene.fog = new THREE.FogExp2(0x141a26, 0.0075);
...
const geo = {
    ...
    cylinders: new Map(),
    records: [],
    material: new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.82, metalness: 0.05 }),
    temp: new THREE.Object3D(),
    get_material(color) {
        if (color && color.isMaterial) return color;
        const key = color === undefined ? 0x999999 : color;
        if (!material_cache.has(key)) {
            const material = new THREE.MeshStandardMaterial({ color: key, roughness: 0.82, metalness: 0.05 });
    ...
    mesh_box(parent, x, y, z, sx, sy, sz, color, opts = {}) {
        ...
        if (opts.no_collide) mesh.userData.no_collide = true;
        parent.add(mesh);
        return mesh;
    },
    ...
    label(parent, text, x, y, z, width, height, opts = {}) {
        ...
        const material = new THREE.MeshStandardMaterial({ map: texture, roughness: 0.85, side: THREE.DoubleSide, emissive: 0xffffff, emissiveMap: texture, emissiveIntensity: opts.emissive === undefined ? 0.55 : opts.emissive });
...
const lighting = {
    ...
    point_lights: [],
    build() {
        this.hemisphere = new THREE.HemisphereLight(0xcfd8e8, 0x6b5747, 2.4);
        scene.add(this.hemisphere);
        this.sun = new THREE.DirectionalLight(0xffe2b0, 3.4);
        this.sun.position.set(-16, 26, 18);
        this.sun.castShadow = true;
        this.sun.shadow.mapSize.set(2048, 2048);
        Object.assign(this.sun.shadow.camera, { left: -26, right: 26, top: 26, bottom: -26, near: 1, far: 80 });
        this.sun.shadow.bias = -0.0002;
        this.sun.shadow.normalBias = 0.035;
        this.sun.shadow.radius = 2;
        this.sun.target.position.set(0, 1, 0);
        scene.add(this.sun, this.sun.target);
        this.fill = new THREE.DirectionalLight(0x9db6cc, 1.0);
        this.fill.position.set(16, 13, -20);
        scene.add(this.fill);
        this.room_light = new THREE.PointLight(0xffc890, 60, 48, 2);
        this.room_light.position.set(0, 10.5, 1);
    ...
    add_lamp(x, y, z, color = 0xffc477, intensity = 1, parent = null) {
        ...
        glow.renderOrder = 3;
        (parent || scene).add(glow);
        this.lamps.push(glow);
        return glow;
    },
    add_point(x, y, z, color, night_power, opts = {}) {
        const light = new THREE.PointLight(color, 0, opts.range || 16, 2);
        light.position.set(x, y, z);
        light.userData.night_power = night_power;
        light.userData.day_power = opts.day_power || 0;
        light.userData.flicker = opts.flicker || 0;
        scene.add(light);
    ...
    update() {
        ...
        const dawn = Math.max(0, 1 - Math.abs(state.day - 0.235) / 0.075);
        state.dusk = dusk;
        state.dawn = dawn;
        this.sun.color.set(0xffe3b7).lerp(new THREE.Color(0xff8b49), dusk * 0.7);
        this.sun.intensity = Math.max(0.12, sun_height * 4.1) * (1 - state.rain * 0.8);
        this.sun.position.set(-18 + state.day * 25, 24 + sun_height * 7, 17);
        this.hemisphere.intensity = THREE.MathUtils.lerp(2.15, 0.95, state.night) * (1 - state.rain * 0.25);
        this.hemisphere.color.set(0xc9dcea).lerp(new THREE.Color(0x5d84bd), state.night);
        this.fill.intensity = 0.85 - state.night * 0.45;
        this.room_light.intensity = 42 + state.night * 46;
        const sky = new THREE.Color(0x39434c).lerp(new THREE.Color(0x101a2a), state.night).lerp(new THREE.Color(0x2f3844), state.rain * 0.6);
        if (dawn > 0) sky.lerp(new THREE.Color(0x8c7d90), dawn * 0.55);
        if (dusk > 0) sky.lerp(new THREE.Color(0x54382e), dusk * 0.4);
        scene.background.copy(sky);
        scene.fog.color.copy(sky);
        scene.fog.density = 0.0072 + dawn * 0.009 + state.night * 0.0012 + state.rain * 0.004;
        for (const lamp of this.lamps) {
            let alpha = (0.06 + state.night * 0.78 + state.rain * 0.4) * lamp.userData.intensity;
            if (lamp.userData.pulse) alpha *= 0.25 + 0.75 * Math.pow(Math.max(0, Math.sin(state.real_time * 5.5)), 24) * 2.2;
            lamp.material.uniforms.u_alpha.value = Math.min(1, alpha);
        }
        for (const light of this.point_lights) {
            let value = THREE.MathUtils.lerp(light.userData.day_power, light.userData.night_power * (1 + state.rain * 0.45), state.night);
            if (light.userData.flicker) value *= 1 - light.userData.flicker * (0.5 + 0.5 * Math.sin(state.real_time * 9.3 + light.position.x * 7));
            light.intensity = value;
        }
    },
};
lighting.build();
const ctx = { THREE, scene, geo, add_lamp: lighting.add_lamp.bind(lighting), add_point: lighting.add_point.bind(lighting) };
