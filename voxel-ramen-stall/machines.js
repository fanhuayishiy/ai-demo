    ...
    anim_rider(def, t) {
        ...
        for (const [t0, t1, px, pz, kind] of pts) {
            if (p >= t0 && p < t1) {
                ...
                if (kind === 'dismount') { x = 4.42 + (1 - k) * 0.55; seated = 1 - k; yaw = -Math.PI / 2; }
                if (kind === 'remount') { x = 4.42 + k * 0.43; seated = k; yaw = -Math.PI / 2; }
                if (kind === 'grab') { bowl = Math.min(1, k * 2); yaw = Math.atan2(2.4 - x, 2.5 - z); }
                if (kind === 'place') { bowl = 1 - Math.max(0, (k - 0.5) * 2); yaw = Math.PI / 2; }
                break;
            }
        }
        if (phase === 'walk') {
