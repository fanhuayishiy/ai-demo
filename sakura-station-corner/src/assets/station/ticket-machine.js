...
import {
  grp, mesh, box, cyl, rbox, lathe, hoop, tubeOf, finish, rand, range,
  weather, decal,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 5505, withTicketBox: true };
...
export function build(options = {}) {
  ...
  // ボタン（数字・金額・硬質プラスチック、指紋の磨耗）
  {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      bs.add(mesh(cyl(0.024, 0.024, 0.004, 12), mDark, { pos: [cx, cy, -0.006], rot: [90 * D2R, 0, 0], cast: false }));
    }
    // 確認ランプ
  ...
  // 紙幣投入口・硬貨投入口・釣銭受け・切符出口
  {
    ...
    for (let i = 0; i < 2; i++) {
      cup.add(mesh(cyl(0.011, 0.011, 0.0017, 12), MAT.metal('#c9a45c', { worn: 0.6, dir: 'h' }), { pos: [i ? 0.012 : -0.01, 0.008 + i * 0.002, i ? -0.008 : 0.006], rot: [range(rnd, -0.1, 0.1), 0, range(rnd, -0.2, 0.2)] }));
    }
    cup.add(mesh(box(0.14, 0.008, 0.02), mDark, { pos: [0, -0.006, -0.05], cast: false }));
    slots.add(cup);
    // 券出口（カバー付き）
    const out = grp('ticket-out', { pos: [-BW * 0.2, BH - 0.66, faceZ + 0.012] });
    out.add(mesh(rbox(0.2, 0.06, 0.02, 0.006, 2), mDark, {}));
    out.add(mesh(box(0.17, 0.01, 0.024), MAT.paint('#15181a', { steps: 2, shadowAmt: 1 }), { pos: [0, -0.006, 0.004], cast: false }));
  ...
  // 上部の行先運賃表示灯箱 + 屋根
  {
    ...
    decal(top, { map: TEX.signboard({ text: 'きっぷうりば', sub: 'TICKETS', bg: PAL.signBlue, fg: '#e4e9ee' }), w: BW - 0.14, h: 0.2, pos: [0, 0, -(BD / 2 - 0.013)], rot: [0, Math.PI, 0], order: 1 });
  ...
  if (withBox) {
    ...
    for (let i = 0; i < 7; i++) {
      kbx.add(mesh(box(0.19, 0.006, 0.1), mPaper, { pos: [range(rnd, -0.01, 0.01), -0.04 + i * 0.007, range(rnd, -0.005, 0.005)], rot: [0, range(rnd, -0.03, 0.03), range(rnd, -0.02, 0.02)], cast: false }));
    }
    // 取出ノブ・ばね口
