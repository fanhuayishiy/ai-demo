...
import {
  grp, mesh, box, cyl, rbox, sph, extrude, shape, tubeOf, finish, rand, range,
  weather, decal, inst,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { steps: 5, width: 1.6, rise: 0.72, seed: 6701 };
...
export function build(options = {}) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    // 段鼻（金属アングル・少し摩耗）
    st.add(mesh(box(width - 0.01, 0.026, 0.028), mNosing, { pos: [0, 0.008, -(TD / 2 - 0.03)], name: 'nosing' }));
    st.add(mesh(box(width - 0.01, 0.012, 0.05), mNosing, { pos: [0, -0.008, -(TD / 2 - 0.02)], cast: false }));
    // 滑り止め（ゴム縞 3 本）
    ...
    if (i < 3) {
      for (const sx of [-1, 1]) {
        const lg = grp(`step-lamp-${i}-${sx}`, { pos: [sx * (width / 2 - 0.24), -0.036, -(TD / 2 - 0.02)] });
  ...
  // 地面際の最後段の周りに土砂・落ち葉
  {
    ...
    g.add(inst(new THREE.PlaneGeometry(0.06, 0.06), leafMat, 22, (i, d, r) => {
  ...
  // 手すりに取り付けた標識（くだり／のぼり）
  {
    ...
    decal(sg, { map: signTex('のぼり', 'UP'), w: 0.28, h: 0.14, pos: [0, 0, -0.008], rot: [0, Math.PI, 0], order: 1 });
    g.add(sg);
  }

  /* ══════════════════════════════════════════ 車椅子スロープ（仮設アルミ・段の端に立て掛け） */
  {
    const RW = 0.64, RL = 2.42;                       // 2 輪分幅・長さ
