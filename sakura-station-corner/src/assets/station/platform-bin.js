...
import {
  grp, mesh, box, cyl, rbox, sph, lathe, tubeOf, finish, rand, range, weather, decal, inst, shadowBlob,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 8411 };
...
export function build(options = {}) {
  ...
  /* ══════════════════════════════════════════ ラベル・注意札 */
  {
    ...
    for (const [hx, hy] of [[-0.14, 0.07], [0.14, 0.07], [-0.14, -0.07], [0.14, -0.07]]) {
      hatch.add(mesh(cyl(0.008, 0.008, 0.01, 6), mIron, { pos: [hx, hy, -0.008], rot: [90 * D2R, 0, 0], cast: false }));
    }
    body.add(hatch);
  }
  ...
  // 足元の落ち葉・砂
  {
    ...
    g.add(inst(new THREE.PlaneGeometry(0.062, 0.062), leafMat, 16, (i, d, r) => {
    ...
    g.add(inst(new THREE.SphereGeometry(0.008, 4, 3), MAT.ballast({ repeat: 1 }), 22, (i, d, r, col) => {
    ...
    // 箱の上に載ってしまった落ち葉
    g.add(mesh(new THREE.PlaneGeometry(0.07, 0.07), leafMat, { pos: [-W / 4 - 0.06, TOP + 0.104, -0.04], rot: [-Math.PI / 2 + 0.1, 0, 1.2], cast: false }));
  }
