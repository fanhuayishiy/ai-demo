...
//  assets/station/rail-furniture.js —— 線路周辺機器（5 kind 共用モジュール）
//  ---------------------------------------------------------------------------
//  ・原点 = 接地面（最下点）中心、+Y 上、**正面 = +Z**（装配層の rotY で向きを決める）
//    - kind='switch' 転換機        0.87 × 0.83 × 1.35 m   （標的は +Z、連査棒は -Z＝線路側へ 1.1m 伸びる）
//    - kind='duct'   ケーブル保護槽 2.60 × 0.34 × 0.76 m   （槽はローカル X 方向に走るので rotY=0 で軌道と並行）
//    - kind='marker' 線路標識クラスタ 0.94 × 1.31 × 0.40 m  （停止位置目標＋注意標＋距離標、各支柱に傾き）
//    - kind='stop'   車止め        2.38 × 0.72 × 0.87 m   （列車は +Z のほうから接近、幅方向 = ローカル X）
//    - kind='drain'  側溝の落口・伏越 1.66 × 0.26 × 0.69 m （側溝はローカル X、伏越は Z 方向で溝を交差）
//  ・基準：レール頂 y=-0.30 / 路盤頂 y=-0.42 / ゲージ 1.067（world/plan.js RAIL と対応）
//  ・経年：錆流痕・掉漆・苔・土砂堆積・変形・落書き除去跡を kind ごとに 3 箇所以上
import {
  grp, mesh, box, cyl, rbox, sph, plane, tor, hoop, tubeOf, inst,
  finish, rand, range, weather, decal,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, jpText, shade } from '../../core/textures.js';
...
export const DEFAULT_OPTIONS = { kind: 'switch', seed: 661 };
...
function buildDuct(g, rnd, seed, K) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i % 2 === 0) c.add(mesh(box(0.032, 0.086, 0.54), K.galv, { pos: [cw / 2 - 0.006, -0.008, 0], name: 'clamp-strap' }));
...
function buildMarker(g, rnd, seed, K) {
  ...
  a.add(st);
  st.add(mesh(box(0.34, 0.3, 0.014), K.paintGrey, { pos: [0, 0, -0.008], name: 'backing' }));
...
function buildStop(g, rnd, seed, K) {
  ...
  for (let i = 0; i < 5; i++) {
    weather(beams, { w: range(rnd, 0.16, 0.42), h: range(rnd, 0.06, 0.12), pos: [range(rnd, -1.0, 1.0), range(rnd, 0.18, 0.42), zBeam + 0.086], kind: i % 2 ? 'rust' : 'chip', color: i % 2 ? '#7c4a2b' : '#b8b2a2', opacity: 0.48, seed: seed + 261 + i, density: 1.6, spread: 0.02 });
  }
  ...
  for (let i = 0; i < 4; i++) leaf(p, rnd, range(rnd, -1.0, 1.0), 0.03, range(rnd, 0.0, 0.2), K, range(rnd, 0.8, 1.2));
...
function buildDrain(g, rnd, seed, K) {
  ...
  for (let i = 0; i < barN; i++) {
    ...
    const sag = i === 3 ? -0.012 : range(rnd, -0.003, 0.003);      // 一つだけ沈んだ棒
...
function buildSpeed(g, rnd, seed, K) {
  ...
  post.add(sp);
  sp.add(mesh(cyl(0.19, 0.19, 0.016, 28), K.paintGrey, { pos: [0, 0, -0.008], rot: [HALF_PI, 0, 0], name: 'sign-backing' }));
...
function buildWeed(g, rnd, seed, K) {
  ...
  for (let i = 0; i < nSeg; i++) {
    ...
    seg.add(mesh(box(LEN / nSeg - 0.006, T, WID - curl * 0.42), K.rubber, {
      pos: [0, T / 2 + curl * 0.03, -curl * 0.18], rot: [curl * 3.4 * D2R, range(rnd, -0.006, 0.006), 0], name: 'mat-panel', cast: false,
    }));
  ...
  const bladeGeo = new THREE.PlaneGeometry(0.026, 0.16);
...
function buildVerge(g, rnd, seed, K) {
  ...
  /* 草（法面側）＋線路側へ倒れ込んだ 3 束 */
  const blade = new THREE.PlaneGeometry(0.03, 0.3);
  ...
  const spill = new THREE.PlaneGeometry(0.034, 0.42);
