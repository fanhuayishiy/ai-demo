...
import {
  grp, mesh, box, cyl, rbox, sph, tubeOf, finish, rand, range, weather, decal, shadowBlob,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  for (let i = 0; i < nS; i++) {
    ...
    const p = mesh(plank(L - 0.02, 0.026, BW - 0.006, bow, range(rnd, -0.5, 0.5), seed + 100 + i), i % 4 === 0 ? mWoodEnd : mWood, {
      pos: [range(rnd, -0.006, 0.006), SH + 0.012 + Math.abs(bow) * 0.3, z], name: `seat-slat-${i}`,
    });
    p.rotation.x = range(rnd, -0.008, 0.008);
    p.rotation.z = range(rnd, -0.006, 0.006);
    seat.add(p);
    // 板の端の打痕・ささくれ
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const bow = range(rnd, -0.008, 0.014);
    ...
    back.add(p);
    // 裏側（腰の後ろ）の落書き除去跡
    const gr = decal(back, { map: graffitiTex(seed + i * 7), w: 0.44, h: 0.06, pos: [range(rnd, -0.3, 0.3), y - 0.004, -SD / 2 + 0.012], rot: [6 * D2R, 0, 0], opacity: 0.55, order: 1 });
    gr.renderOrder = 4 + i;
  }
  ...
  /* ══════════════════════════════════════════ ゴミの貼り付き・落ち葉・こぼれ種 */
  {
    ...
    for (let i = 0; i < 4; i++) {
      grime.add(mesh(new THREE.PlaneGeometry(0.05, 0.05), leafMat, {
