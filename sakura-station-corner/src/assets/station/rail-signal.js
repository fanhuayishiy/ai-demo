...
import {
  grp, mesh, box, cyl, rbox, sph, hoop, tubeOf, finish, rand, range,
  weather, decal,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 4404, kind: 'absolute' };
...
export function build(options = {}) {
  ...
  // 土砂の堆积・苔・落ち葉
  f.add(mesh(rbox(0.52, 0.06, 0.46, 0.06, 2), MAT.stone({ color: '#8b7c62' }), { pos: [0, -0.005, 0.02], cast: false, name: 'dirt-heap' }));
  ...
  /* ══════════════════════════════════════════ 信号頭（3 灯・笠・仕切り・背面） */
  {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const lens = mesh(new THREE.SphereGeometry(R, 22, 11, 0, Math.PI * 2, 0, th), lensMat(kind, intens[i]), {
      ...
      sec.add(mesh(new THREE.CylinderGeometry(0.088, 0.093, 0.14, 18, 1, true, Math.PI / 2, Math.PI), mCase, { pos: [0, 0.008, 0.07], rot: [90 * D2R, 0, 0], name: `visor-${kind}` }));
      sec.add(mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.014, 18, 1, true, Math.PI / 2, Math.PI), mCase, { pos: [0, 0.008, 0.14], rot: [90 * D2R, 0, 0] }));
