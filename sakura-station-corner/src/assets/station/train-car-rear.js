...
import {
  grp, mesh, box, cyl, rbox, sph, plane, lathe, tubeOf, hoop, coil,
...
export const DEFAULT_OPTIONS = { seed: 5202 };
...
export function build(options = {}) {
  ...
  /* ══════════════════════════════════════════ 客室（先頭車より簡素・座席の向きを変える） */
  {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (const cz of [-0.34, 0.34]) {
        ...
        it.add(mesh(new THREE.TorusGeometry(0.052, 0.01), mRubberOld, { pos: [x + 0.93, FLOOR + 1.52, cz], rot: [8 * D2R, 0, 0], cast: false }));
      }
    }
  }
  ...
  {   // 後台形窓（サッシ＋ガラス＋内側の仕切壁）
    ...
    w.add(mesh(box(0.8, 0.54, 0.014), mGlass, { pos: [0, 0, -0.006], name: 'glass-rear', cast: false }));
