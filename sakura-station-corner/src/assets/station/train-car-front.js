...
import {
  grp, mesh, box, cyl, rbox, sph, plane, lathe, tubeOf, hoop, coil, extrude, shape,
...
export const DEFAULT_OPTIONS = { seed: 4101 };
...
function plateGeo(w, h, holes = [], th = 0.1) {
  ...
  const g = new THREE.ExtrudeGeometry(s, { depth: th, bevelEnabled: false, curveSegments: 8, steps: 1 });
...
export function build(options = {}) {
  ...
  const xN = HALF, xT = -HALF;            // 先端（東）/ 尾灯端（西）
  const LEN = S.len;
  ...
  /* ---------- 側面レイアウト（扉 2 箇所 + 固定窓列） ---------- */
  const doors = [{ c: xS1 - 2.62 * K16, w: 1.30 }, { c: -3.30 * K16, w: 1.30 }].sort((a, b) => a.c - b.c);
  const bays = [];
  {
    let cur = xS0;
  ...
  function torSafe(r, t) { return new THREE.TorusGeometry(r, t, 6, 12); }
  ...
  // タイフォン（汽笛・2 口）
  {
    ...
    for (const s of [-1, 1]) {
      ...
      tf.add(mesh(new THREE.TorusGeometry(0.056, 0.009, 6, 14), mSteel, { pos: [0.118, 0, s * 0.072], rot: [0, 90 * D2R, 0] }));
    }
    nose.add(tf);
  }
  ...
  // 幌（ゴム蛇腹）＋アルミ枠
  {
    ...
    gd.add(mesh(plateGeo(0.36, 0.46, [[0, 0, 0.28, 0.36, 0.03]], 0.018), mSash, { pos: [-0.005, 0.4, 0.024], rot: [0, -90 * D2R, 0] }));
