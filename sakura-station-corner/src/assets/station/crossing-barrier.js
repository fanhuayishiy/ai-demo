...
import {
  grp, mesh, box, cyl, rbox, sph, lathe,
  finish, rand, range, weather, decal, pipe, hoop,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { kind: 'barrier', side: 'left', seed: 623 };
...
export function build(options = {}) {
  ...
  const body = MAT.metalPaint(PAL.storeBand3 === '#e2685e' ? '#c9564c' : '#c2564a', { worn: 0.85, repeat: 3, base: '#e08d80' });
  const mat = {
    body,
  ...
  if (kind === 'barrier') {
    ...
    /* 腕木（降りた位置・白赤反射帯・先端灯り・LED・ゴムキャップ・曲がり・錆） */
    {
      ...
      // 線路側はみ出し防止（腕木下に下げる針金と小札）
      {
        ...
        decal(tag, { map: TEX.signboard({ text: 'はみだし かんきょう', bg: '#e8c446', fg: '#3a3325' }), w: 0.24, h: 0.09, pos: [0, 0, -0.004], rot: [0, Math.PI, 0], order: 1 });
        wire.add(tag);
        g.add(wire);
      }
      // 錆・日焼け・擦れ（腕木全体）
  ...
  /* ════════════════════ 警報機 ════════════════════ */
    ...
    for (const sx of [-1, 1]) {
      ...
      const lens = mesh(new THREE.SphereGeometry(R, 20, 10, 0, Math.PI * 2, 0, th),
      ...
      // 遮光フード（半円殼＋前立ち上がり）
      const hr = 0.072;
      head.add(mesh(new THREE.CylinderGeometry(hr, hr, 0.13, 16, 1, true, 0, Math.PI), mat.body, { pos: [lx, 0.012, 0.12], rot: [Math.PI / 2 - 0.32, 0, 0] }));
      head.add(mesh(new THREE.RingGeometry(hr - 0.004, hr + 0.014, 16, 1, 0, Math.PI), mat.body, { pos: [lx, 0.012, 0.183], rot: [0, 0, 0], cast: false }));
  ...
  /* ════════════════════ 非常ボタン ════════════════════ */
    ...
    // 赤カバー（半球ドーム）とボタン
    boxg.add(mesh(new THREE.SphereGeometry(0.085, 18, 10, 0, Math.PI * 2, 0, Math.PI / 2), MAT.metalPaint('#d43a2f', { worn: 0.6, base: '#e8615a', side: THREE.DoubleSide }), { pos: [0, 0.02, 0.1], rot: [Math.PI, 0, 0], name: 'red-cover' }));
    ...
    // 誘導灯（上部：非常ボタンの位置を示す青白灯。breathe で柔らかく明暗）
    {
      ...
      gd.add(mesh(cyl(0.036, 0.036, 0.012, 14), MAT.lampShade({ color: '#dceefc', emissive: '#5fa0d8', emissiveIntensity: 0.7, steps: 2 }), {
        pos: [0, -0.004, 0.031], rot: [90 * D2R, 0, 0], name: 'guide-lens', cast: false,
      }));
