...
import {
  grp, mesh, box, rbox, cyl, tor, plane, extrude, shape, sakuraPetalRestGeo,
  finish, rand, range, put, rotY, weather, decal, row, grill, inst,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 803, len: 5.2 };
...
export function build(options = {}) {
  ...
  const M = {
    ...
    meshFabric: MAT.paint('#3f4744', { map: TEX.metal({ base: '#3f4744', worn: 0.2, repeat: 16 }).map, side: THREE.DoubleSide, spec: 0.1 }),
  ...
  // 枠
  for (const [dx, dy, bw, bh] of [[0, DH / 2 + 0.026, DW + 0.10, 0.052], [-(DW / 2 + 0.026), 0, 0.052, DH], [DW / 2 + 0.026, 0, 0.052, DH]]) {
  ...
  for (const dy of [0.30, DH - 0.34]) {
    ...
    leaf.add(hgrp);
    hgrp.add(mesh(box(0.024, 0.10, 0.012), M.iron, { pos: [0, 0, -0.020] }));
    hgrp.add(mesh(cyl(0.009, 0.009, 0.11, 8), M.iron, { pos: [0.006, 0, 0] }));
    hgrp.add(mesh(cyl(0.012, 0.012, 0.012, 8), M.iron, { pos: [0.006, 0.058, 0] }));
    weather(hgrp, { w: 0.06, h: 0.12, pos: [0.02, -0.07, -0.006], kind: 'rust', color: PAL.rust, opacity: 0.55, seed: seed + 31 + Math.round(dy * 100), density: 1.8, spread: 0.006 });
  }
  ...
  lock.add(mesh(cyl(0.013, 0.013, 0.05, 10), M.iron, { pos: [-0.004, 0, 0.042], rot: [Math.PI / 2, 0, 0] }));
  ...
  for (const sx of [-1, 1]) {
    ...
    eave.add(mesh(box(0.03, 0.05, 0.05), M.iron, { pos: [sx * (DW / 2 + 0.06), -0.20, -0.02], name: 'canopy-anchor' }));
  }
  ...
  // 網戸（外側に吊る・少し開いたまま）
  const ago = grp('screen', { pos: [-0.10, 0, 0.10], rotY: 16 });
  kuchi.add(ago);
  ...
  win.add(noOut(mesh(box(WW - 0.08, WH - 0.08, 0.008), M.dark, { pos: [0, 0, -0.030], cast: false, name: 'win-curtain-void' })));
  ...
  winE.add(noOut(mesh(box(0.42, 0.32, 0.006), M.dark, { pos: [0, 0, -0.004], cast: false, name: 'gable-void' })));
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    seg.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), new THREE.Vector3(b[0] - a[0], b[1] - a[1], b[2] - a[2]).normalize());
    water.add(seg);
  }
  // 壁に添って上る縦管＋サドルバンド
  ...
  shed.add(mesh(cyl(0.016, 0.016, 0.90, 6), M.woodOld, { pos: [0.42, shedY + 0.48, -0.20], rot: [0.12, 0, 0.1], name: 'shed-pole' }));
  ...
  for (const s of [-1, 1]) {
    ...
    for (const [ox, oy, bw, bh] of [[0, (wallH - 0.06) / 2 - 0.014, doorW, 0.028], [0, -(wallH - 0.06) / 2 + 0.014, doorW, 0.028], [-(doorW / 2 - 0.014), 0, 0.028, wallH - 0.06], [doorW / 2 - 0.014, 0, 0.028, wallH - 0.06]]) {
      d.add(mesh(box(rq(bw), rq(bh), 0.026), M.aluDark, { pos: [ox, oy, 0.004], name: 'door-frame' }));
    }
    // 下部通風格子
    d.add(noOut(grill(d, { w: doorW - 0.10, h: 0.09, nx: 2, ny: 4, bar: 0.006, mat: M.aluDark, pos: [0, -(wallH - 0.06) / 2 + 0.09, 0.016] })));
  ...
  hasp.add(mesh(tor(0.014, 0.0045, 5, 12), M.iron, { pos: [0.055, -0.006, 0.012], name: 'padlock-shackle' }));
  hasp.add(noOut(mesh(box(0.012, 0.006, 0.004), M.dark, { pos: [0.055, -0.030, 0.021], cast: false, name: 'padlock-keyway' })));
  weather(hasp, { w: 0.10, h: 0.16, pos: [0.05, -0.10, 0.014], kind: 'rust', color: '#8a5236', opacity: 0.55, seed: seed + 61, density: 2.0, spread: 0.01 });
  // 屋根（波板・片流れ・軒先の出）＋ 妻金物
  const sRoofAng = 0.15;
  const sRun = SD + 0.30;
  ...
  // 換気口（波板貫通の小型フード）
  shed.add(mesh(cyl(0.05, 0.05, 0.06, 10), M.aluDark, { pos: [-0.36, shedY + SH + 0.16, -0.10], rot: [-sRoofAng, 0, 0], name: 'shed-vent' }));
  // 錆・釘::経年
  ...
  const towel = grp('towel', { pos: [-1.30, 1.46, 1.14], rotY: 0.05 });
  hoshi.add(towel);
  towel.add(mesh(box(0.30, 0.46, 0.010), M.towel, { pos: [0, -0.21, 0.012], rot: [0.06, 0, 0.03], name: 'towel-body' }));
  towel.add(mesh(box(0.30, 0.02, 0.028), M.towel, { pos: [0, -0.006, 0.002], name: 'towel-fold' }));
  for (let i = 0; i < 5; i++) {
  ...
  drain.add(mesh(box(rq(dLen), 0.05, dW), M.base, { pos: [0, Z_DRN ? 0 : 0, 0], name: 'drain-floor-placeholder' }));
  // ↑ プレースホルダではなく明示的に組み直す
  ...
  gr2.add(noOut(mesh(box(0.008, 0.02, 0.6), MAT.paint('#cec5ae', { steps: 2 }), { pos: [0.006, -0.30, 0], cast: false, name: 'roller-edge' })));
