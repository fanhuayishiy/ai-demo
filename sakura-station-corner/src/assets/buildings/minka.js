...
import {
  grp, mesh, box, rbox, cyl, plane, tor, lathe, extrude,
  finish, rand, range, weather, decal, row, grid, grill, inst, put, rotY,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 4801, w: 3.4, d: 4.5, roof: 'tile' };

/* ------------------------------ 寸法定数（m） ------------------------------ */
const WT = 0.155;        // 外壁厚
const FND_TOP = 0.42;    // 基礎天＝床レベル
const WALL_TOP = 2.44;   // 壁天端（軒天井よりやや上）
const EAVE_TIP = 2.30;   // 軒先の高さ
const SLOPE = 0.56;      // 勾配（≈5.5 寸）
const EAVE_X = 0.28;     // 長手（傾斜）側軒の出 ── 東は小巷なので詰める
const EAVE_Z = 0.26;     // 妻側の出し（雨棟木出し）
...
function windowUnit(M, o) {
  ...
  const gl = M.glass;
  g.add(mesh(box(w - fw * 2 - 0.01, h - fw * 2 - 0.01, 0.006), gl, { pos: [-0.012, 0, -0.006], name: 'win-glass-a' }));
...
/** 換気口／格栅（壁面取付） */
function ventGrille(M, { w = 0.26, h = 0.2, pos, rotYdeg = 0, mat, slats = 5 }) {
  const g = grp('vent', { pos, rotY: rotYdeg });
  g.add(mesh(box(w, h, 0.03), mat, { name: 'vent-plate' }));
  for (let i = 0; i < slats; i++) {
    const y = -h / 2 + ((i + 0.5) / slats) * h;
    g.add(mesh(box(w - 0.035, 0.012, 0.022), M.plasticDark, { pos: [0, y, 0.018], rot: [0.42, 0, 0], name: 'vent-louver' }));
  }
  g.add(mesh(tor(w * 0.5, 0.012, 4, 12), M.aluDark, { pos: [0, 0, 0.022], scale: [1, h / w, 1], name: 'vent-ring' }));
  return g;
}

/** エアコン室外機（据付金具・配管・ドレン・色褪せ） */
function acOutdoorUnit(M, { seed = 1, rotYdeg = 0, pos, scale = 1 }) {
  const g = grp('ac-unit', { pos, rotY: rotYdeg, scale });
  const W = 0.797, H = 0.548, D = 0.282;
  // 据付金具（L 型アングル 2 つ）＋ ボルト
  for (const s of [-1, 1]) {
    g.add(mesh(box(0.05, 0.14, D * 0.9), M.galv, { pos: [s * (W / 2 - 0.09), 0.07, 0], name: 'ac-bracket' }));
    g.add(mesh(cyl(0.011, 0.011, 0.026, 8), M.darkIron, { pos: [s * (W / 2 - 0.09), 0.135, 0], name: 'ac-bolt' }));
  }
  // 本体（上板・側板・前面格栅・後面フィン）
  g.add(mesh(rbox(W, H, D, 0.014, 2), M.acCase, { pos: [0, H / 2 + 0.14, 0], name: 'ac-body' }));
  g.add(mesh(cyl(0.19, 0.19, 0.02, 20), M.acCase, { pos: [-0.06, H / 2 + 0.14, D / 2 + 0.004], rot: [Math.PI / 2, 0, 0], name: 'ac-fan-housing' }));
  const fan = grp('ac-fan', { pos: [-0.06, H / 2 + 0.14, D / 2 + 0.014] });
  for (let i = 0; i < 3; i++) {
    fan.add(mesh(box(0.32, 0.055, 0.008), M.plasticDark, { rot: [0, 0, (i / 3) * Math.PI * 2], pos: [0, 0, 0], name: 'ac-blade' }));
  }
  fan.add(mesh(cyl(0.026, 0.026, 0.03, 10), M.aluDark, { rot: [Math.PI / 2, 0, 0], name: 'ac-hub' }));
  grill(g, { w: 0.42, h: 0.4, nx: 6, ny: 8, bar: 0.008, mat: M.aluDark, pos: [W / 2 - 0.28, H / 2 + 0.14, D / 2 + 0.006] });
  g.add(mesh(box(W - 0.06, H - 0.09, 0.014), M.finDark, { pos: [0, H / 2 + 0.14, -D / 2 - 0.004], name: 'ac-condenser' }));
  // 天板のすき間・配管・ドレンホース
  g.add(mesh(box(W - 0.1, 0.012, D - 0.1), M.aluDark, { pos: [0, H + 0.146, 0], name: 'ac-top-slot' }));
  const pipePts = [[W / 2 - 0.1, 0.2, -D / 2], [W / 2 + 0.04, 0.2, -D / 2 - 0.1], [W / 2 + 0.06, 0.32, -D / 2 - 0.12]];
  for (let i = 0; i < pipePts.length - 1; i++) {
    const a = pipePts[i], b = pipePts[i + 1];
    g.add(mesh(cyl(0.021, 0.021, rq(Math.hypot(b[0] - a[0], b[1] - a[1], b[2] - a[2])), 8), M.insulation,
      { pos: [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2, (a[2] + b[2]) / 2], name: 'ac-pipe' }));
  }
  g.add(mesh(cyl(0.012, 0.012, 0.30, 7), M.rubberMat, { pos: [-W / 2 + 0.12, -0.01, -D / 2 - 0.05], rot: [1.25, 0, 0.15], name: 'ac-drain' }));
  weather(g, { w: W, h: H * 0.8, pos: [0, H / 2 + 0.14, D / 2 + 0.017], kind: 'rust', color: '#7c4a2b', opacity: 0.34, seed: seed + 5, density: 1.5, count: 2, spread: 0.1 });
  weather(g, { w: W * 0.8, h: H * 0.6, pos: [0, H / 2 + 0.14, -D / 2 - 0.013], rot: [0, Math.PI, 0], kind: 'dirt', color: '#494136', opacity: 0.42, seed: seed + 9, density: 1.2 });
  return g;
}

/** プロパンボンベ（10kg：ラテン形状・首輪・残量シール・錆） */
function propaneTank(M, { pos, rotYdeg = 0, s = 1 }) {
  const g = grp('propane', { pos, rotY: rotYdeg, scale: s });
  const pts = [[0, 0], [0.13, 0.008], [0.155, 0.06], [0.155, 0.36], [0.13, 0.42], [0.075, 0.455], [0.052, 0.47], [0.05, 0.5]];
  g.add(mesh(lathe(pts, 18), M.tank, { name: 'tank-body' }));
  // 首輪（ガード）＋ 弁
  g.add(mesh(tor(0.083, 0.014, 6, 16), M.tankCollar, { pos: [0, 0.475, 0], name: 'tank-collar' }));
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2;
    g.add(mesh(box(0.014, 0.09, 0.014), M.tankCollar, { pos: [Math.cos(a) * 0.083, 0.52, Math.sin(a) * 0.083], name: 'tank-post' }));
  }
  g.add(mesh(cyl(0.022, 0.026, 0.05, 8), M.brass, { pos: [0, 0.53, 0], name: 'tank-valve' }));
  g.add(mesh(box(0.055, 0.04, 0.008), M.paperWhite, { pos: [0.128, 0.22, 0.075], rot: [0, 0.5, 0], name: 'tank-label' }));
  weather(g, { w: 0.3, h: 0.34, pos: [0, 0.16, 0.163], kind: 'rust', color: '#8b5330', opacity: 0.42, seed: 41, density: 1.6, count: 2, spread: 0.06 });
  return g;
}

/** 水道メーター箱（コンクリート蓋＋立ち上がり＋凍結覆い） */
function meterBox(M, { pos, rotYdeg = 0 }) {
  const g = grp('meter-box', { pos, rotY: rotYdeg });
  g.add(mesh(rbox(0.34, 0.26, 0.30, 0.012, 2), M.concDark, { pos: [0, 0.13, 0], name: 'meter-rise' }));
  g.add(mesh(box(0.38, 0.036, 0.34), M.concDark, { pos: [0, 0.272, 0], name: 'meter-lid' }));
  g.add(mesh(box(0.10, 0.012, 0.05), M.aluDark, { pos: [0.08, 0.294, 0], name: 'meter-handle' }));
  // 凍結防止の布カバー（端がめくれて経年）
  const cv = mesh(rbox(0.30, 0.05, 0.26, 0.02, 2), M.fabricBlue, { pos: [0.02, 0.312, -0.01], rot: [0, 0.06, 0], name: 'meter-frost-cover' });
  g.add(cv);
  g.add(mesh(box(0.05, 0.05, 0.014), M.brass, { pos: [-0.19, 0.14, 0.06], name: 'meter-cock' }));
  weather(g, { w: 0.36, h: 0.2, pos: [0, 0.1, 0.157], rot: [0, 0, 0], kind: 'moss', color: '#6b8450', opacity: 0.42, seed: 53, density: 1.5 });
  return g;
}

/** 屋根上 TV アンテナ（マスト・ブーム・八木素子・UHF 輪） */
function tvAntenna(M, { pos, rotYdeg = 0, s = 1 }) {
  const g = grp('tv-antenna', { pos, rotY: rotYdeg, scale: s });
  g.add(mesh(cyl(0.022, 0.026, 0.9, 8), M.galv, { pos: [0, 0.45, 0], name: 'ant-mast' }));
  g.add(mesh(box(0.14, 0.05, 0.12), M.galv, { pos: [0, 0.03, 0], name: 'ant-base' }));
  g.add(mesh(cyl(0.026, 0.026, 0.03, 8), M.rustIron, { pos: [0, 0.06, 0.07], rot: [Math.PI / 2, 0, 0], name: 'ant-clamp' }));
  const boom = grp('ant-boom', { pos: [0, 0.82, 0], rot: [0, 0, -0.16] });
  boom.add(mesh(box(0.022, 0.022, 1.05), M.aluBright, { pos: [0, 0, 0], rot: [Math.PI / 2, 0, 0], name: 'ant-boom' }));
  row(boom, 9, 0.1, (i, x) => {
    const len = 0.5 - i * 0.028;
    return mesh(box(rq(len), 0.008, 0.008), M.aluBright, { pos: [0, 0.004, x], name: 'ant-element' });
  }, { z: 0, start: -0.42 });
  // UHF ループ（丸アンテナ）
  boom.add(mesh(tor(0.14, 0.008, 5, 12), M.aluBright, { pos: [0, 0.05, -0.5], rot: [0, Math.PI / 2, 0], name: 'ant-loop' }));
  boom.add(mesh(box(0.05, 0.04, 0.06), M.plasticDark, { pos: [0, -0.02, -0.3], name: 'ant-balun' }));
  g.add(boom);
  // 同軸の垂れ
  const coax = [[0, 0.8, 0], [0.05, 0.5, 0.05], [0.02, 0.2, 0.09], [0, 0.02, 0.06]];
  for (let i = 0; i < coax.length - 1; i++) {
    const a = coax[i], b = coax[i + 1];
    g.add(mesh(cyl(0.008, 0.008, rq(Math.hypot(b[0] - a[0], b[1] - a[1], b[2] - a[2])), 6), M.rubberMat,
      { pos: [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2, (a[2] + b[2]) / 2], name: 'ant-coax' }));
  }
  weather(g, { w: 0.1, h: 0.7, pos: [0.03, 0.4, 0.02], kind: 'rust', color: '#8a5230', opacity: 0.4, seed: 61, density: 1.2, count: 2, spread: 0.05 });
  return g;
}

/** BS アンテナ（壁面／屋根面のパラボラ） */
function bsDish(M, { pos, rotYdeg = 0, tilt = 0.5 }) {
  const g = grp('bs-dish', { pos, rotY: rotYdeg });
  g.add(mesh(cyl(0.018, 0.018, 0.34, 8), M.galv, { pos: [0, 0.17, 0], name: 'dish-mast' }));
  const d = grp('dish-head', { pos: [0, 0.34, 0.03], rot: [tilt, 0, 0] });
  d.add(mesh(cyl(0.26, 0.2, 0.06, 22, true), M.panelWhite, { name: 'dish-shell' }));
  d.add(mesh(cyl(0.24, 0.24, 0.012, 22), M.panelWhite, { pos: [0, -0.026, 0], name: 'dish-face' }));
  d.add(mesh(cyl(0.02, 0.02, 0.2, 6), M.aluBright, { pos: [0, 0.06, 0.05], rot: [Math.PI / 2 - tilt, 0, 0], name: 'dish-arm' }));
  d.add(mesh(box(0.05, 0.05, 0.07), M.plasticDark, { pos: [0, 0.1, 0.14], name: 'dish-horn' }));
  g.add(d);
  weather(g, { w: 0.5, h: 0.4, pos: [0, 0.3, 0.02], kind: 'chip', color: '#8b7f70', opacity: 0.34, seed: 67, density: 1.1 });
  return g;
}

/** 植木（低木：幹＋葉カード。常緑の南天／梅の木風） */
function shrub(M, { pos, h = 0.7, r = 0.36, seed = 3, tone = 0 }) {
  const g = grp('shrub', { pos });
  const rnd = rand(seed);
  g.add(mesh(cyl(0.014, 0.024, rq(h * 0.7), 6), M.barkSmall, { pos: [0, h * 0.35, 0], rot: [0, 0, range(rnd, -0.1, 0.1)], name: 'shrub-trunk' }));
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * 6.283 + range(rnd, -0.4, 0.4);
    g.add(mesh(cyl(0.008, 0.012, rq(h * 0.45), 5), M.barkSmall, {
      pos: [Math.cos(a) * 0.05, h * 0.62, Math.sin(a) * 0.05], rot: [Math.sin(a) * 0.5, 0, -Math.cos(a) * 0.5], name: 'shrub-branch',
    }));
  }
  const cards = Math.max(10, Math.round(r * r * 210));
  const geo = plane(0.11, 0.11, 1, 1).clone();
  geo.translate(0, 0.055, 0);
  g.add(inst(geo, M.leaf[tone], cards, (i, d, rr2, col) => {
    const u = rr2() * 6.283, v = Math.acos(1 - rr2() * 1.35);
    const rad = r * (0.62 + rr2() * 0.42);
    d.position.set(Math.sin(v) * Math.cos(u) * rad, h * 0.78 + Math.cos(v) * rad * 0.8 - r * 0.25, Math.sin(v) * Math.sin(u) * rad * 0.92);
    d.rotation.set(rr2() * 6.283, rr2() * 6.283, rr2() * 6.283);
    const s = 0.7 + rr2() * 0.8; d.scale.set(s, s, 1);
    const k = 0.84 + rr2() * 0.32; col.setRGB(k, k * (0.98 + rr2() * 0.06), k * 0.9);
  }, { name: 'shrub-leaves' }));
  return g;
}
...
export function build(options = {}) {
  ...
  const ridgeY = EAVE_TIP + SLOPE * (w / 2 + EAVE_X);
  const runX = w / 2 + EAVE_X;
  const slopeLen = Math.hypot(runX, runX * SLOPE);
  const ang = Math.atan(SLOPE);
  const roofZ = d + EAVE_Z * 2;
  const hx = w / 2, hz = d / 2;
  ...
  const plasterFaded = MAT.concrete({ base: '#ded6c2', repeat: 1, cracked: true, tint: '#f6efdf', sat: 0.9 });
  const clapboard = MAT.wood({ light: '#a3917142'.slice(0, 7), dark: '#6c5c46', repeat: 1, uv: { repeat: [1.2, 1.2] } });
  const M = {
    plaster, baseMat,
    ...
    meshFabric: MAT.paint('#39433f', { map: TEX.metal({ base: '#39433f', worn: 0.2, repeat: 14 }).map, side: THREE.DoubleSide, spec: 0.1 }),
