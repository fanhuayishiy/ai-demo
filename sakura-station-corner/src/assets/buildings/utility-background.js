...
import {
  grp, mesh, box, rbox, cyl, tor, lathe, tubeOf, catenary,
  finish, rand, range, weather, inst, plane,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 807, span: 36 };
...
function tower(M, { x, z, h, seed, lean = 0 }) {
  ...
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) {
    ...
    for (let k = 0; k < 4; k++) {
      g.add(mesh(cyl(0.008, 0.008, 0.10, 6), bolt, { pos: [px + (k % 2 ? 0.10 : -0.10), 0.29, pz + (k < 2 ? 0.10 : -0.10)], name: 'anchor-bolt' }));
      g.add(noOut(mesh(box(0.026, 0.008, 0.026), bolt, { pos: [px + (k % 2 ? 0.10 : -0.10), 0.342, pz + (k < 2 ? 0.10 : -0.10)], cast: false, name: 'washer-nut' })));
    }
  ...
  g.add(mesh(box(0.30, 0.20, 0.010), M.towerSign, { pos: [-0.30, 1.42, halfAt(0.2) + 0.04], name: 'danger-sign' }));
...
export function build(options = {}) {
  ...
  const rnd = rand(seed);
  const S = Math.max(12, options.span ?? 36);
  ...
  const tB = tower(M, { x: 12.6, z: -0.40, h: 6.1, seed: seed + 19, lean: -0.9 });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const s of [-1, 1]) {
      ...
      const geo = new THREE.ExtrudeGeometry(tri, { depth: 0.05, bevelEnabled: false, curveSegments: 1 });
