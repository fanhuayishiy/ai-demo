...
import {
  grp, mesh, box, rbox, cyl, tor, plane, lathe,
...
export const DEFAULT_OPTIONS = { kind: 'block', len: 3.6, h: null, seed: 811 };
...
function materials(seed) {
  return {
    ...
    meshFabric: MAT.paint('#4b5250', { map: TEX.metal({ base: '#4b5250', worn: 0.2, repeat: 14 }).map, side: THREE.DoubleSide, spec: 0.12 }),
    shadeFilm: MAT.plastic('#a8b7ab', { spec: 0.2, transparent: true, opacity: 0.66, side: THREE.DoubleSide }),
    ...
    bag: MAT.plastic('#e6e2d6', { spec: 0.5, transparent: true, opacity: 0.86, side: THREE.DoubleSide }),
    ...{ seed },
  };
}
...
function buildBlock(g, M, { L, H, seed }) {
  ...
  for (let i = 0; i < nPost; i++) {
    ...
    weather(pg, { w: 0.12, h: 0.42, pos: [0, 0.6, -(T + 0.10) / 2 - 0.004], rot: [0, Math.PI, 0], kind: 'dirt', color: '#5f584c', opacity: 0.3, seed: seed + 7 + i, density: 1.5 });
  }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    side.add(mesh(cyl(0.022, 0.022, rq(topY - 0.1), 10), M.pvc, { pos: [x, (topY - 0.1) / 2, -(T / 2 + 0.03)], name: 'pvc-riser' }));
    side.add(mesh(box(0.06, 0.024, 0.05), M.iron, { pos: [x, topY * 0.62, -(T / 2 + 0.012)], name: 'pipe-saddle' }));
    side.add(noOut(mesh(cyl(0.019, 0.019, 0.02, 9), M.void, { pos: [x, topY - 0.1, -(T / 2 + 0.03)], cast: false, name: 'pipe-mouth' })));
  }
  side.add(mesh(box(0.044, rq(topY * 1.16), 0.044), M.woodOld, { pos: [L * 0.18, topY * 0.56, -(T / 2 + 0.10)], rot: [0.14, 0, 0.09], name: 'shore-brace' }));
  side.add(mesh(box(0.20, 0.036, 0.044), M.woodOld, { pos: [L * 0.18, 0.02, -(T / 2 + 0.16)], name: 'shore-foot' }));
  const tag = grp('warning-tag', { pos: [-L * 0.05, topY * 0.78, -(T / 2 + 0.055)] });
  side.add(tag);
  ...
  edgeWeeds(body, M, { x0: -L / 2 + 0.05, x1: L / 2 - 0.05, z: -(T / 2 + 0.06), seed: seed + 19, n: 16 });
  return { topY };
}
...
function buildPlaster(g, M, { L, H, seed }) {
  ...
  for (let i = 0; i < n; i++) {
    const w = range(rnd, 0.20, 0.33), h = range(rnd, 0.13, 0.22), d = range(rnd, 0.13, 0.20);
  ...
  weather(body, { w: L * 0.7, h: 0.18, pos: [-L * 0.2, 0.12, -(T / 2 + 0.03)], rot: [0, Math.PI, 0], kind: 'moss', color: PAL.moss, opacity: 0.44, seed: seed + 23, density: 1.8 });
  ...
  const retro = grp('retro-mesh', { pos: [0, 0, -(T / 2 + 0.05)] });
  body.add(retro);
  ...
  edgeWeeds(body, M, { x0: -L / 2 + 0.08, x1: L / 2 - 0.08, z: -(T / 2 + 0.07), seed: seed + 31, n: 18 });
  return { capY };
}
...
function buildMesh(g, M, { L, H, seed }) {
  ...
  for (let i = 0; i < nBay; i++) {
    ...
    noOut(grill(panel, { w: pw - 0.05, h: ph - 0.06, nx, ny: Math.max(5, Math.round(ph / 0.075)), bar: 0.0035, mat: M.metalMesh ?? M.alu, pos: [0, 0, -0.002] }));
    if (tattered) {
      // 千切れた区域：框の内側 1/3 が欠けて、残った線が垂れる
      panel.add(noOut(mesh(box(pw * 0.30, ph * 0.34, 0.004), M.void, { pos: [pw * 0.28, -ph * 0.24, -0.004], cast: false, receive: false, name: 'mesh-hole' })));
      for (let k = 0; k < 4; k++) {
...
function coilWrap(mat, { x = 0, y = 0, r = 0.020, turns = 6 }) {
  ...
  const geo = new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts.map((p) => new THREE.Vector3(p[0], p[1], p[2]))), turns * 10, 0.0018, 5, false);
...
function buildGate(g, M, { L, H, seed }) {
  ...
  for (const dy of [0.26, DH - 0.22]) {
    ...
    leaf.add(hg);
    hg.add(mesh(box(0.026, 0.09, 0.010), M.iron, { pos: [0.006, 0, -0.004] }));
