...
import {
  grp, mesh, box, cyl, rbox, sph, cone, tubeOf,
  finish, rand, range, weather, decal, shadowBlob,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, jpText, memo } from '../../core/textures.js';
...
export const DEFAULT_OPTIONS = { kind: 'fascia', seed: 33 };
...
function buildFascia(g, K, seed, rnd) {
  ...
  for (const px of postXs) {
    ...
    for (const bx of [-0.045, 0.045]) {
      steel.add(noOut(mesh(cyl(0.0088, 0.0088, 0.050, 6), K.iron, { pos: [px + bx, -0.440, -0.18] })));
  ...
  decal(back, { map: TEX.gradient({ stops: [[0, 'rgba(255,232,186,1)'], [1, 'rgba(255,255,255,0)']] }), w: BW - 0.10, h: BH - 0.10, pos: [0, baseY + 0.28, -0.1250], rot: [0, Math.PI, 0], opacity: 0.30 });
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    wire.add(noOut(mesh(tubeOf([
      [x - 0.10, baseY + 0.02, -0.06], [x - 0.02, baseY - 0.06, -0.118], [x + 0.14, baseY - 0.12, -0.150],
    ], 0.0055, 10, 5), K.cable)));
  }
  wire.add(noOut(mesh(tubeOf([
  ...
  weather(steel, { w: 0.6, h: 0.10, pos: [-BW * 0.28, baseY + 0.65, -0.10], kind: 'rust', color: '#8a5236', opacity: 0.5, seed: seed + 47, density: 1.5, spread: 0.02 });
...
function buildPylon(g, K, seed, rnd) {
  ...
  weather(fnd, { w: 0.34, h: 0.10, pos: [-0.20, 0.14, 0.268], kind: 'dirt', color: '#7d7261', opacity: 0.5, seed: seed + 52, density: 1.5, spread: 0.02 });
  ...
  decal(head, { map: TEX.lightPanel({ text: '深夜料金', bg: '#f6f2e6', fg: '#3b4650', rows: [{ t: '22 時〜 5 時', v: '228 円' }, { t: 'レジ袋', v: '有料' }, { t: '公共料金', v: '取扱い' }] }), w: FW - 0.16, h: FH - 0.20, pos: [0, headY, -(FD / 2 + 0.004)], rot: [0, Math.PI, 0] });
  // 桜のイラスト（前面右下）＋ 春の帯
...
function buildPillar(g, K, seed, rnd) {
  ...
  weather(body, { w: 0.30, h: 0.40, pos: [-(CW / 2 + 0.003), 0.40, -0.04], rot: [0, -Math.PI / 2, 0], kind: 'dirt', color: '#8b8271', opacity: 0.5, seed: seed + 82, density: 1.6, spread: 0.06 });
  ...
  plate.add(mesh(rbox(0.34, 0.34, 0.012, 0.004, 2), K.faceWhite, { pos: [0.02, 1.90, -(CW / 2 + 0.016)], rot: [0, Math.PI, 0] }));
  decal(plate, { map: TEX.signboard({ text: '公共料金払込', sub: '収納代行・ATM 併設', bg: '#f2ede0', fg: '#4a5560', stripe: PAL.storeBand3 }), w: 0.28, h: 0.28, pos: [0.02, 1.90, -(CW / 2 + 0.025)], rot: [0, Math.PI, 0] });
  ...
  lamp.add(mesh(box(0.030, 0.058, 0.034), K.aluDk, { pos: [0, 0.0, -0.060] }));
  bolt(lamp, K, 0, 0.0, -0.076, [0, 0, Math.PI / 2], 0.0066);
  lamp.add(noOut(mesh(tubeOf([[0, -0.02, -0.070], [-0.05, -0.10, -0.076], [-0.06, -0.30, -0.050]], 0.0045, 10, 5), K.cable)));
  ...
  head.position.set(0.086, -0.050, 0.086);
  head.rotation.set(0.34, -0.78, 0.20);
  cam.add(head);
  head.add(mesh(cone(0.042, 0.048, 14), K.faceWhite, { pos: [0, -0.004, 0], rot: [Math.PI, 0, 0] }));        // 防雨フード
  ...
  head.add(noOut(mesh(cyl(0.016, 0.016, 0.006, 12), MAT.darkIron({ repeat: 6 }), { pos: [0, -0.080, 0], rot: [Math.PI / 2, 0, 0] })));
  ...
  cam.add(noOut(mesh(tubeOf([[0.02, 0.0, 0.02], [-0.03, -0.02, 0.0], [-0.10, -0.05, -0.04], [-0.16, -0.12, -0.10]], 0.0048, 14, 5), K.cable)));
