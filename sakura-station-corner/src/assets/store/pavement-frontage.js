...
import {
  grp, mesh, box, cyl, rbox, sph, tor, finish, rand, range, put, tubeOf,
...
export function build(options = {}) {
  ...
  for (const [x, z, w, h] of [[-3.30, -0.40, 1.2, 0.7], [0.70, -0.18, 1.5, 0.9], [3.90, 0.46, 1.1, 0.6]]) {              // 目地の白華
  ...
  for (const dx of [-0.19, 0.19]) {
    const pts = [[-0.72 + dx, 0.72], [-0.20 + dx, 0.20], [0.40 + dx, -0.14], [1.16 + dx, -0.28]];
  ...
  for (const [x, z, s] of [[pX + 0.70, -0.64, 0.46], [-4.06, -0.50, 0.40], [2.30, 0.44, 0.32]]) {                        // 油染み
  ...
  weather(surf, { w: 1.5, h: 0.36, pos: [-0.80, yB + 0.007, 0.36], rot: [-Math.PI / 2, 0, 0], kind: 'scratch', color: '#e7e2d4', opacity: 0.3, seed: seed + 311, density: 1.6, spread: 0.03 });
  ...
  for (let i = 0; i < 3; i++) {
    const m = mesh(box(0.29, 0.018, 0.29), i === 2 ? MAT.tactile({ kind: 'line', repeat: 1, base: '#cfa944', sat: 0.82 }) : tLine, {
      pos: [0, yB - 0.009, -0.56 + i * 0.30], rot: [0, range(rnd, -0.008, 0.008), 0], cast: false,
    });
    tac.add(m);
  }
  ...
  for (const [rx, rz, r] of [[-1.70, -0.23, 0.20], [2.20, -0.23, 0.17], [-3.86, 0.14, 0.14]]) {   // 植木鉢の置き跡の輪
  ...
  stick('段差注意', 'mind the step', pX - 0.52, yB + 0.007, -0.30, 0.40, 0.22, 90);          // 段の手前
  stick('ゴミ持帰 願', '美化推進', pX + 1.20, yP + 0.008, 0.06, 0.44, 0.20, 0, '#f2eee1', '#4a5259');
  for (const sx of [-1, 1]) {                                                                 // 出入口前の動線矢印（色褪せた白線）
    g.add(noOut(mesh(box(0.05, 0.008, 0.30), markWhite, { pos: [sx * 0.66, yB + 0.001, -0.30], rot: [0, sx * 0.10, 0], cast: false })));
  }
