...
import {
  grp, mesh, box, cyl, rbox, sph, tor, finish, rand, range, put, hoop, tubeOf, catenary, grill,
  weather, decal,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const wallClip = (parent, x, y, r, mat = galv) => {
    ...
    parent.add(c);
    c.add(mesh(box(0.026, 0.022, 0.018), mat, { pos: [0, 0, -0.008] }));
    c.add(noOut(mesh(tor(r + 0.004, 0.005, 5, 14, Math.PI * 1.25), mat, { rot: [0, 0, -0.4], cast: false })));
    bolt(c, 0, -0.018, -0.002, 0.0055);
    return c;
  };

  /* ================= １. 換気口ガラリ（角型：アルミ羽根・防虫网・内部ダクト） ================= */
  const ventA = put(grp('louver-rect'), -2.50, 2.40, 0);
  g.add(ventA);
  ...
  hood.add(noOut(hoop(0.158, 0.008, aluOld, { pos: [0, -0.030, 0], rot: [90 * D2R, 0, 0], cast: false })));
  ...
  for (const bx of [dFrom + 0.40, -0.40]) {                                                                      // 支持金物
    ...
    br.add(mesh(box(0.026, 0.020, 0.132), galv, { pos: [0, -0.020, -0.072] }));
    br.add(mesh(box(0.030, 0.150, 0.010), galv, { pos: [0, -0.080, -0.136] }));
  ...
  const tear = put(grp('insulation-tear'), -1.20, dY, dZ);
  duct.add(tear);
  ...
  tear.add(noOut(mesh(box(0.09, 0.003, 0.13), foil, { pos: [0.06, -0.10, 0.074], rot: [-0.6, 0.1, 0.4] })));
  ...
  duct.add(pan);
  pan.add(mesh(rbox(0.30, 0.016, 0.22, 0.005, 2), galv, { pos: [0, 0, -0.006] }));
  pan.add(mesh(box(0.30, 0.05, 0.008), galv, { pos: [0, 0.026, 0.10] }));
  for (const sx of [-1, 1]) pan.add(mesh(box(0.008, 0.05, 0.22), galv, { pos: [sx * 0.15, 0.026, -0.006] }));
  pan.add(mesh(cyl(0.011, 0.011, 0.02, 10), deep, { pos: [0.10, -0.012, 0.05], cast: false }));
  pan.add(mesh(tubeOf([[0.10, -0.024, 0.05], [0.13, -0.44, 0.09], [0.11, -1.10, 0.11], [0.13, -1.80, 0.10], [0.15, -2.42, 0.09]], 0.0105, 24, 7), MAT.rubber('#3a3d40')));
  ...
  wrap.add(noOut(mesh(box(0.10, 0.003, 0.08), foil, { pos: [0.23, -0.030, 0.060], rot: [-0.7, 0.1, 0.6] })));
  ...
  const insulator = (x, y) => {
    ...
    elec.add(it);
    it.add(mesh(box(0.058, 0.058, 0.026), galv, { pos: [0, 0, -0.010] }));
  ...
  for (let k = 0; k < 3; k++) {
    ...
    iA.add(noOut(mesh(cyl(0.006, 0.006, 0.030, 6), iron, { pos: [off, -0.030, 0.060], cast: false })));
  }
  ...
  iB.add(noOut(mesh(cyl(0.006, 0.006, 0.030, 6), iron, { pos: [0, -0.030, 0.060], cast: false })));
  ...
  for (const dy of [0.10, -0.10]) hatch.add(mesh(box(hW - 0.06, 0.010, 0.006), steelPaint, { pos: [hx, hy + dy, 0.040] }));
  ...
  head.add(noOut(mesh(cyl(0.017, 0.017, 0.012, 14), MAT.glassLite({ color: '#e8dfd0', opacity: 0.45 }), { pos: [0.030, -0.004, 0], rot: [0, Math.PI / 2, 0], cast: false })));
  ...
  shade.add(noOut(mesh(cyl(0.086, 0.086, 0.030, 18), MAT.bulb({ color: '#fff6e0' }), { pos: [0, -0.006, 0], cast: false })));
  ...
  weather(shade, { w: 0.20, h: 0.06, pos: [0, -0.030, 0.0], kind: 'dirt', color: '#4b4a44', opacity: 0.5, seed: seed + 277, density: 1.6, spread: 0.006 });
  ...
  decal(g, { map: TEX.gradient({ stops: [[0, 'rgba(255,246,222,1)'], [1, 'rgba(255,240,206,0)']] }), w: 0.6, h: 1.2, pos: [-2.60, 1.48, 0.013], opacity: 0.26 });
  ...
  weather(wear, { w: 1.6, h: 0.5, pos: [-1.10, 2.30, 0.008], kind: 'chip', color: '#c2bbab', opacity: 0.4, seed: seed + 261, density: 1.4, spread: 0.03 });
  ...
  for (const [x, y, w, h] of [[-0.20, 2.30, 0.86, 0.014], [-2.16, 1.02, 0.014, 0.72], [2.66, 1.32, 0.014, 0.50]]) {
