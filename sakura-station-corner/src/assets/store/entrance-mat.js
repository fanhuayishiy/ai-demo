...
import {
  grp, mesh, box, cyl, rbox, plane, tubeOf,
...
export const DEFAULT_OPTIONS = { w: 1.6, d: 0.9, seed: 55 };
...
export function build(options = {}) {
  ...
  const mkFlap = (x, z, dir, len, wd, ang) => {
    ...
    tilt.add(noOut(mesh(box(wd - 0.016, 0.0022, len - 0.016), rubberCore, { pos: [0, -0.0058, -len / 2], cast: false })));
    return tilt;
  };
  ...
  weather(fA, { w: 0.14, h: 0.10, pos: [0, 0.0062, -0.10], rot: [-Math.PI / 2, 0, 0], kind: 'dirt', color: '#5d5647', opacity: 0.5, seed: seed + 13, density: 1.4, spread: 0.003 });
  ...
  step.add(mesh(box(W + 0.10, 0.018, 0.010), aluDk, { pos: [0, -0.002, fz1 + 0.005] }));                            // 押えリブ
  ...
  weather(step, { w: 0.30, h: 0.10, pos: [W * 0.3, -0.070, fz1 + 0.006], kind: 'scratch', color: '#e6e6e2', opacity: 0.4, seed: seed + 22, density: 1.4, spread: 0.01 });
