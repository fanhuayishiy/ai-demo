...
import {
  grp, mesh, box, cyl, rbox, tor, sph, lathe, tubeOf, coil,
...
export function build(options = {}) {
  ...
  radial(base, 4, 0.070, (i, a, x, z) => {
    ...
    b.add(mesh(cyl(0.0142, 0.0142, 0.005, 6), M.rust, { name: 'washer', pos: [0, -0.008, 0] }));
    return b;
  });
  ...
  if (kind === 'post') {
    ...
    hk.add(bendHook(hk, M.steel, seed + 5));
    hk.add(mesh(rbox(0.020, 0.026, 0.008, 0.003, 2), M.padlock, { name: 'spare-key-tag', pos: [0.026, -0.030, 0] }));
    P.add(hk);
    // 錆・掉漆・擦れ（車輪が当たる面）
  ...
  if (kind === 'ulock') {
    ...
    lock.add(mesh(rbox(0.020, 0.016, 0.006, 0.003, 2), M.rubber, { name: 'keyhole-flap', pos: [0.002, -0.020, 0.066], rot: [0, 0, d2r(24)] }));
    lock.add(mesh(box(0.030, 0.006, 0.014), M.white, { name: 'model-plate', pos: [0.032, 0.006, -0.020] }));
    ...
    U.add(mesh(cyl(0.0108, 0.0108, 0.026, 8), M.chrome, { name: 'shackle-leg-L', pos: [0, 0.286, -0.080], rot: [d2r(8), 0, 0] }));
    ...
    weather(U, { w: 0.08, h: 0.06, pos: [0, 0.286, -0.070], kind: 'rust', color: PAL.rust, opacity: 0.45, seed: seed + 19, spread: 0.03 });
    // 鍵（差しっぱなしのキー＋キーリング）
  ...
  if (kind === 'chain') {
    ...
    for (let i = 0; i <= 46; i++) {
      ...
      cpts.push(new THREE.Vector3(Math.cos(a) * rr, yy, Math.sin(a) * rr));
    }
    cpts.push(new THREE.Vector3(0.118, 0.072, 0.060));
    cpts.push(new THREE.Vector3(0.128, 0.030, 0.018));
    cpts.push(new THREE.Vector3(0.112, 0.020, -0.030));
    const chainCurve = new THREE.CatmullRomCurve3(cpts, false, 'catmullrom', 0.05);
    const CH = grp('chain');
    ...
    along(CH, chainCurve, 44, (i, p, t) => {
      ...
      mm.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), tg);
    ...
    pl.add(mesh(cyl(0.0075, 0.0075, 0.010, 10), M.postDark, { name: 'padlock-keyhole', pos: [0, -0.006, 0.016], rot: [Math.PI / 2, 0, 0] }));
    ...
    // 地面に接する部分の泥
    weather(C, { w: 0.14, h: 0.045, pos: [0.112, 0.056, -0.020], rot: [0, Math.PI / 2, 0], kind: 'dirt', color: '#6d6250', opacity: 0.5, seed: seed + 25, spread: 0.008 });
  }
...
function bendHook(parent, mat, seed) {
  ...
  for (let i = 0; i <= 10; i++) {
    ...
    pts.push(new THREE.Vector3(0.018 + Math.cos(a) * 0.020, -Math.sin(a) * 0.026, 0));
  }
