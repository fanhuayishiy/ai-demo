...
import {
  grp, mesh, box, cyl, rbox, tor, sph, lathe, tubeOf, coil, grill, shadowBlob,
  finish, rand, range, weather, decal, radial, along, catenary, d2r,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, shade } from '../../core/textures.js';
...
const SC = [0, 0.768, -0.190];               // シートクラスター
...
function ring(pos, dir, r, t, mat, name = 'weld') {
  ...
  mm.quaternion.setFromUnitVectors(FZ, new THREE.Vector3(dir[0], dir[1], dir[2]).normalize());
  return mm;
}
/** 六角ボルト */
...
function tubeDecal(parent, { at, axis, side, w = 0.05, h = 0.10, kind = 'chip', color = PAL.rust, opacity = 0.5, seed = 1, offset = 0.0016 }) {
  ...
  const m4 = new THREE.Matrix4().makeBasis(b, a, n);
  const mm = new THREE.Mesh(new THREE.PlaneGeometry(w, h),
    MAT.decal({ map: TEX.wear({ kind, color, seed, density: 1.25 }), opacity, side: THREE.DoubleSide, order: 1 }));
...
function tireMesh(R, t, squash, mat, name = 'tire') {
  const mm = mesh(new THREE.TorusGeometry(R, t, 12, 44), mat, { name });
  if (squash < 1) {
    mm.scale.y = squash;
...
function wheel(o) {
  ...
  if (drum) {                                                 // ローラーブレーキ ドラム（左）
    ...
    hub.add(ring([0, 0, -0.070], [0, 0, 1], 0.044, 0.004, M.chrome, 'drum-ring'));
  }
  w.add(hub);
  ...
  rg.add(mesh(box(0.012, 0.010, 0.014), M.darkMetal, { name: 'reflector-clip', pos: [rmid, 0, -0.002] }));
  w.add(rg);
...
export function build(options = {}) {
  ...
  const M = {
    ...
    rim: MAT.metal('#c8ccd0', { worn: 0.45, repeat: 3, side: THREE.DoubleSide }),
    rimDark: MAT.metal('#8d9298', { worn: 0.55, side: THREE.DoubleSide }),
    ...
    fender: MAT.hardPlastic(shade(frame, 0.44), { worn: 0.7, repeat: 3, side: THREE.DoubleSide }),
    guard: MAT.hardPlastic('#eee9de', { worn: 0.6 }),
    guardSide: MAT.hardPlastic('#eee9de', { worn: 0.6, side: THREE.DoubleSide }),
    ...
    reflector: MAT.hardPlastic('#d1402f', { worn: 0.35, side: THREE.DoubleSide }),
  ...
  for (const s of [-1, 1]) {
    ...
    // 泥除けゴム（チェーンステー後端）
    F.add(mesh(rbox(0.012, 0.006, 0.10, 0.002, 2), M.rubberCap, { name: 'chainstay-mudflap', pos: [s * 0.048, 0.238, -0.300], rot: [d2r(4), 0, 0] }));
  }
  // 溶接ビード
  ...
  // 経年（掉漆・錆・傷・泥）
  {
    ...
    tubeDecal(F, { at: [-0.0195, 0.655, -0.150], axis: stAx, side: [-1, 0, 0], w: 0.030, h: 0.130, kind: 'rust', color: PAL.rust, opacity: 0.48, seed: seed + 11 });
    ...
    tubeDecal(F, { at: [0.0509, 0.2965, -0.190], axis: csAx, side: [0, 1, 0], w: 0.022, h: 0.160, kind: 'dirt', color: '#6d5f4b', opacity: 0.5, seed: seed + 27 });
  }
  ...
  tubeDecal(H, { at: [0.0555, 0.452, 0.418], axis: [0, -0.060, 0.082], side: [1, 0, 0], w: 0.026, h: 0.110, kind: 'rust', color: PAL.rust, opacity: 0.48, seed: seed + 5 });
  ...
  for (const s of [-1, 1]) {
    ...
    lev.add(bend([[0.012, 0, -0.098], [0.014, 0, -0.126], [0.020, 0.004, -0.140]], 0.0055, M.chrome, 'lever-tip', 6));
    HB.add(lev);
    HB.add(bolt([s * 0.240, 0.896, 0.062], 0.0055, M.darkMetal, 'y', 'reach-screw'));
  }
  // ベル
  const bell = grp('bell', { pos: [-0.180, 0.902, 0.150], rot: [0, d2r(-22), 0] });
  bell.add(mesh(sph(0.026, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.52), M.chrome, { name: 'bell-dome' }));
  bell.add(mesh(cyl(0.0265, 0.0245, 0.007, 14), M.steel, { name: 'bell-rim', pos: [0, -0.001, 0] }));
  bell.add(mesh(cyl(0.0075, 0.0075, 0.014, 8), M.darkMetal, { name: 'bell-post', pos: [0, -0.009, 0] }));
  bell.add(mesh(box(0.007, 0.004, 0.026), M.chrome, { name: 'bell-striker', pos: [0.014, 0.002, 0.012], rot: [0, d2r(20), 0] }));
  HB.add(bell);
  // ミラー（左・球部を多少振り向いた状態）
  const mir = grp('mirror', { pos: [-0.300, 0.898, 0.052], rot: [d2r(-14), d2r(24), d2r(28)] });
  ...
  shf.add(mesh(box(0.009, 0.004, 0.028), M.darkMetal, { name: 'shifter-lever', pos: [0.012, -0.009, 0.004] }));
  HB.add(shf);
  ...
  sad.add(mesh(rbox(0.120, 0.020, 0.198, 0.026, 3), M.saddleTop, { name: 'saddle-crown', pos: [0, 0.017, -0.008] }));
  sad.add(mesh(rbox(0.054, 0.024, 0.072, 0.014, 2), M.saddle, { name: 'saddle-nose', pos: [0, -0.002, 0.150] }));
  sad.add(mesh(rbox(0.130, 0.020, 0.054, 0.012, 2), M.saddle, { name: 'saddle-tail', pos: [0, -0.008, -0.148] }));
  for (const s of [-1, 1]) {
  ...
  sad.add(bolt([-0.044, -0.038, -0.030], 0.0052, M.darkMetal, 'x', 'rail-bolt-r'));
  ...
  for (const s of [-1, 1]) {
    ...
    const sp = mesh(coil(0.0135, 0.050, 5, 10, 0.0032), M.steel, { name: 'seat-spring', pos: [s * 0.046, 0.912, -0.210], rot: [d2r(12), 0, s * d2r(6)] });
  ...
  const pAt = (c, r, a, drop = 0) => new THREE.Vector3(chainX, c[1] + Math.sin(a) * r - drop, c[0] + Math.cos(a) * r);
  const pathPts = [];
  ...
  {                                                                                                 // 下側ラン（垂れ）
    ...
    pathPts.push(p1);
    pathPts.push(p1.clone().lerp(p2, 0.38).sub(new THREE.Vector3(0, sag, 0)));
    pathPts.push(p1.clone().lerp(p2, 0.66).sub(new THREE.Vector3(0, sag * 0.86, 0)));
    pathPts.push(p2);
  }
  const wrap = Math.PI * 2 - (aTop - aBot);                                                          // ギア後側を回る 172°
  for (let i = 1; i <= 10; i++) pathPts.push(pAt(cG, cogR, aBot - (wrap * i) / 10));
  const chainCurve = new THREE.CatmullRomCurve3(pathPts, true, 'catmullrom', 0.02);
  ...
  {
    const plate = [
      new THREE.Vector3(gx, BB[1] + ringR * Math.sin(aTop) + 0.016, BB[2] + ringR * Math.cos(aTop)),
      new THREE.Vector3(gx, 0.404, -0.200),
      new THREE.Vector3(gx, 0.398, -0.400),
      new THREE.Vector3(gx, RW + cogR + 0.016, ZR + 0.020),
      new THREE.Vector3(gx, RW + cogR + 0.012, ZR - 0.058),
    ];
    const curve = new THREE.CatmullRomCurve3(plate, false, 'catmullrom', 0.3);
  ...
  CG.add(bolt([gx + 0.004, 0.392, -0.300], 0.0062, M.darkMetal, 'x', 'guard-bolt2'));
  ...
  tubeDecal(CG, { at: [gx + 0.0026, 0.398, -0.420], axis: [0, -0.006, -0.10], side: [1, 0, 0], w: 0.040, h: 0.130, kind: 'chip', color: '#9c937e', opacity: 0.55, seed: seed + 37 });
  ...
  lamp.add(mesh(cyl(0.0105, 0.0105, 0.028, 8), M.darkMetal, { name: 'lamp-stem', pos: [0, -0.044, -0.010] }));
  lamp.add(mesh(rbox(0.032, 0.012, 0.028, 0.004, 2), M.steel, { name: 'lamp-plate', pos: [0, -0.058, -0.008] }));
  lamp.add(bolt([0.018, -0.058, -0.008], 0.0058, M.darkMetal, 'x', 'lamp-bolt'));
  lamp.add(bend([[0, -0.058, 0.004], [-0.034, -0.070, 0.056], [-0.050, -0.050, 0.118]], 0.0028, M.wire, 'lamp-wire', 10));
  L.add(lamp);
  ...
  dyn.add(bend([[0, -0.014, 0], [0, -0.030, 0.014], [0, -0.044, 0.030]], 0.0042, M.steel, 'dynamo-arm', 8));
  dyn.add(mesh(cyl(0.0086, 0.0086, 0.015, 10), M.rubberCap, { name: 'dynamo-roller', pos: [0, -0.050, 0.034], rot: [0, 0, Math.PI / 2] }));
  dyn.add(bolt([0, 0.044, 0], 0.0062, M.darkMetal, 'y', 'dynamo-bolt'));
  dyn.add(bend([[0.014, 0.020, 0], [0.030, 0.012, 0.062], [0.020, -0.020, 0.118]], 0.0026, M.wire, 'dynamo-lead', 10));
  L.add(dyn);
  // 尾灯（キャリア下）
  ...
  tl.add(mesh(rbox(0.030, 0.022, 0.004, 0.004, 2), M.redGlow, { name: 'tail-lens', pos: [0, 0, -0.010] }));
  ...
  rref.add(mesh(rbox(0.044, 0.026, 0.0025, 0.004, 2), M.redGlow, { name: 'rear-reflector-face', pos: [0, 0, -0.003] }));
  L.add(rref);
  // サドル後方の小型反射板・太阳能テール
  ...
  for (const s of [-1, 1]) {
    ...
    RC.add(bend([[s * 0.076, rackY - 0.008, rackZ1 + 0.100], [s * 0.062, 0.614, -0.520], [s * 0.058, RW + 0.026, ZR - 0.040]], 0.0058, M.steel, 'rack-stay-rear', 12));
  ...
  clip.add(bend([[-0.052, 0, 0], [-0.030, 0.030, 0.006], [0, 0.040, 0.004], [0.030, 0.030, 0.006], [0.052, 0, 0]], 0.0042, M.chrome, 'clip-wire', 16));
  clip.add(bolt([0, 0.004, -0.010], 0.0075, M.darkMetal, 'x', 'clip-pivot'));
  RC.add(clip);
  ...
  const ST = grp('stand');
  bike.add(ST);
  const px = -0.050, py = 0.246, pz = -0.050;
  ...
  tubeDecal(ST, { at: [-0.118, 0.145, -0.004], axis: [0, -0.104, 0.048], side: [-1, 0, 0], w: 0.022, h: 0.130, kind: 'rust', color: PAL.rust, opacity: 0.6, seed: seed + 59 });

  /* ============ 11. U字ロック（前輪まわりを施錠・カバー剥がれ） ============ */
  const UL = grp('u-lock');
  bike.add(UL);
  const ulx = -0.100, ulz = ZR + 0.150;
  ...
  for (const c of [{ y: 0.556, z: ZF + 0.298 }, { y: 0.560, z: ZR - 0.058 }]) {
    ...
    for (const s of [-1, 1]) {
      cal.add(seg([s * 0.012, -0.006, 0], [s * 0.030, -0.124, 0.010], 0.0055, M.steel, 'caliper-arm'));
  ...
  BR.add(bend([[0, 0.560, ZR - 0.058], [-0.040, 0.522, ZR - 0.090], [-0.062, 0.436, ZR - 0.072], [-0.062, 0.400, ZR - 0.032]], 0.0036, M.steel, 'roller-brake-rod', 14));
  // 変速ワイヤー
  ...
  bike.add(rear);
  const cap = mesh(lathe([[0, -0.013], [0.070, -0.012], [0.116, -0.004], [0.126, 0.003], [0.118, 0.007], [0.070, 0.003], [0, 0.003]], 24), M.white, { name: 'hub-cap-shell', pos: [-0.0720, RW, ZR], rot: [0, 0, Math.PI / 2] });
  bike.add(cap);
  cap.userData.tint = 1;
  ...
  /* ============ 14. 全体做旧・接地影 ============ */
  tubeDecal(bike, { at: [0.0120, 0.216, -0.140], axis: [0, 0.063, -0.538], side: [0, 1, 0], w: 0.024, h: 0.140, kind: 'dirt', color: '#6d6250', opacity: 0.45, seed: seed + 67 });
  shadowBlob(g, { r: 0.27, pos: [0, 0.001, ZR], opacity: 0.22 });
  shadowBlob(g, { r: 0.27, pos: [0, 0.001, ZF], opacity: 0.20 });
  shadowBlob(g, { r: 0.11, pos: [-0.20, 0.001, 0.05], opacity: 0.16 });
