...
import {
  grp, mesh, box, cyl, rbox, tor, sph, lathe, tubeOf,
  finish, rand, range, weather, decal, radial, d2r,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const M = {
    ...
    acrylic: MAT.glassLite({ color: '#f0e4bc', opacity: 0.42, side: THREE.DoubleSide }),
  ...
  radial(base, 4, 0.062, (i, a, x, z) => {
    ...
    b.add(mesh(cyl(0.0130, 0.0130, 0.006, 6), M.bolt, { name: 'washer', pos: [0, -0.008, 0] }));
    return b;
  });
  // 支柱（溶接継ぎ・塗装の傷み）
  const post = grp('post');
  ...
  weather(frame, { w: 0.20, h: 0.16, pos: [-0.200, 0.060, 0.024], kind: 'dirt', color: '#efe6cf', opacity: 0.42, seed: seed + 15, spread: 0.04 });
  weather(frame, { w: 0.11, h: 0.09, pos: [0.232, -0.132, 0.024], kind: 'moss', color: PAL.moss, opacity: 0.28, seed: seed + 17, spread: 0.014 });
  B.add(frame);
  // 支柱への金具
  for (const sy of [-0.060, 0.120]) {
    ...
    B.add(mesh(cyl(0.0110, 0.0110, 0.008, 6), M.bolt, { name: 'band-nut', pos: [-0.046, boardY + sy, -0.030], rot: [0, 0, Math.PI / 2] }));
  }
  ...
  weather(tb, { w: 0.16, h: 0.05, pos: [0.05, -0.030, 0.016], kind: 'dirt', color: '#8b8268', opacity: 0.35, seed: seed + 19, spread: 0.02 });
  g.add(tb);
  ...
  for (let i = 0; i < 9; i++) {
    const t = mesh(rbox(0.170, 0.112, 0.0035, 0.0015, 1), M.ticket, { name: 'ticket-' + i, pos: [(i % 3 - 1) * 0.004, -0.004 - i * 0.0022, 0.020 - i * 0.0035], rot: [d2r(-2 + i * 0.6), d2r((i % 2 ? 1 : -1) * 2.4), d2r(i % 2 ? 1.2 : -1.6)] });
    t.userData.noOutline = true;
    tk.add(t);
  }
  decal(tk, { map: TEX.signboard({ text: '整理券', sub: 'No. 042', bg: '#f7f0dc', fg: '#b8433a', ar: 1.6 }), w: 0.150, h: 0.092, pos: [0, -0.004, 0.024], opacity: 0.95 });
  PO.add(tk);
  // 投入口・蓋・錠前
  PO.add(mesh(rbox(0.150, 0.020, 0.030, 0.004, 2), M.postDark, { name: 'slot-lip', pos: [0, 0.104, 0.028] }));
  const lid = grp('box-lid', { pos: [0, 0.100, -0.010], rot: [d2r(-24), 0, 0] });
  lid.add(mesh(rbox(0.226, 0.012, 0.100, 0.004, 2), M.acrylicFrame, { name: 'lid-plate' }));
  lid.add(mesh(cyl(0.0080, 0.0080, 0.014, 8), M.bolt, { name: 'lid-hinge-L', pos: [-0.090, 0.004, -0.046], rot: [0, 0, Math.PI / 2] }));
  ...
  // 取付バンド（支柱に巻く）
  for (const by of [0.052, -0.060]) {
    PO.add(mesh(tor(0.0385, 0.0060, 5, 16), M.steel, { name: 'band', pos: [-0.036, by, -0.030], rot: [0, Math.PI / 2, 0] }));
    PO.add(mesh(box(0.016, 0.020, 0.014), M.bolt, { name: 'band-bolt', pos: [-0.036, by, 0.008] }));
  }
  weather(PO, { w: 0.16, h: 0.10, pos: [0.030, -0.060, 0.054], kind: 'dirt', color: '#8b7f5e', opacity: 0.4, seed: seed + 21, spread: 0.03 });
  weather(PO, { w: 0.12, h: 0.12, pos: [-0.080, 0.030, 0.054], kind: 'scratch', color: '#cfc9b4', opacity: 0.3, seed: seed + 23, spread: 0.03 });
