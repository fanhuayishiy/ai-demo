...
function wheel(o) {
  ...
  if (icoHub) {                                      // 3段内装ハブ（太いドラム）
    ...
    hub.add(mesh(box(0.014, 0.010, 0.020), M.darkMetal, { name: 'igh-cable-stub', pos: [0.026, 0.014, -0.030] }));
    hub.add(ring([0, 0, -0.060], [0, 0, 1], 0.026, 0.0035, M.chrome, 'igh-ring'));
  } else {
    hub.add(mesh(cyl(0.0205, 0.0205, 0.058, 14), M.hub, { name: 'hub-body', rot: [Math.PI / 2, 0, 0] }));
  }
  hub.add(mesh(cyl(0.0255, 0.0200, 0.018, 14), M.hub, { name: 'hub-flange-L', pos: [0, 0, -0.030], rot: [Math.PI / 2, 0, 0] }));
  ...
  rg.add(mesh(box(0.011, 0.009, 0.013), M.darkMetal, { name: 'reflector-clip', pos: [(rIn + rOut) * 0.6, 0, -0.002] }));
  w.add(rg);
  return w;
}
...
export function build(options = {}) {
  ...
  const M = {
    ...
    rim: MAT.metal('#d3d7da', { worn: 0.35, repeat: 3, side: THREE.DoubleSide }),
    rimDark: MAT.metal('#9aa0a6', { worn: 0.45, side: THREE.DoubleSide }),
    ...
    fender: MAT.hardPlastic(shade(accent, 0.86), { worn: 0.55, repeat: 3, side: THREE.DoubleSide }),
    skirt: MAT.hardPlastic(shade(frame, 0.96), { worn: 0.5, side: THREE.DoubleSide }),
    guard: MAT.hardPlastic(shade(frame, 0.98), { worn: 0.5 }),
    guardSide: MAT.hardPlastic(shade(frame, 0.98), { worn: 0.5, side: THREE.DoubleSide }),
    ...
    reflector: MAT.hardPlastic('#d1402f', { worn: 0.3, side: THREE.DoubleSide }),
  ...
  F.add(ring([0, 0.300, -0.014], [0, -0.008, -0.074], 0.0230, 0.0028, M.frameDark, 'weld-step'));
  ...
  tubeDecal(F, { at: [0.0215, 0.402, 0.216], axis: [0, -0.110, 0.072], side: [1, 0, 0], w: 0.030, h: 0.085, kind: 'chip', color: '#9fa4a8', opacity: 0.55, seed: seed + 3 });
  tubeDecal(F, { at: [-0.0210, 0.640, -0.140], axis: [0, SC[1] - BB[1], SC[2] - BB[2]], side: [-1, 0, 0], w: 0.028, h: 0.110, kind: 'rust', color: PAL.rust, opacity: 0.4, seed: seed + 11 });
  tubeDecal(F, { at: [0.0120, 0.290, -0.180], axis: [0, RW - BB[1], ZR - BB[2]], side: [0, 1, 0], w: 0.022, h: 0.150, kind: 'dirt', color: '#6d5f4b', opacity: 0.5, seed: seed + 27 });
  tubeDecal(F, { at: [0, 0.7585, -0.190], axis: [0, SC[1] - BB[1], SC[2] - BB[2]], side: [0, 1, 0], w: 0.026, h: 0.090, kind: 'scratch', color: '#c7ccd0', opacity: 0.35, seed: seed + 17 });
  ...
  bell.add(mesh(cyl(0.0068, 0.0068, 0.013, 8), M.darkMetal, { name: 'bell-post', pos: [0, -0.008, 0] }));
  ...
  lamp.add(mesh(rbox(0.024, 0.020, 0.020, 0.004, 2), M.darkMetal, { name: 'lamp-strap', pos: [0, 0.020, -0.030] }));
  HB.add(lamp);
  ...
  sad.add(mesh(rbox(0.128, 0.018, 0.186, 0.026, 3), M.saddle, { name: 'saddle-crown', pos: [0, 0.016, -0.006] }));
  sad.add(mesh(rbox(0.056, 0.022, 0.068, 0.014, 2), M.saddle, { name: 'saddle-nose', pos: [0, -0.002, 0.142] }));
  for (const s of [-1, 1]) sad.add(bend([[s * 0.030, -0.024, 0.082], [s * 0.032, -0.032, 0], [s * 0.030, -0.030, -0.092]], 0.0040, M.chrome, 'saddle-rail', 12));
  sad.add(mesh(rbox(0.078, 0.018, 0.034, 0.005, 2), M.darkMetal, { name: 'rail-cradle', pos: [0, -0.034, 0] }));
  // サドルカバー（伸びたゴム布・端にシワ）
  const cov = grp('saddle-cover', { pos: [0, 0.022, -0.004], rot: [d2r(-2), 0, 0] });
  cov.add(mesh(rbox(0.170, 0.014, 0.262, 0.040, 3), M.saddleCover, { name: 'cover-top' }));
  for (const s of [-1, 1]) {
    cov.add(mesh(rbox(0.012, 0.030, 0.070, 0.006, 2), M.saddleCover, { name: 'cover-skirt', pos: [s * 0.082, -0.014, -0.030], rot: [0, 0, s * d2r(14)] }));
  }
  cov.add(mesh(rbox(0.070, 0.010, 0.030, 0.004, 2), M.saddleCover, { name: 'cover-nose-tuck', pos: [0, -0.006, 0.132], rot: [d2r(24), 0, 0] }));
  sad.add(cov);
  ...
  pathPts.push(p1, p1.clone().lerp(p2, 0.5).sub(new THREE.Vector3(0, sag, 0)), p2);
  ...
  {
    const plate = [
      new THREE.Vector3(gx, BB[1] + ringR * Math.sin(aTop) + 0.015, BB[2] + ringR * Math.cos(aTop)),
      new THREE.Vector3(gx, 0.396, -0.190),
      new THREE.Vector3(gx, 0.392, -0.380),
      new THREE.Vector3(gx, RW + cogR + 0.016, ZR + 0.018),
      new THREE.Vector3(gx, RW + cogR + 0.012, ZR - 0.056),
    ];
  ...
  tubeDecal(CG, { at: [gx + 0.0026, 0.392, -0.400], axis: [0, -0.006, -0.10], side: [1, 0, 0], w: 0.036, h: 0.110, kind: 'chip', color: '#9c937e', opacity: 0.5, seed: seed + 37 });
  ...
  if (basket) {
    ...
    umb.add(mesh(cyl(0.0050, 0.0050, 0.120, 8), M.darkMetal, { name: 'umbrella-shaft', pos: [0, -0.130, 0] }));
    ...
    mnt.add(seg([-0.100, yBase - 0.010, z0 - 0.006], [-0.052, 0.544, 0.376], 0.0072, M.steel, 'mount-L'));
  ...
  sk.add(skArc);
  for (const [yy, zz] of [[0.560, -0.400], [0.606, -0.545], [0.520, -0.690]]) {
  ...
  rref.add(mesh(rbox(0.046, 0.028, 0.0025, 0.004, 2), M.redGlow, { name: 'rear-reflector-face', pos: [0, 0, -0.003] }));
  L.add(rref);
  ...
  rc.add(bend([[-0.100, -0.020, 0], [-0.050, -0.048, 0.010], [0.050, -0.048, -0.010], [0.100, -0.020, 0]], 0.0028, M.wire, 'roll-cord', 14));
  bike.add(rc);

  /* ---------- 9. リアキャリア（小型）＋防犯登録札 ---------- */
  const RC = grp('rear-carrier');
  bike.add(RC);
  const rackY = 0.700, rackZ0 = -0.250, rackZ1 = -0.790;
  for (const s of [-1, 1]) {
    ...
    RC.add(bend([[s * 0.072, rackY - 0.010, rackZ1 + 0.090], [s * 0.058, 0.600, -0.480], [s * 0.057, RW + 0.024, ZR - 0.038]], 0.0055, M.steel, 'rack-stay-rear', 12));
  ...
  ST.add(spr);
  tubeDecal(ST, { at: [-0.112, 0.140, -0.002], axis: [0, -0.100, 0.044], side: [-1, 0, 0], w: 0.020, h: 0.115, kind: 'rust', color: PAL.rust, opacity: 0.5, seed: seed + 59 });
  ...
  for (const c of [{ y: 0.548, z: ZF + 0.292 }, { y: 0.552, z: ZR - 0.052 }]) {
    ...
    for (const s of [-1, 1]) {
      cal.add(seg([s * 0.011, -0.006, 0], [s * 0.028, -0.118, 0.010], 0.0052, M.steel, 'caliper-arm'));
