...
import {
  grp, mesh, box, cyl, rbox, tor, lathe, tubeOf, inst,
  finish, rand, range, weather, decal, row, d2r,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  row(pad, bays, DX, (i, x) => {
    ...
    t.add(mesh(box(0.010, 0.026, 0.010), M.bolt, { name: 'number-stem', pos: [0, -0.010, 0.024] }));
    return t;
  });
  ...
  for (let i = 0; i < bays; i++) {
    ...
    const h = 0.418 + range(rnd, -0.006, 0.006);
    const hw = 0.148 + range(rnd, -0.004, 0.006);
  ...
  row(WS, bays, DX, (i, x) => {
    ...
    // 斜め面（タイヤを受けめる）
    b.add(mesh(rbox(0.226, 0.030, 0.052, 0.006, 2), M.concrete, { name: 'stop-ramp', pos: [0, ch * 0.94, -0.040], rot: [d2r(30), 0, 0] }));
    // 落とし込み（区画番号の掘り込み風）
    b.add(mesh(box(0.050, 0.004, 0.026), M.signFace, { name: 'stop-mark', pos: [0, ch + 0.0245, -0.020] }));
    // 基礎ピン
    for (const px of [-0.082, 0.082]) {
      b.add(mesh(cyl(0.0085, 0.0085, 0.030, 8), M.bolt, { name: 'stop-pin', pos: [px, 0.036, -0.030] }));
      b.add(mesh(cyl(0.0140, 0.0140, 0.006, 6), M.bolt, { name: 'stop-pin-head', pos: [px, 0.054, -0.030] }));
    }
  ...
  for (const s of [-1, 1]) {
    ...
    for (const sy of [-0.050, 0.050]) bd.add(mesh(cyl(0.0060, 0.0060, 0.020, 8), M.bolt, { name: 'board-bolt', pos: [0, sy, 0.012], rot: [Math.PI / 2, 0, 0] }));
    eg.add(bd);
  ...
  const leafGeo = new THREE.PlaneGeometry(0.062, 0.044);
  ...
  const greenLeaves = inst(leafGeo, M.leafGreen, 12, (i, d, r) => {
    d.position.set((r() - 0.5) * (span + 0.10), 0.0295, -0.30 + r() * 0.90);
  ...
  weather(g, { w: 0.22, h: 0.08, pos: [-span * 0.28, 0.0335, -0.20], rot: [-Math.PI / 2, 0, 0], kind: 'moss', color: PAL.moss, opacity: 0.24, seed: seed + 63, count: 2, spread: 0.006 });
