...
function tubeDecal(parent, { at, axis, side, w = 0.05, h = 0.10, kind = 'chip', color = PAL.rust, opacity = 0.5, seed = 1, offset = 0.0016 }) {
  ...
  const mm = new THREE.Mesh(new THREE.PlaneGeometry(w, h),
    MAT.decal({ map: TEX.wear({ kind, color, seed, density: 1.35 }), opacity, side: THREE.DoubleSide, order: 1 }));
  mm.position.set(at[0], at[1], at[2]).addScaledVector(n, offset);
  mm.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(b, a, n));
...
function wheel(o) {
  ...
  if (rusty < 1) {                                  // 反射板は片側のみ（片方は欠落）
    ...
    rg.add(mesh(box(0.011, 0.009, 0.013), M.rustyMetal, { name: 'reflector-clip', pos: [(rIn + rOut) * 0.58, 0, -0.002] }));
    w.add(rg);
  }
  return w;
}
...
export function build(options = {}) {
  ...
  const M = {
    ...
    rim: MAT.metal('#a49b90', { worn: 0.85, repeat: 2, side: THREE.DoubleSide }),
    rimDark: MAT.metal('#7d7469', { worn: 0.9, side: THREE.DoubleSide }),
    ...
    fender: MAT.hardPlastic(shade(faded, 0.6), { worn: 0.95, repeat: 2, side: THREE.DoubleSide }),
    ...
    reflector: MAT.hardPlastic('#a8443c', { worn: 0.7, side: THREE.DoubleSide }),
  ...
  // 錆・退色・掉漆（いたる所）
  {
    ...
    tubeDecal(F, { at: [-0.0180, 0.640, -0.140], axis: stAx, side: [-1, 0, 0], w: 0.028, h: 0.200, kind: 'rust', color: PAL.rust, opacity: 0.68, seed: seed + 7 });
    tubeDecal(F, { at: [0.0180, 0.560, -0.110], axis: stAx, side: [1, 0, 0], w: 0.026, h: 0.100, kind: 'chip', color: '#a89f92', opacity: 0.6, seed: seed + 9 });
    ...
    tubeDecal(F, { at: [0.048, 0.292, -0.200], axis: csAx, side: [0, 1, 0], w: 0.020, h: 0.200, kind: 'dirt', color: '#5f5344', opacity: 0.6, seed: seed + 15 });
    tubeDecal(F, { at: [-0.048, 0.286, -0.320], axis: csAx, side: [0, -1, 0], w: 0.020, h: 0.180, kind: 'dirt', color: '#5f5344', opacity: 0.55, seed: seed + 17 });
  }
  ...
  H.add(bend([[-0.022, 0.516, 0.326], [-0.040, 0.452, 0.418], [-0.050, 0.386, 0.500], [-0.056, RW + 0.004, ZF - 0.014]], 0.0128, M.frame, 'fork-blade-L', 22));
  ...
  // 旧型の大きなベル（鉄・錆）
  const bell = grp('bell', { pos: [-0.150, 0.856, 0.170], rot: [0, d2r(-14), 0] });
  ...
  bell.add(mesh(cyl(0.0072, 0.0072, 0.016, 8), M.darkMetal, { name: 'bell-post', pos: [0, -0.010, 0] }));
  ...
  sad.add(mesh(rbox(0.112, 0.016, 0.176, 0.022, 3), M.saddle, { name: 'saddle-crown', pos: [0, 0.014, -0.006] }));
  sad.add(mesh(rbox(0.050, 0.020, 0.064, 0.012, 2), M.saddle, { name: 'saddle-nose', pos: [0, -0.002, 0.140] }));
  // 割れ目（黒い切れ目 + ガムテープ 2 枚）
  sad.add(mesh(box(0.004, 0.030, 0.112), M.rubber, { name: 'saddle-crack', pos: [0.018, 0.008, -0.030] }));
  sad.add(mesh(box(0.072, 0.0025, 0.026), M.rubber, { name: 'saddle-crack-2', pos: [-0.030, 0.018, 0.052], rot: [0, d2r(24), 0] }));
  sad.add(mesh(rbox(0.056, 0.004, 0.130, 0.002, 2), M.tape, { name: 'duct-tape-1', pos: [0.018, 0.021, -0.030], rot: [0, 0, d2r(4)] }));
  sad.add(mesh(rbox(0.078, 0.004, 0.048, 0.002, 2), M.tape, { name: 'duct-tape-2', pos: [-0.026, 0.022, 0.056], rot: [0, d2r(24), 0] }));
  for (const s of [-1, 1]) sad.add(bend([[s * 0.028, -0.022, 0.078], [s * 0.030, -0.030, 0], [s * 0.028, -0.028, -0.088]], 0.0040, M.rustyMetal, 'saddle-rail', 12));
  ...
  pathPts.push(p1, p1.clone().lerp(p2, 0.30).sub(new THREE.Vector3(0, sag * 0.72, 0)), p1.clone().lerp(p2, 0.62).sub(new THREE.Vector3(0, sag, 0)), p2);
  ...
  for (let i = 0; i <= 20; i++) {
    ...
    guardShape.push(new THREE.Vector3(gx, BB[1] + Math.sin(a) * 0.098, BB[2] + Math.cos(a) * 0.098));
  }
  CG.add(bend(guardShape, 0.0075, M.guard, 'guard-edge', 26));
  {
    ...
    for (let i = 0; i <= N; i++) {
      ...
      const inner = new THREE.Vector3(gx, BB[1] + (q.y - BB[1]) * 0.30, BB[2] + (q.z - BB[2]) * 0.30);
    ...
    CG.add(mesh(geo, MAT.hardPlastic('#b9b3a6', { worn: 0.95, side: THREE.DoubleSide }), { name: 'guard-plate' }));
  }
  ...
  FD.add(mesh(rbox(0.058, 0.006, 0.052, 0.002, 2), M.rustyMetal, { name: 'fender-split-plate', pos: [-0.004, 0.618, ZF + 0.206], rot: [d2r(-24), 0, d2r(-4)] }));
  for (const s of [-1, 1]) {
  ...
  lamp.add(mesh(cyl(0.0098, 0.0098, 0.024, 8), M.rustyMetal, { name: 'lamp-stem', pos: [0, -0.044, -0.008] }));
  lamp.add(bend([[0, -0.052, 0.004], [-0.030, -0.066, 0.050], [-0.046, -0.048, 0.108]], 0.0030, M.wire, 'lamp-wire', 10));
  L.add(lamp);
  // 尾灯は欠落、ステーとネジだけ残る
  ...
  tl.add(mesh(tor(0.0075, 0.0016, 4, 10), M.rubber, { name: 'tail-rubber-band', pos: [0, -0.010, 0.006], rot: [d2r(70), 0, 0] }));
  L.add(tl);
  ...
  bike.add(RC);
  const rackY = 0.700, rackZ0 = -0.250, rackZ1 = -0.800;
  for (const s of [-1, 1]) {
    ...
    RC.add(bend([[s * 0.074, rackY - 0.014, rackZ1 + 0.090], [s * 0.060, 0.596, -0.470], [s * 0.056, RW + 0.022, ZR - 0.036]], 0.0056, M.rustyMetal, 'rack-stay-rear', 12));
  ...
  const ST = grp('stand');
  bike.add(ST);
  const px = -0.050, py = 0.248, pz = -0.046;
  ...
  tubeDecal(ST, { at: [-0.132, 0.140, -0.006], axis: [0, -0.104, 0.076], side: [-1, 0, 0], w: 0.024, h: 0.150, kind: 'rust', color: PAL.rust, opacity: 0.8, seed: seed + 59 });
  ...
  for (const c of [{ y: 0.512, z: ZF + 0.268 }, { y: 0.548, z: ZR - 0.048 }]) {
    ...
    for (const s of [-1, 1]) {
      cal.add(seg([s * 0.011, -0.006, 0], [s * 0.028, -0.116, 0.010], 0.0052, M.steel, 'caliper-arm'));
  ...
  // 後ブレーキはロッド（シートステー沿い）
  BR.add(bend([[0, 0.836, 0.214], [-0.012, 0.792, -0.150], [-0.014, 0.700, -0.240], [-0.012, 0.600, -0.420], [0, 0.548, ZR - 0.046]], 0.0042, M.steel, 'rear-rod', 22));
  BR.add(mesh(cyl(0.0072, 0.0072, 0.020, 8), M.rustyMetal, { name: 'rear-rod-crank', pos: [-0.014, 0.700, -0.240], rot: [0, 0, Math.PI / 2] }));
  ...
  tubeDecal(bike, { at: [-0.0180, RW + 0.348 * Math.sin(2.80), ZR + 0.348 * Math.cos(2.80)], axis: [1, 0, 0], side: [0, Math.sin(2.80), Math.cos(2.80)], w: 0.100, h: 0.028, kind: 'moss', color: '#6d8a52', opacity: 0.4, seed: seed + 63 });
