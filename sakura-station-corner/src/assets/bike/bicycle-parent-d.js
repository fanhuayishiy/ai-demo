...
function wheel(o) {
  ...
  rg.add(mesh(box(0.012, 0.010, 0.014), M.darkMetal, { name: 'reflector-clip', pos: [(rIn + rOut) * 0.6, 0, -0.002] }));
  w.add(rg);
  return w;
}
...
export function build(options = {}) {
  ...
  const M = {
    ...
    rim: MAT.metal('#cfd3d6', { worn: 0.42, repeat: 3, side: THREE.DoubleSide }),
    rimDark: MAT.metal('#93999f', { worn: 0.5, side: THREE.DoubleSide }),
    ...
    fender: MAT.hardPlastic(shade(frame, 0.5), { worn: 0.66, repeat: 3, side: THREE.DoubleSide }),
    guard: MAT.hardPlastic(cream, { worn: 0.55 }),
    guardSide: MAT.hardPlastic(cream, { worn: 0.55, side: THREE.DoubleSide }),
    ...
    reflector: MAT.hardPlastic('#c9412f', { worn: 0.35, side: THREE.DoubleSide }),
  ...
  for (const s of [-1, 1]) {
    ...
    for (const [gy, gz] of [[0.026, -0.030], [-0.014, 0.010]]) {
  ...
  F.add(ring([0, 0.304, -0.020], [0, -0.006, -0.080], 0.0258, 0.0030, M.frameDark, 'weld-step'));
  F.add(ring([0, HTT[1] - 0.028, HTT[2] - 0.002], [0, HTT[1] - HTB[1], HTT[2] - HTB[2]], 0.0278, 0.0032, M.frameDark, 'weld-head'));
  {
    const dtAx = [0, 0.160, -0.340];
    ...
    tubeDecal(F, { at: [0.0540, 0.286, -0.200], axis: csAx, side: [0, 1, 0], w: 0.024, h: 0.170, kind: 'dirt', color: '#6d5f4b', opacity: 0.5, seed: seed + 9 });
  }
  ...
  for (const s of [-1, 1]) {
    ...
    lev.add(bend([[0.012, 0, -0.102], [0.014, 0, -0.130], [0.020, 0.004, -0.144]], 0.0058, M.chrome, 'lever-tip', 6));
    HB.add(lev);
    // 親ブレーキ（子ども側レバーも兼ねる大型レバー）
    lev.add(mesh(rbox(0.010, 0.008, 0.046, 0.003, 2), M.red, { name: 'lever-grip', pos: [0.012, 0.010, -0.060] }));
  }
  // 大型バッテリーライト（バー中央・角型大）
  ...
  lamp.add(mesh(rbox(0.030, 0.024, 0.026, 0.005, 2), M.darkMetal, { name: 'lamp-strap', pos: [0, 0.026, -0.040] }));
  lamp.add(bolt([0.020, 0.026, -0.040], 0.0058, M.chrome, 'z', 'lamp-bolt'));
  lamp.add(bend([[0, 0.0, -0.052], [-0.030, -0.040, -0.090], [-0.034, -0.060, -0.150]], 0.0030, M.wire, 'lamp-wire', 10));
  HB.add(lamp);
  // ベル（大きな親指ベル）
  ...
  bell.add(mesh(cyl(0.0080, 0.0080, 0.014, 8), M.darkMetal, { name: 'bell-post', pos: [0, -0.009, 0] }));
  ...
  sad.add(mesh(rbox(0.128, 0.020, 0.202, 0.026, 3), M.saddle, { name: 'saddle-crown', pos: [0, 0.018, -0.008] }));
  sad.add(mesh(rbox(0.056, 0.024, 0.072, 0.014, 2), M.saddle, { name: 'saddle-nose', pos: [0, -0.002, 0.154] }));
  ...
  shell.add(mesh(rbox(0.170, 0.140, 0.020, 0.010, 2), M.seatPad, { name: 'cs-back-pad', pos: [0, 0.110, -0.080], rot: [d2r(9), 0, 0] }));
  ...
  bars.add(bend([[-0.108, seatY + 0.022, seatZ + 0.110], [-0.050, seatY + 0.030, seatZ + 0.132], [0.050, seatY + 0.030, seatZ + 0.132], [0.108, seatY + 0.022, seatZ + 0.110]], 0.0085, M.resin, 'cs-bar-front', 16));
  ...
  fr.add(mesh(cyl(0.0090, 0.0090, 0.090, 8), M.steel, { name: 'fr-arm', pos: [0, 0.060, -0.020], rot: [0, 0, Math.PI / 2] }));
  ...
  fr.add(bolt([0.048, 0.060, -0.020], 0.0068, M.darkMetal, 'x', 'fr-pivot'));
  CS.add(fr);
  // 日除け（ワイヤー骨 + 布・折りシワ）
  ...
  const cp = grp('canopy-cloth');
  {
    ...
    cp.add(mesh(geo, MAT.fabric({ color: '#d9603f', repeat: 12, side: THREE.DoubleSide }), { name: 'canopy-surface' }));
  }
  for (let k = 0; k < 4; k++) {
  ...
  {
    const plate = [
      new THREE.Vector3(gx, BB[1] + ringR * Math.sin(aTop) + 0.017, BB[2] + ringR * Math.cos(aTop)),
      new THREE.Vector3(gx, 0.406, -0.210),
      new THREE.Vector3(gx, 0.400, -0.420),
      new THREE.Vector3(gx, RW + cogR + 0.018, ZR + 0.020),
      new THREE.Vector3(gx, RW + cogR + 0.014, ZR - 0.062),
    ];
  ...
  tubeDecal(CG, { at: [gx + 0.003, 0.398, -0.430], axis: [0, -0.006, -0.10], side: [1, 0, 0], w: 0.040, h: 0.120, kind: 'chip', color: '#9c937e', opacity: 0.5, seed: seed + 21 });
  ...
  bike.add(RC);
  const rackY = 0.724, rackZ0 = -0.280, rackZ1 = -0.900;
  for (const s of [-1, 1]) {
    ...
    RC.add(bend([[s * 0.084, rackY - 0.012, rackZ1 + 0.110], [s * 0.068, 0.624, -0.550], [s * 0.062, RW + 0.030, ZR - 0.042]], 0.0068, M.steel, 'rack-stay-rear', 12));
  ...
  if (basket) {
    ...
    lid.add(mesh(rbox(bw - 0.060, 0.006, bl - 0.070, 0.003, 2), M.basketCloth, { name: 'lid-cloth', pos: [0, -0.008, 0] }));
    ...
    st.add(mesh(rbox(0.180, 0.130, 0.200, 0.022, 3), M.bag, { name: 'diaper-bag', pos: [-0.040, yBase + 0.080, zMid + 0.050], rot: [0, d2r(8), 0] }));
    ...
    hel.add(mesh(tor(0.0590, 0.0060, 5, 16), M.helmet, { name: 'helmet-rim', pos: [0, -0.0220, 0], rot: [Math.PI / 2, 0, 0] }));
    ...
    hel.add(mesh(tubeOf(catenary([-0.040, -0.026, 0.030], [-0.052, -0.100, 0.010], 0.020, 8), 0.0022, 10, 5), M.harness, { name: 'helmet-strap-1' }));
    hel.add(mesh(tubeOf(catenary([0.040, -0.026, 0.030], [0.052, -0.100, 0.010], 0.020, 8), 0.0022, 10, 5), M.harness, { name: 'helmet-strap-2' }));
  ...
  rref.add(mesh(rbox(0.048, 0.030, 0.0025, 0.004, 2), M.redGlow, { name: 'rear-reflector-face', pos: [0, 0, -0.003] }));
  L.add(rref);
  ...
  for (const c of [{ y: 0.560, z: ZF + 0.296 }, { y: 0.566, z: ZR - 0.056 }]) {
    ...
    for (const s of [-1, 1]) {
      cal.add(seg([s * 0.013, -0.007, 0], [s * 0.032, -0.130, 0.011], 0.0060, M.steel, 'caliper-arm'));
      cal.add(mesh(rbox(0.015, 0.022, 0.008, 0.003, 2), M.rubberCap, { name: 'brake-pad', pos: [s * 0.034, -0.138, 0.015] }));
      cal.add(bolt([s * 0.032, -0.130, 0.022], 0.0052, M.darkMetal, 'z', 'pad-pin'));
    }
