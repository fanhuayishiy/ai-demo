...
import { grp, mesh, box, cyl, sph, capsule, rand, range, decal } from '../../core/kit.js';
...
function ricePoint(X, Y, Z, sh) {
  ...
  if (y < FLAT) y = FLAT;
  return new THREE.Vector3(x * sh.sx, y * sh.sy + sh.yShift, Z * sh.sz);
}
function riceNormal(p, sh) {
  return new THREE.Vector3(p.x / (sh.sx * sh.sx), (p.y - sh.yShift) / (sh.sy * sh.sy), p.z / (sh.sz * sh.sz)).normalize();
}
/** 米粒の凹凸（決定的ノイズ） */
...
export function build(options = {}) {
  ...
  /* ---- 米粒本体 ---- */
  const riceGrp = grp('rice-body', { pos: [0, TRAY_H - 0.0016, -0.0008], rot: [sp.tilt * 0.55, 0.03, 0.012] });
  g.add(riceGrp);
  ...
  function scatter(count, mat, scale, eRange, aRange) {
    for (let i = 0; i < count; i++) {
      ...
      gm.position.copy(p);
      gm.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), n);
  ...
  if (sp.topo === 'dollop') {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      dollop.add(mesh(sph(s, 10, 8), dMat, { pos: [range(rnd, -0.0024, 0.0024), i * 0.0024, range(rnd, -0.0022, 0.0022)], cast: false }));
    }
    dollop.add(mesh(cyl(0.0014, 0.0020, 0.0032, 8), dMat, { pos: [0.0030, 0.0092, -0.0016], rot: [0.2, 0, -0.5], cast: false }));
  }
  if (sp.topo === 'bonito') {
    const bMat = MAT.food({ color: '#7a4f33', spec: 0.32, side: THREE.DoubleSide, shadowAmt: 0.8 });
    for (let i = 0; i < 9; i++) {
      ...
      fl.position.set(Math.cos(a) * rr, sh.yTop - 0.0012 + range(rnd, -0.0012, 0.0018), Math.sin(a) * rr * 0.8);
  ...
  const noriMat = MAT.nori({ side: THREE.DoubleSide, spec: 0.36, specPower: 56, steps: 3 });
  ...
  /* ---- ラップフィルム ---- */
  if (sp.film) {
    const filmMat = MAT.glassLite({ color: '#f4fafa', opacity: 0.28, spec: 1.0, specPower: 250, specCut: 0.05, sheen: 0.52, rim: 0.55, side: THREE.DoubleSide, steps: 2 });
    ...
    // フィルムのシール筋＋天面のつば
    const seam = mesh(box(0.030, 0.0007, 0.0044), MAT.glassLite({ color: '#ffffff', opacity: 0.5, side: THREE.DoubleSide }), {
      name: 'film-seam', pos: [0, sh.yTop + 0.0012, 0], cast: false, receive: false,
    });
    seam.rotation.z = 0.05;
    riceGrp.add(seam);
    if (sp.defect === 'loose') {
      riceGrp.add(mesh(sheetGeo(10, 6, (u, v) => new THREE.Vector3(
  ...
  if (sp.defect === 'torn') {
    const scrap = mesh(box(0.0115, 0.0005, 0.0074), MAT.nori({ side: THREE.DoubleSide }), { cast: false, receive: false });
    scrap.position.set(range(rnd, -0.020, 0.020), TRAY_H - 0.0040, range(rnd, -0.013, 0.006));
