...
import { grp, mesh, box, rbox, cyl, sph, lathe, tor, circ, plane, extrude, shape, tubeOf, rand, range, weather, decal } from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, memo, shade, rr } from '../../core/textures.js';
...
function mats(o) {
  ...
  return {
    card: (v) => MAT.poster({
      map: cardTex(v), color: '#ffffff', steps: 2, spec: 0.1, shadowAmt: 0.75, side: THREE.DoubleSide,
      uv: { repeat: [1 / SPEC[v].w, 1 / SPEC[v].h], offset: [0.5, 0.5] }, ...t,
    }),
    cardBack: MAT.paper({ color: '#e8e0cc', spec: 0.06, shadowAmt: 0.85, ...t }),
    blister: MAT.glassLite({ color: '#eef6f8', opacity: 0.22, spec: 1, specPower: 230, side: THREE.DoubleSide, depthWrite: false }),
    bag: MAT.glassLite({ color: '#e9f1f2', opacity: 0.3, map: bagTex('#fff'), side: THREE.DoubleSide, depthWrite: false }),
...
function vBattery(P, M, rnd, o) {
  ...
  const card = cardPlate(P, { w: S.w, h: S.h, t: 0.0016, hole: 0.0058, headY: S.h * 0.415, mat: M.card('battery') });
  // 穴の補強リング（打ち抜きの白ボロが見える）
  P.add(noHull(mesh(new THREE.TorusGeometry(0.0062, 0.0008, 6, 18), M.cardBack, { pos: [0, S.h * 0.415 - S.h / 2, 0.001], cast: false })));
  const cellH = 0.0505, cellR = 0.0053;
  for (let i = 0; i < 4; i++) {
    const c = grp(`cell-${i}`); P.add(c);
    const x = (i - 1.5) * (cellR * 2 + 0.0022);
    c.position.set(x, -S.h * 0.085, 0.0075);
  ...
  // ブリスター（4 本を覆う泡）
  blister(P, { w: 0.044, h: 0.066, d: 0.0165, mat: M.blister, z: 0.0016, y: -S.h * 0.085 });
  ...
  decal(P, { map: TEX.wear({ kind: 'chip', color: '#efe7d2', seed: (o.seed ?? 1) + 3, density: 1.6 }), w: 0.02, h: 0.016, pos: [-0.011, -S.h * 0.3, S.w * 0.03 + 0.0026], opacity: 0.8 });
  weather(P, { w: 0.014, h: 0.02, pos: [S.w * 0.28, -S.h * 0.42, 0.003], kind: 'dirt', color: '#8b7f66', opacity: 0.35, seed: (o.seed ?? 1) + 5, spread: 0.001 });
}
...
function vUmbrella(P, M, rnd, o) {
  ...
  // 袋（透明パイプ＋口）
  const bagH = S.h * 0.58, bagW = S.w * 0.76, bagY = -S.h * 0.145;
  ...
  u.add(mesh(lathe([[0, 0], [uR * 0.55, 0.004], [uR, 0.014], [uR * 1.02, uH * 0.62], [uR * 0.84, uH - 0.01], [uR * 0.4, uH], [0, uH]], 14),
    MAT.fabric({ color: '#39465e', map: terryTex('#39465e'), repeat: 6, tint: o.tint }), { pos: [-0.0015, bunA, 0.0105], name: 'cloth-bun' }));
  // 束の仕切りテープ（マジック）2 箇所
  for (const f of [0.24, 0.74]) {
    u.add(noHull(mesh(tor(uR * 1.08, 0.0021, 5, 16), MAT.rubber('#2b3040'), { pos: [-0.0015, bunA + uH * f, 0.0105], rot: [-90 * D2R, 0, 0], cast: false })));
  }
  // 柄（グリップ）・芯金・先端金具
  u.add(mesh(cyl(0.0064, 0.006, 0.026, 12), MAT.rubber('#2c3038'), { pos: [-0.0015, uTop - 0.013, 0.0105] }));
  u.add(mesh(cyl(0.004, 0.0036, 0.017, 10), MAT.metal('#b7bbbe'), { pos: [-0.0015, bunA - 0.0085, 0.0105] }));
  u.add(noHull(mesh(sph(0.0038, 8, 6), MAT.metal('#b7bbbe'), { pos: [-0.0015, bunA - 0.0165, 0.0105], cast: false })));
  // 袋の印刷（商品名タテ）
...
function vTowel(P, M, rnd, o) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    P.add(noHull(mesh(rbox(tw * (1 - t * 0.035), 0.0088, 0.0132 + t * 0.0022, 0.0022, 2), i % 2 ? cloth : clothLt, {
      pos: [(t - 0.5) * 0.0035, stackY + (i - 2) * 0.0088, 0.0108 + (i % 2 ? 0.0007 : -0.0007)],
  ...
  weather(P, { w: 0.018, h: 0.022, pos: [-S.w * 0.2, stackY - 0.016, 0.0242], kind: 'dirt', color: '#8b7f66', opacity: 0.24, seed: (o.seed ?? 1) + 11, spread: 0.001 });
}
...
function vPen(P, M, rnd, o) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const x = (i - 0.5) * 0.017;
    pen.position.set(x, -S.h * 0.1, 0.009);
  ...
  blister(P, { w: 0.04, h: 0.122, d: 0.016, mat: M.blister, z: 0.0014, y: -S.h * 0.1 });
  // 缺陷：ブリスターの角切れ・ペン先のかすかな曲がり
  P.add(noHull(mesh(box(0.008, 0.0022, 0.003), MAT.paint('#efe9d8', { steps: 2 }), { pos: [0.014, -S.h * 0.02, 0.014], rot: [0, 0, 0.3], cast: false })));
  weather(P, { w: 0.014, h: 0.018, pos: [-S.w * 0.24, -S.h * 0.42, 0.003], kind: 'dirt', color: '#7f7a63', opacity: 0.32, seed: (o.seed ?? 1) + 13, spread: 0.001 });
}
const coneTiny = (r, h) => new THREE.ConeGeometry(r, h, 10);
...
function vNotebook(P, M, rnd, o) {
  ...
  B.position.set(0, -S.h * 0.1, 0.009);
  B.rotation.z = -1.2 * D2R;
  // 本文（糊とじ＝背が丸い）
  const pages = new THREE.BoxGeometry(w, h, d);
  ...
  B.add(noHull(mesh(plane(w * 0.9, h * 0.9), MAT.poster({ map: TEX.poster({ title: '方眼 80 枚', sub: '春のしおりのしおり', bg: '#f6efd8', accent: '#c9a227', seed: 5 }), spec: 0.14, side: THREE.DoubleSide }), { pos: [0.0006, 0, d / 2 + 0.0044], cast: false })));
  // 背の糊とじ（‑X 側へ膨らむ半円）＋ホチキス 2 箇所
  B.add(noHull(mesh(new THREE.CylinderGeometry(d / 2 + 0.0012, d / 2 + 0.0012, h * 0.985, 12, 1, false, Math.PI, Math.PI),
  ...
  blister(P, { w: S.w * 0.82, h: S.h * 0.63, d: 0.0225, mat: M.blister, z: 0.0014, y: -S.h * 0.1 });
  // 缺陷：表紙の角の丸み・ゴムが劣化で伸びた跡
  weather(P, { w: 0.016, h: 0.016, pos: [w * 0.28, -S.h * 0.02, 0.0246], kind: 'chip', color: '#e6dcc0', opacity: 0.42, seed: (o.seed ?? 1) + 17, spread: 0.0012 });
}
...
function vRazor(P, M, rnd, o) {
  ...
  hd.position.set(-S.w * 0.28, Y0, 0.01);
  hd.rotation.z = 2 * D2R;
  const hL = 0.104;
  ...
  for (let i = 0; i < 5; i++) {
    head.add(noHull(mesh(box(0.0132, 0.00035, 0.0009), M.metal, { pos: [0, -0.0022 + i * 0.0011, 0.0044], cast: false })));
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let k = 0; k < 5; k++) bl.add(noHull(mesh(box(0.0148, 0.0004, 0.0008), M.metal, { pos: [0, -0.0034 + k * 0.0017, 0.0037], cast: false })));
  ...
  weather(P, { w: 0.018, h: 0.02, pos: [-S.w * 0.24, S.h * 0.3, 0.0032], kind: 'chip', color: '#cfc8b6', opacity: 0.42, seed: (o.seed ?? 1) + 19, spread: 0.0012 });
}
...
function vWiper(P, M, rnd, o) {
  ...
  // パウチ本体（ソフト＋中央が膨らむ）
  const g = new THREE.BoxGeometry(w, h, 0.012, 8, 10, 1);
...
export const VARIANT_LIST = Object.keys(VARIANTS);
