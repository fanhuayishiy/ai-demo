...
import { grp, mesh, box, rbox, cyl, sph, capsule, rand, range, weather, decal } from '../../core/kit.js';
...
function buildTray(stage, M, rnd, seed) {
  ...
  const walls = [
    ...
    ['wall-back', 0, FL * 0.4, -(TD / 2 - WT / 2), [-draft, 0, 0], TW, WT],
    ['wall-right', TW / 2 - WT / 2, FL * 0.4, 0, [0, 0, -draft], WT, TD - WT * 2],
    ['wall-left', -(TW / 2 - WT / 2), FL * 0.4, 0, [0, 0, draft], WT, TD - WT * 2],
  ];
  ...
  // 吸水垫（ドリンカー）
  const pad = mesh(rbx(TW - 0.018, 0.0008, TD - 0.020, 0.0004), M.pad, { name: 'absorb-pad', pos: [range(rnd, -0.002, 0.002), FL + 0.0016, range(rnd, -0.001, 0.002)] });
  ...
  weather(stage, { w: 0.040, h: 0.009, pos: [-0.052, 0.012, -(TD / 2 + 0.0014)], rot: [0, PI, 0], kind: 'dirt', color: '#cfc9b6', opacity: 0.30, seed: seed + 12, density: 1.2, spread: 0.002 });
}
...
function shariLump(parent, M, rnd, len, wid, hgt, x, z, ry) {
  ...
  parent.add(p);
  p.add(mesh(rbx(len, hgt, wid, Math.min(0.0062, hgt * 0.5), 2), M.rice, { name: 'shari-body', pos: [0, FL + hgt / 2 + 0.0016, 0] }));
...
function netaPiece(parent, M, rnd, kind, len, wid, top, opts = {}) {
  ...
  const neta = mesh(rbx(len, th, wid, 0.0017, 2), body, { name: 'neta-' + kind, pos: [0, top + th / 2, 0], rot: [0, 0, range(rnd, -0.04, 0.04)] });
  parent.add(neta);
  // 脂の筋・照り
  ...
  for (let i = 0; i < nl; i++) {
    ...
    parent.add(mesh(box(len * 0.86, 0.0007, 0.0016 + (i % 2) * 0.0006), line, {
      name: 'neta-line' + i, pos: [range(rnd, -0.002, 0.002), top + th * (0.42 + 0.5 * t), -wid / 2 + wid * t + range(rnd, -0.001, 0.001)],
  ...
  if (kind === 'tamago') {  // 海苔の帯
    parent.add(mesh(rbx(0.0075, th + 0.0115, wid + 0.0035, 0.0009), M.nori, { name: 'nori-band', pos: [range(rnd, -0.003, 0.003), top - 0.0022, 0] }));
  }
  if (kind === 'ebi') {     // 海老の尾と節
    for (let i = 0; i < 3; i++) {
...
function soyDish(parent, M, x, z, rnd, seed) {
  ...
  d.add(mesh(cyl(0.0052, 0.0052, 0.0004, 12), M.soyHi, { name: 'soy-reflection', pos: [-0.004, 0.00255, 0.003], cast: false }));
  // 縁の醤油垂れ
...
function ginger(parent, M, x, y, z, rnd) {
  ...
  for (let i = 0; i < 5; i++) {
    gr.add(mesh(rbx(0.0128, 0.0012, 0.0088, 0.0006), i % 2 ? M.ginger : M.gingerPale, {
      name: 'ginger-ruff' + i, pos: [range(rnd, -0.0025, 0.0025), i * 0.0016 + 0.0008, range(rnd, -0.0022, 0.0022)],
...
function daikon(parent, M, x, y, z, rnd) {
  ...
  for (let i = 0; i < 12; i++) {
    d.add(mesh(box(0.030 + rnd() * 0.010, 0.0006, 0.0008), M.daikon, {
      name: 'daikon-shred' + i, pos: [range(rnd, -0.008, 0.008), 0.0030 + i * 0.00042 + range(rnd, -0.001, 0.001), range(rnd, -0.010, 0.010)],
...
function maki(parent, M, rnd, x, z, fill, h) {
  ...
  // ゴマ
  for (let i = 0; i < 5; i++) {
    const s = mesh(sph(0.0011, 6, 5), i % 2 ? M.sesame : M.sesameDark, { name: 'sesame' + i, pos: [range(rnd, -0.008, 0.008), top + 0.0004, range(rnd, -0.008, 0.008)], cast: false });
    noHull(s); p.add(s);
  }
  return p;
}
...
export function build(options = {}) {
  ...
  const M = {
    ...
    leaf: MAT.food({ color: PAL.awningGreen, spec: 0.34, side: THREE.DoubleSide, shadowAmt: 0.74 }),
  ...
  if (sp.layout === 'nigiri') {
    ...
    const rows = [0.0385, -0.0045];
    let k = 0;
    for (let r = 0; r < 2; r++) {
      for (let i = 0; i < 3; i++) {
        const kind = kinds[k++ % kinds.length];
        const x = -0.058 + i * 0.058 + range(rnd, -0.0025, 0.0025);
        const z = rows[r] + range(rnd, -0.0018, 0.0018);
        const len = 0.0465 + range(rnd, -0.0018, 0.0026), wid = 0.0232 + range(rnd, -0.0012, 0.0016);
        const hgt = 0.0128 + range(rnd, -0.0012, 0.0018);
    ...
    soyDish(stage, M, -0.0620, -0.0395, rnd, seed);
    wasabi(stage, M, 0.0590, FL + 0.0075, -0.0400, 0.0082, rnd);
    ginger(stage, M, -0.0205, FL + 0.0024, -0.0410, rnd);
    ...
    let n = 0;
    for (const [x, z] of [[-0.040, 0.0300], [0.0000, 0.0325], [-0.0205, -0.0115], [0.0205, -0.0130]]) {
      maki(stage, M, rnd, x + range(rnd, -0.002, 0.002), z + range(rnd, -0.002, 0.002), fills[n], 0.0228 + range(rnd, -0.0012, 0.0016));
      n++;
    }
    // 巻き end に零れた飯粒・acap
    ...
    ginger(stage, M, -0.0640, FL + 0.0024, 0.0280, rnd);
  } else if (sp.layout === 'chirashi') {
    // シャリを全面に敷く
    const bed = mesh(rbx(TW - 0.014, 0.0122, TD - 0.016, 0.0050, 2), M.rice, { name: 'shari-bed', pos: [0, FL + 0.0078, 0] });
    stage.add(bed);
    ...
    // 具の散らし
    const cubes = [
      ['#e9874f', 0.0135, -0.050, 0.026], ['#a9333b', 0.0125, 0.010, 0.030], ['#e9874f', 0.0130, 0.048, -0.008],
      ['#a9333b', 0.0122, -0.018, -0.030], ['#efc65e', 0.0140, 0.030, 0.034], ['#5f8f45', 0.0110, -0.060, -0.012],
    ];
    cubes.forEach((c, i) => {
      stage.add(mesh(rbx(c[1], c[1] * 0.62, c[1] * 0.78, 0.0026, 2), MAT.food({ color: tint ?? c[0], spec: 0.5, specPower: 34, shadowAmt: 0.62 }), {
    ...
    // 海老・きぬさや・かまぼこ
    netaPiece(shariLump(stage, M, rnd, 0.0420, 0.0210, 0.0072, -0.0420, -0.0300, -14), M, rnd, 'ebi', 0.0440, 0.0230, FL + 0.0110, {});
    for (let i = 0; i < 3; i++) {
      stage.add(mesh(rbx(0.0200, 0.0016, 0.0072, 0.0008), MAT.food({ color: '#7fa63f', spec: 0.4 }), {
        name: 'pea' + i, pos: [-0.004 + i * 0.014, FL + 0.0196 + i * 0.0006, -0.0400 + range(rnd, -0.003, 0.003)], rot: [0, range(rnd, -0.6, 0.6), 0], cast: false,
      }));
    }
    wasabi(stage, M, -0.0720, FL + 0.0136, 0.0400, 0.0070, rnd);
  } else {
    /* sashimi5：扇状に five 枚・大根と紫蘇 */
    stage.add(mesh(leafGeo(0.058, 0.036, 0.0042), M.leaf, { name: 'shiso-bed', pos: [0.020, FL + 0.0042, 0.006], rot: [0, -0.30, 0] }));
    const order = ['maguro', 'sake', 'katsu', 'tako', 'shiromi'];
    order.forEach((kind, i) => {
      const t = i / (order.length - 1);
      const x = -0.0640 + t * 0.1220;
      const z = 0.0280 - t * 0.0520;
      const p = grp('sashimi-' + kind, { pos: [x, 0, z], rotY: -34 + t * 26 + range(rnd, -4, 4) });
      stage.add(p);
      const len = 0.0520 + range(rnd, -0.002, 0.003), wid = 0.0234 + range(rnd, -0.0012, 0.0018);
    ...
    daikon(stage, M, 0.0560, FL + 0.0026, -0.0360, rnd);
    wasabi(stage, M, -0.0700, FL + 0.0066, -0.0350, 0.0078, rnd);
    soyDish(stage, M, -0.0600, 0.0390, rnd, seed);
    // 白髪ねぎ
    for (let i = 0; i < 6; i++) {
  ...
  decal(stage, { map: TEX.wear({ kind: 'scratch', color: '#ffffff', seed: seed + 41, density: 1.4 }), w: 0.040, h: 0.024, pos: [-0.020, CREST + 0.0004, -0.006], rot: [PI / 2, 0, 0.3], opacity: 0.22, order: 6 });
  ...
  decal(stage, {
    ...
    w: 0.0440, h: 0.0140, pos: [-0.0280, 0.0135, FD2 - 0.0014], opacity: 0.99, order: 2,
  });
  // 値札の掠れ（生活痕）
  weather(stage, { w: 0.018, h: 0.008, pos: [-0.0200, 0.0150, FD2 - 0.0010], kind: 'dirt', color: '#fff6e2', opacity: 0.42, seed: seed + 43, density: 1.1, spread: 0.0015 });
  ...
  if (sp.defect === 'peel') {
    ...
    weather(stage, { w: 0.028, h: 0.012, pos: [0.0560, RIM - 0.0040, -0.0440], kind: 'chip', color: '#dfe9ec', opacity: 0.40, seed: seed + 45, density: 1.2, spread: 0.0018 });
  }
  if (sp.defect === 'smudge') {
    // シャリベッドのへり欠け＋指紋
    decal(stage, { map: TEX.wear({ kind: 'dirt', color: '#e8e2cf', seed: seed + 46, density: 1.3 }), w: 0.034, h: 0.016, pos: [0.030, FL + 0.0150, -0.0440], rot: [-PI / 2 + 0.36, 0, 0], opacity: 0.5 });
    weather(stage, { w: 0.044, h: 0.011, pos: [-0.040, 0.0160, TD / 2 + 0.0016], kind: 'dirt', color: '#b9b3a1', opacity: 0.34, seed: seed + 47, density: 1.4, spread: 0.002 });
  }
  if (sp.defect === 'drip') {
    // 醤油の垂れた跡と刺身の汁
    weather(stage, { w: 0.016, h: 0.012, pos: [-0.0560, 0.0205, TD / 2 + 0.0016], kind: 'dirt', color: '#5d3f22', opacity: 0.34, seed: seed + 48, density: 1.5, spread: 0.002 });
  ...
  // トレイ底の打痕（全 variant）
  weather(stage, { w: 0.028, h: 0.005, pos: [-0.0560, 0.0090, TD / 2 + 0.0014], kind: 'chip', color: '#6a6f76', opacity: 0.30, seed: seed + 49, density: 1.0, spread: 0.001 });
