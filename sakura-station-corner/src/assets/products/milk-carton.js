...
import { grp, mesh, box, cyl, tor, circ, shape, rand, range, decal } from '../../core/kit.js';
...
function bodyGeo(sp) {
  ...
  const s = shape((k) => { k.setFromPoints(pts.map(([x, y]) => new THREE.Vector2(x, y))); k.closePath(); });
...
export function build(options = {}) {
  ...
  // 前面
  decal(g, { map: poster, w: 0.0620, h: 0.0930, pos: [-0.0020, sp.bodyH * 0.575, zf] });
  ...
  for (const sx of [-1, 1]) {
    ...
    decal(g, { map: sideTex, w: 0.0320, h: 0.0480, pos: [sx * xf, sp.bodyH * 0.275, -0.001], rot: [0, sx * Math.PI / 2, 0], order: 2 });
  }
  // 賞味期限印字
  decal(g, { map: TEX.lightPanel({ text: sp.date, bg: '#efe9db', fg: '#3b3a36', mode: 'sign' }), w: 0.0260, h: 0.0130, pos: [-0.0140, 0.0150, zf], order: 3 });
