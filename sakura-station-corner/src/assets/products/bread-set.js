...
function mats(o) {
  ...
  return {
    ...
    lettuce: MAT.food({ color: '#7fae5a', spec: 0.6, specPower: 44, sheen: 0.12, side: THREE.DoubleSide, ...t }),
    ...
    bag: MAT.glassLite({ color: '#f2f6f7', opacity: 0.26, spec: 1, specPower: 240, side: THREE.DoubleSide, depthWrite: false }),
    bagPrintMat: (title, sub, ink) => MAT.glassLite({ color: '#ffffff', opacity: 0.95, map: bagPrint(title, sub, ink), side: THREE.DoubleSide, depthWrite: false }),
...
function breadBag(parent, M, { w, h, d, seed, print, twistAt = 1 }) {
  ...
  const g = new THREE.BoxGeometry(w, h, d, 10, 8, 8);
  const p = g.attributes.position; const v = new THREE.Vector3(); const dv = new THREE.Vector3();
  const rnd = rand(seed);
  for (let i = 0; i < p.count; i++) {
    v.fromBufferAttribute(p, i);
  ...
  G.add(noHull(mesh(tubeOf([[gEnd - 0.0035, h * 0.7, 0], [gEnd + 0.0006, h * 0.74, 0.0022], [gEnd + 0.0016, h * 0.66, -0.0018]], 0.0006, 8, 4), M.tie('#c9403a'), { cast: false })));
  // 印刷テープ（袋の接合部＝正面に出る）
...
function cutFace(parent, M, { w, h, pos, rot = [0, 0, 0], crust = '#c98f52', crumb = '#f6e9cf' }) {
  ...
  G.add(noHull(mesh(rbox(w * 1.02, h * 1.02, 0.0012, Math.min(w, h) * 0.16, 2), M.crust(crust), { pos: [0, 0, -0.0012], cast: false, receive: false })));
...
const PACKPOS = {
  ...
  3: { xy: [[-0.031, 0], [0, 0.016], [0.031, -0.004]], r: 0.0185 },
...
function vSandwich(P, M, rnd, o) {
  ...
  // 上のパン／下のパン
  S.add(mesh(tri(false), M.crumbSoft('#f3e6c8'), { pos: [-0.001, 0.0285, 0.001], name: 'bread-top' }));
  S.add(mesh(tri(true), M.crumbSoft('#f3e6c8'), { pos: [0.001, 0.0085, -0.001], name: 'bread-bottom' }));
  // 耳（三角の 3 辺に沿って周回）
  ...
  const leaf = new THREE.PlaneGeometry(bw * 0.98, bd * 0.98, 8, 8);
  const lp = leaf.attributes.position; const lv = new THREE.Vector3();
  ...
  S.add(noHull(mesh(leaf, M.lettuce, { pos: [0.002, 0.0212, -0.002], rot: [-90 * D2R, 0, -0.05], cast: false })));
  S.add(noHull(mesh(box(bw * 0.8, 0.0034, bd * 0.7), M.eggFill, { pos: [-0.004, 0.0185, 0.004], rot: [0, 0.05, 0], cast: false })));
  S.add(noHull(mesh(box(bw * 0.72, 0.0028, bd * 0.62), M.ham, { pos: [0.002, 0.0162, -0.004], rot: [0, -0.06, 0], cast: false })));
...
function vCream(P, M, rnd, o) {
  ...
  const line = [[-rx * 0.62, ry * 1.78, 0.001], [-rx * 0.2, ry * 1.9, -0.002], [rx * 0.2, ry * 1.9, 0.002], [rx * 0.62, ry * 1.74, 0]];
