...
import { grp, mesh, cyl, sph, lathe, tor, rbox, box, tubeOf, extrude, shape, circ, rand, range, weather, decal } from '../../core/kit.js';
...
function vBar(inner, M, rnd, o) {
  ...
  // バニラ本体（欠けた箇所から見える）
  B.add(mesh(face(W / 2, H, D, 0), M.vanilla, { pos: [0, 0, 0], name: 'bar-core' }));
  // チョコ殻（一回り大きく、上面は別 disk）
  const shell = mesh(face(W / 2 + 0.0022, H + 0.0012, D + 0.0044, 1), M.choco, { pos: [0, 0, 0], name: 'bar-shell' });
  B.add(shell);
...
function vParfait(inner, M, rnd, o) {
  ...
  P.add(noHull(mesh(tubeOf([[0.0055, hC * 0.9 + 0.027, 0.0028], [0.0075, hC * 0.9 + 0.0325, -0.002], [0.0032, hC * 0.9 + 0.0338, -0.0068]], 0.0006, 8, 4), MAT.food({ color: '#6d8a45', spec: 0.3 }), { cast: false })));
  ...
  P.add(noHull(mesh(sph(0.0042, 8, 6), M.cream, { pos: [-0.0075, hC * 0.94, 0.004], scale: [1.3, 0.5, 1.1], cast: false })));
...
function vPopsicle(inner, M, rnd, o) {
  ...
  // 突出しの持ち手（実体の棒）
  stick(K, M, { y: H + 0.0135, h: 0.027, x: -0.0005, rot: [0, 0, 2.5 * D2R] });
  // 霜・缺陷：先欠け
