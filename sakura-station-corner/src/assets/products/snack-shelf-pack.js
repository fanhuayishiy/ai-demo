...
import { grp, mesh, box, rbox, cyl, sph, lathe, tor, circ, plane, tubeOf, rand, range, weather, decal, grid } from '../../core/kit.js';
...
const W = 0.12, H = 0.06, D = 0.09;      // envelope（棚が確保する幅）
...
function mats(o) {
  const t = { tint: o.tint };
  return {
    board: (key, title, sub, bg, ink, motif) => MAT.poster({ map: packArt(key, title, sub, bg, ink, motif), color: '#ffffff', steps: 2, spec: 0.12, shadowAmt: 0.74, side: THREE.DoubleSide, ...t }),
    ...
    cupClear: MAT.glassLite({ color: '#f0f6f6', opacity: 0.3, spec: 1, specPower: 230, side: THREE.DoubleSide, depthWrite: false }),
    alu: MAT.metalPaint('#dfe3e6', { worn: 0.15, repeat: 3, map: aluTex(), spec: 0.8, specPower: 150, shadowAmt: 0.6 }),
    film: MAT.glassLite({ color: '#eef4f6', opacity: 0.2, map: filmTex(), side: THREE.DoubleSide, depthWrite: false }),
...
function vBoxsnack(P, M, rnd, o) {
  ...
  // 内側の紙ライナー（クシュッとした包装紙）
  const lin = new THREE.PlaneGeometry(bw * 0.9, bd * 0.8, 6, 6);
  const lp = lin.attributes.position; const lv = new THREE.Vector3();
  ...
  for (let j = 0; j < nz; j++) for (let i = 0; i < nx; i++) {
    ...
    c.position.set(x + range(rnd, -0.002, 0.002), 0.004, z + range(rnd, -0.002, 0.002));
...
function vYogurt(P, M, rnd, o) {
  ...
  for (let j = 0; j < nz; j++) for (let i = 0; i < nx; i++) {
    ...
    // アルミ蓋（ピールオフ・中心が膨らむ）
    const lidG = new THREE.SphereGeometry(rT + 0.0016, 20, 6, 0, 6.284, 0, 0.34);
...
function vCookie(P, M, rnd, o) {
  ...
  for (let j = 0; j < nz; j++) for (let i = 0; i < nx; i++) {
    ...
    // 焼いた表面（微細な凹凸を頂点変位で）
    const g = new THREE.CylinderGeometry(r, r * 0.985, 0.0072, 20, 1, false);
  ...
  for (const [sx, rz] of [[-1, 6], [1, -5]]) {
    cracked.add(mesh(new THREE.CylinderGeometry(0.012, 0.0118, 0.0068, 14, 1, false, sx < 0 ? 0 : Math.PI, Math.PI), M.cakeCrumb('#d3a566'), { rot: [0, 0, 0], pos: [sx * 0.0022, 0, sx * 0.0016], scale: [1, 1, 0.9] }));
  }
