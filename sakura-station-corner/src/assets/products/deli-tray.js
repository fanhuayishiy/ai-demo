...
import { grp, mesh, box, rbox, cyl, sph, lathe, tor, circ, plane, tubeOf, extrude, shape, rand, range, weather, decal } from '../../core/kit.js';
...
const coneGeo = (r, h) => new THREE.ConeGeometry(r, h, 8);
const TW = 0.134, TH = 0.045, TD = 0.094;    // トレイ基準（envelope 150×45×110 内）
...
function mats(o) {
  ...
  return {
    ...
    wrap: MAT.glassLite({ color: '#eef6f7', opacity: 0.19, map: wrapTex(), side: THREE.DoubleSide, depthWrite: false }),
    paper: MAT.paper({ color: '#fbf6ea', map: paperTex(), spec: 0.05, shadowAmt: 0.82, ...t }),
    // 食材
    leaf: (c) => MAT.food({ color: c, map: undefined, spec: 0.55, specPower: 40, specCut: 0.2, sheen: 0.14, shadowAmt: 0.6, side: THREE.DoubleSide, steps: 4, ...t }),
    leafDeep: (c) => MAT.food({ color: c, spec: 0.45, specPower: 30, sheen: 0.1, shadowAmt: 0.68, side: THREE.DoubleSide, ...t }),
    ...
    ginger: MAT.food({ color: '#f2b7c4', spec: 0.55, specPower: 40, side: THREE.DoubleSide, ...t }),
    salt: MAT.paint('#fdfdf6', { spec: 0.5, specPower: 120, steps: 2 }),
    sesame: MAT.paint('#211d18', { spec: 0.5, specPower: 90, steps: 2 }),
    parsely: MAT.food({ color: '#4f8a45', spec: 0.5, specPower: 40, side: THREE.DoubleSide, ...t }),
...
function leafGeo(len, wid, crinkle, seed) {
  const g = new THREE.PlaneGeometry(len, wid, 9, 7);
  const p = g.attributes.position; const v = new THREE.Vector3();
  const rnd = rand(seed);
  for (let i = 0; i < p.count; i++) {
    v.fromBufferAttribute(p, i);
...
function filmCover(parent, M, { w, d, h, y0, seed }) {
  const g = new THREE.BoxGeometry(w, h, d, 12, 4, 9);
  const p = g.attributes.position; const v = new THREE.Vector3();
  for (let i = 0; i < p.count; i++) {
    v.fromBufferAttribute(p, i);
...
function vSalad(P, M, rnd, o) {
  ...
  for (const [x, z, ry] of [[0.012, 0.024, 0.3], [-0.022, -0.006, -0.6]]) {
    // 玉子（1/4 巾切）
    const e = grp('egg'); B.add(e);
    e.position.set(x, base + 0.0095, z); e.rotation.set(0, ry, 0);
    e.add(noHull(mesh(new THREE.CylinderGeometry(0.0128, 0.0128, 0.017, 16, 1, false, 0, Math.PI / 2), M.eggWhite, { pos: [0, 0.0085, 0], rot: [90 * D2R, 0, 0], cast: false })));
    e.add(noHull(mesh(new THREE.CylinderGeometry(0.0068, 0.0068, 0.0166, 14, 1, false, 0, Math.PI / 2), M.eggYolk, { pos: [0, 0.0085, 0], rot: [90 * D2R, 0, 0], cast: false })));
  }
  for (let i = 0; i < 14; i++) {
...
function vKaraage(P, M, rnd, o) {
  ...
  const spot = [[-0.036, 0.011], [-0.007, 0.021], [0.024, 0.005], [-0.022, -0.013], [0.011, -0.021], [0.04, -0.01], [-0.044, -0.005], [0.042, 0.017]];
  for (let i = 0; i < n; i++) {
    ...
    k.position.set(x + range(rnd, -0.003, 0.003), base + ry * 0.74, z + range(rnd, -0.003, 0.003));
  ...
  lm.add(mesh(new THREE.CylinderGeometry(0.0165, 0.0165, 0.007, 16, 1, false, 0, Math.PI / 3), M.lemon, { pos: [0, 0, 0.0035] }));
  lm.add(noHull(mesh(new THREE.CylinderGeometry(0.0142, 0.0142, 0.0076, 14, 1, false, 0, Math.PI / 3), M.lemonIn, { pos: [0, 0, 0.0035], cast: false })));
  for (let i = 0; i < 4; i++) {
...
function vHamburgu(P, M, rnd, o) {
  ...
  // デミグラスソース（上から掛けて縁を伝う）
  const src = new THREE.SphereGeometry(1, 22, 12, 0, 6.284, 0, 0.62);
  const sp = src.attributes.position; const v = new THREE.Vector3();
  ...
  patty.add(mesh(src, M.sauce, { pos: [0.002, 0.0062, -0.001], name: 'demi-glace' }));
  ...
  msh.add(noHull(mesh(tubeOf([[-0.012, 0.0115, 0.004], [0, 0.014, -0.002], [0.013, 0.011, 0.003]], 0.0016, 10, 5), MAT.food({ color: '#e6d3a0', spec: 0.4 }), { cast: false })));
  // 人参（花形）・ブロッコリー 1 房
...
function vEdamame(P, M, rnd, o) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const g = podGeo(len, r, 2 + ((i % 2) ? 1 : 0), i * 7 + 3);
...
function vSushiRoll(P, M, rnd, o) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    r.position.set(x + range(rnd, -0.002, 0.002), base + hgt / 2 + (row ? 0.001 : 0), z);
  ...
  for (let i = 0; i < 3; i++) {
    const gp = new THREE.PlaneGeometry(0.018, 0.012, 5, 4);
    const pp = gp.attributes.position; const vv = new THREE.Vector3();
...
function vPotato(P, M, rnd, o) {
  ...
  const mp = mound.attributes.position; const v = new THREE.Vector3();
