...
import { grp, mesh, cyl, sph, lathe, tor, rbox, box, tubeOf, circ, rand, range, weather, decal } from '../../core/kit.js';
...
function mats(tint, lidKey) {
  ...
  return {
    paperOut: MAT.paper({ color: '#ffffff', map: print, side: THREE.DoubleSide, uv: { repeat: [1, 1] }, spec: 0.12, ...t }),
    paperIn: MAT.paper({ color: '#f3ecdd', side: THREE.DoubleSide, spec: 0.08, shadowAmt: 0.9, ...t }),
    printBand: MAT.poster({ map: print, color: '#ffffff', side: THREE.DoubleSide, sat: 1.02, ...t }),
    sleeve: MAT.paper({ color: '#ffffff', map: slv, alphaMap: slv, alphaTest: 0.5, side: THREE.DoubleSide, spec: 0.08, shadowAmt: 0.82, ...t }),
    lid: MAT.hardPlastic(LIDCOLOR[lidKey] || lidKey || LIDCOLOR.black, { repeat: 5, spec: 0.55, specPower: 78, ...t }),
    lidIn: MAT.plastic('#b9b6ae', { steps: 2, side: THREE.DoubleSide, shadowAmt: 0.95 }),
...
function oneCup(parent, M, sp, o, rnd, idx) {
  ...
  C.add(mesh(new THREE.CylinderGeometry(bandR(hb * 0.62), bandR(hb * 0.32), bandH, 28, 1, true, -Math.PI / 2, Math.PI * 2),
...
function cupLid(C, M, sp, o, lidKey) {
  ...
  const prof = [
    ...
    [r0 * 0.88, lid * 0.3], [r0 * 0.95, lid * 0.1], [r0, 0.0], [r0 - 0.0016, -0.001],
    [r0 - 0.0055, -0.0022], [r0 - 0.0055, lid * 0.34], [r0 - 0.0075, lid * 0.36], [0, lid * 0.36],
  ];
  ...
  spout.add(noHull(mesh(new THREE.CylinderGeometry(rt * 0.3, rt * 0.32, 0.004, 16, 1, true), M.lid, { scale: [1, 1, 0.62], cast: false })));
  spout.add(noHull(mesh(circ(rt * 0.29, 16), M.coffee, { pos: [0, -0.0006, 0], rot: [-90 * D2R, 0, 0], scale: [1, 0.62, 1], cast: false, receive: false })));
...
function cupSleeve(C, M, sp, o) {
  ...
  const g = new THREE.CylinderGeometry(r1, r0, y1 - y0, 30, 1, true, -Math.PI / 2, Math.PI * 2);
...
function cupStraw(C, M, sp, o) {
  ...
  const pts = [[0.004, sp.hb * 0.28, 0.002], [0.0055, sp.hb * 0.8, 0.003], [0.0085, sp.hb + sp.lid + 0.004, 0.0035], [0.0135, top - 0.014, -0.001], [0.021, top, -0.007]];
...
export function build(options = {}) {
  ...
  for (let i = stack - 1; i >= 0; i--) {
    ...
    if (i === 0) {
      ...
      if (options.straw && (options.lid !== false || options.fill !== false)) cupStraw(C, M, sp, { seed });
