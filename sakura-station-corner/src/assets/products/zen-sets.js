...
import { grp, mesh, cyl, sph, lathe, tor, tubeOf, extrude, shape, circ, rand, range, weather } from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, heightToNormal, memo, shade, rr } from '../../core/textures.js';
...
function mats(tint) {
  ...
  return {
    ...
    konnyaku: MAT.food({ color: '#e2dfd6', map: konnyakuTex(), spec: 0.55, specPower: 22, specCut: 0.2, sheen: 0.1, shadowAmt: 0.62, side: THREE.DoubleSide, ...t }),
    ...
    chikuwa: MAT.food({ color: '#dcab5f', spec: 0.6, specPower: 26, specCut: 0.17, sheen: 0.12, shadowAmt: 0.64, side: THREE.DoubleSide, steps: 4, ...t }),
    chikuwaIn: MAT.food({ color: '#b98a4d', spec: 0.3, specPower: 14, shadowAmt: 0.9, side: THREE.DoubleSide, ...t }),
...
/** 竹串：尖った先端 + 節 + 煮汁の滴り */
function skewer(parent, M, { x = 0, z = 0, y0 = -0.004, y1 = 0.108, tilt = 0, through = true }) {
  const s = grp('skewer');
  const r = 0.0016;
  const L = y1 - y0;
...
function vDaikon(inner, M, rnd, o) {
  ...
  body.add(noHull(mesh(cyl(R + 0.0006, R + 0.0006, 0.006, 26, true), MAT.water({
    color: '#b9814a', opacity: 0.32, spec: 0.5, side: THREE.DoubleSide, scroll: [0.002, 0.004],
  ...
  if (o.skewer) skewer(inner, M, { x: 0.001, z: -0.0015, tilt: 1.2 });
}
...
function vEgg(inner, M, rnd, o) {
  ...
  const shellHalf = mesh(new THREE.SphereGeometry(1, 24, 16, Math.PI, Math.PI), M.eggWhite, { name: 'egg-shell' });
  ...
  face.add(noHull(mesh(sph(1, 20, 12, 0, 6.284, 0, 0.62), M.eggYolk, { pos: [0, -0.0015, 0.0012], scale: [0.0126, 0.0126, 0.0112], cast: false })));
  face.add(noHull(mesh(new THREE.RingGeometry(0.0112, 0.0142, 24), M.eggYolkDeep, { pos: [0, -0.0015, 0.0009], cast: false, receive: false })));
  // 白身の外側（殻側）の薄皮筋
  ...
  if (o.skewer) skewer(inner, M, { x: -0.004, z: 0.004, tilt: 2.4 });
}
...
function vKonnyaku(inner, M, rnd, o) {
  ...
  const s = shape((sh) => {
    ...
    sh.quadraticCurveTo(-0.0022, H * 0.485, -0.0058, H * 0.44);
  ...
  k.add(noHull(mesh(inner2, MAT.food({ color: '#cfcdc4', transparent: true, opacity: 0.5, spec: 0.4, side: THREE.BackSide }), { cast: false, receive: false })));
  // 缺陷：端の欠け・表面のえぐれ
  k.add(noHull(mesh(sph(0.0038, 8, 6), MAT.food({ color: '#c8c5bb', spec: 0.45 }), {
    pos: [W * 0.55, -H * 0.3, D * 0.2], scale: [1.4, 0.5, 0.7], cast: false,
  })));
  weather(k, { w: 0.02, h: 0.02, pos: [-0.004, H * 0.06, D / 2 + 0.0026], kind: 'dirt', color: '#9b7746', opacity: 0.26, seed: (o.seed ?? 1) + 17, density: 1.0, spread: 0.002 });
  glossyDrops(k, M, [[0.004, -H * 0.28, D * 0.55, 0.002], [0.008, H * 0.05, D * 0.5, 0.0016], [-0.006, -H * 0.05, D * 0.52, 0.0018]], (o.seed ?? 1) + 19);
...
function vSatsuma(inner, M, rnd, o) {
  ...
  if (o.skewer) skewer(inner, M, { x: -0.006, z: 0.004, y0: 0.002, tilt: 1.6 });
}
...
function vNorimaki(inner, M, rnd, o) {
  ...
  const wrap = (rad, y0, hgt, seam, mat) => {
    const g = new THREE.CylinderGeometry(rad, rad * 0.99, hgt, 26, 1, true, seam, Math.PI * 2);
  ...
  weather(n, { w: 0.016, h: 0.02, pos: [0, H * 0.35, -(R + 0.0026)], rot: [0, Math.PI, 0], kind: 'chip', color: '#8d8574', opacity: 0.4, seed: (o.seed ?? 1) + 41, density: 1.2, spread: 0.002 });
