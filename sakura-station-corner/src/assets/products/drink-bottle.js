...
import { grp, mesh, cyl, tor, lathe, circ, rand, range, weather } from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  shellGeo = displace(shellGeo, (v) => {
    ...
    // 下部の水平リブ（型割りの沟）
    if (v.y > 0.018 && v.y < 0.070) dr += -0.00042 * sp.rib * (0.5 + 0.5 * Math.sin(v.y * 205));
    // 縦の微細な収縮あと
  ...
  if (sp.defect === 'dent') shellGeo = displace(shellGeo, dentFn([range(rnd, -1.0, 1.0) + (rnd() > 0.5 ? 0.4 : -2.4), range(rnd, 0.055, 0.125), 0.020, range(rnd, 0.0038, 0.0062)]));
  if (sp.defect === 'crush') shellGeo = displace(shellGeo, crushFn(sp));
