...
import { grp, mesh, box, rbox, cyl, sph, tor, plane, circ, tubeOf, extrude, shape, rand, range, weather, decal } from '../../core/kit.js';
...
function mats(o) {
  const t = { tint: o.tint };
  return {
    clear: MAT.glassLite({ color: '#eef6f8', opacity: 0.26, spec: 1, specPower: 240, side: THREE.DoubleSide, depthWrite: false }),
    ...
    card: (map) => MAT.poster({ map, color: '#ffffff', steps: 2, spec: 0.1, specPower: 26, shadowAmt: 0.75, side: THREE.DoubleSide, ...t }),
    ...
    board: MAT.paper({ color: '#f3ecd8', map: TEX.paper({ base: '#f3ecd8', repeat: 2 }).map, spec: 0.07, shadowAmt: 0.8, side: THREE.DoubleSide, ...t }),
    tape: MAT.glassLite({ color: '#f6f1e2', opacity: 0.5, side: THREE.DoubleSide, depthWrite: false }),
...
function vShelf(P, M, rnd, o) {
  ...
  G.add(mesh(box(S.w, S.h, wall), M.clear, { pos: [0, S.h / 2, -S.d / 2 + wall / 2], name: 'tag-back' }));
  ...
  // 上面の差し込みリップ（紙札を落とすフチ）
  for (const [y, z, sx, sz] of [[S.h - 0.0004, S.d / 2 - 0.0006, S.w, 0.0012], [S.h - 0.0004, -S.d / 2 + 0.0006, S.w, 0.0012]]) {
  ...
  G.add(noHull(mesh(plane(S.w - 0.005, S.h - 0.005), face, { pos: [0, S.h / 2, -0.0012], cast: false, receive: false })));
...
function vNew100(P, M, rnd, o) {
  ...
  // 商品名・税抜注記
  G.add(noHull(mesh(plane(0.03, 0.012), M.card(faceTex('name100', 'お徳用', '10 個入 · 税抜', '#e0812c')), { pos: [-S.w * 0.16, S.h * 0.52, S.d / 2 + 0.0006], cast: false, receive: false })));
  // 黄緑の「100 円均一」テープ
  G.add(noHull(mesh(box(S.w, 0.0026, 0.0006), M.holderY, { pos: [0, S.h - 0.0014, S.d / 2 - 0.0004], cast: false })));
  // 缺陷：端の剥がれ・日焼け・糊残り
  weather(G, { w: 0.014, h: 0.008, pos: [-S.w * 0.32, S.h * 0.2, S.d / 2 + 0.0009], kind: 'chip', color: '#d9d2bd', opacity: 0.45, seed: (o.seed ?? 1) + 5, spread: 0.0008 });
...
function vSale(P, M, rnd, o) {
  ...
  // 裏の離型材（うすい青白）
  G.add(noHull(mesh(plane(R * 1.94, R * 1.94), MAT.paper({ color: '#e6eef2', side: THREE.DoubleSide, spec: 0.05 }), { pos: [0, R, -0.0005], rot: [0, Math.PI, 0], cast: false, receive: false })));
  // 捲くれた角（1 箇所）
...
function vHook(P, M, rnd, o) {
  ...
  G.add(noHull(mesh(new THREE.TorusGeometry(0.0042, 0.0009, 6, 16), M.holder, { pos: [0, h - 0.006, 0], cast: false })));
...
function vPop(P, M, rnd, o) {
  ...
  clip.add(mesh(box(0.024, 0.009, 0.0016), M.holder, { pos: [0, -0.004, -0.0052] }));
  clip.add(noHull(mesh(tor(0.0028, 0.0011, 5, 12), M.holder, { pos: [0.011, 0.001, -0.0026], rot: [0, Math.PI / 2, 0], cast: false })));
  clip.add(noHull(mesh(tor(0.0028, 0.0011, 5, 12), M.holder, { pos: [-0.011, 0.001, -0.0026], rot: [0, Math.PI / 2, 0], cast: false })));
  // 数字（価格）を下部に
...
function vFloor(P, M, rnd, o) {
  ...
  const fold = 0.14;                          // 上端の折り（前後パネルの傾き）
  ...
  G.add(noHull(mesh(cyl(0.012, 0.012, w, 12, 1, false, 0, Math.PI), face, { pos: [0, h + 0.006, 0.003], rot: [0, 90 * D2R, 0], cast: false })));
  // 底の足（前に折り返した台＝自立させる錘）
  ...
  digitsDecal(holder, { text: '¥150', w: 0.022, h: 0.011, pos: [0.012, -0.008, 0.0026], bg: '#fffdf2', fg: '#c0392b' });
