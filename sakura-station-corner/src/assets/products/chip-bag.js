...
import { grp, mesh, box, cyl, tor, rbox, tubeOf, rand, range, decal } from '../../core/kit.js';
...
function finGeo(w, h, teeth, depth, up) {
  ...
  s.lineTo(x0, lo);
  const geo = new THREE.ExtrudeGeometry(s, { depth, bevelEnabled: false, curveSegments: 2 });
...
export function build(options = {}) {
  ...
  const film = MAT.paint(sp.bg, {
    ...
    map: TEX.paper({ base: sp.bg, repeat: 3 }).map, side: THREE.DoubleSide, sat: 1.04,
  });
  ...
  const front = MAT.poster({
    ...
    side: THREE.DoubleSide, steps: 3, sat: sp.defect === 'faded' ? 0.72 : 1.06,
  ...
  const back = MAT.poster({
    ...
    side: THREE.DoubleSide, steps: 3, sat: 1.0, spec: 0.34,
  });
  const bandMat = MAT.poster({
    map: TEX.drinkLabel({ name: sp.title, sub: sp.sub, ml: '65g', a: sp.band, b: sp.bg, kind: 'bag' }),
    side: THREE.DoubleSide, uv: { offset: [0.5, 0] }, steps: 3, sat: sp.defect === 'faded' ? 0.74 : 1.06, spec: 0.44,
  });
  const PI = Math.PI;
  ...
  const fw = W * 0.90;
  const fh = FIN * 1.45;
  holder.add(mesh(finGeo(fw, fh, 15, 0.0026, true), finMat, { name: 'seal-top', pos: [0, H - 0.0006 - fh, -0.0013] }));
  holder.add(mesh(finGeo(fw, fh, 15, 0.0028, false), finMat, { name: 'seal-bottom', pos: [0, 0.0004, -0.0014] }));
  // フィンの際（アルミ内面が覗く）
  ...
  if (sp.clip) {
    ...
    clip.add(mesh(tubeOf([[-0.0300, 0.0022, 0.0], [-0.0352, -0.0030, 0.0], [-0.0280, -0.0074, 0.0]], 0.0022, 8, 5), clipMat, { name: 'clip-arm-l', cast: false }));
    clip.add(mesh(tubeOf([[0.0300, 0.0022, 0.0], [0.0352, -0.0030, 0.0], [0.0280, -0.0074, 0.0]], 0.0022, 8, 5), clipMat, { name: 'clip-arm-r', cast: false }));
  ...
  if (sp.defect === 'scuff') {
    decal(holder, { map: TEX.wear({ kind: 'dirt', color: '#cfc7ae', seed: seed + 22, density: 1.6 }), w: 0.040, h: 0.026, pos: [-0.020, 0.0620, zsurf(0.0620)], opacity: 0.42 });
  }
  if (sp.defect === 'faded') {
