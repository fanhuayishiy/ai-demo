...
import { grp, mesh, box, rbox, cyl, tor, lathe, rand, range, weather, decal } from '../../core/kit.js';
...
function foilLiner(parent, M, rnd, y0, y1, seed) {
  ...
  for (let i = 0; i < 3; i++) {
    const p = mesh(rbx(W * (0.64 - i * 0.08), 0.0012, D * (0.72 + i * 0.14), 0.0006), M.foil, {
      name: 'foil-crease' + i, pos: [(i - 1) * 0.003 + range(rnd, -0.001, 0.001), hh - 0.0040 - i * 0.0030, 0.0006],
...
function barCore(bar, M, sp, rnd, topY) {
  ...
  if (k === 'choco') {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      bar.add(m);
      m.add(mesh(cyl(0.0038, 0.0030, 0.0160, 10), M.biscuit, { name: 'stem', pos: [0, -0.0075, 0] }));
      m.add(mesh(lathe([[0, 0.0068], [0.0040, 0.0062], [0.0086, 0.0036], [0.0092, 0.0004], [0.0074, -0.0012], [0, -0.0016]], 16), M.choco, { name: 'cap', pos: [0, 0.0008, 0] }));
    }
  } else if (k === 'almond') {
    ...
    for (let i = 0; i < 5; i++) {
      bar.add(mesh(rbx(0.0066, 0.0026, 0.0040, 0.0012), M.almond, {
        name: 'almond' + i, pos: [range(rnd, -0.004, 0.004), topY - 0.0042 - i * 0.0090, D * 0.26],
...
export function build(options = {}) {
  ...
  if (pack > 1) {
    ...
    for (let i = 0; i < pack; i++) {
      ...
      one.rotation.set(0, range(rnd, -0.09, 0.09), range(rnd, -0.006, 0.006));
      bundle.add(one);
    }
...
function single(sp, M, seed, seedN, side) {
  ...
  const lean = range(rnd, -0.030, 0.030) * side;               // 陳列の小さな倒れ（Y 軸のみ＝寸法を変えない）
  const holder = grp('holder', { rot: [0, lean, range(rnd, -0.004, 0.004)] });
  bar.add(holder);
  ...
  // 側シーム（折り代の筋）
  for (const s of [1, -1]) {
    holder.add(mesh(patchGeo(sp, s > 0 ? -0.20 : PI - 0.20, s > 0 ? 0.20 : PI + 0.20, 0.06, 0.94, 0.0003, topCut), M.foilDark,
      { name: 'side-seam' + s, cast: false, receive: false, renderOrder: 2 }));
  }

  /* --- 下フィン（ギザ）と上フィン／開封なら破断エッジ --- */
  const fw = W * 0.93, fh = FIN * 1.5;
  holder.add(mesh(finGeo(fw, fh, 9, 0.0022, false), M.film, { name: 'seal-bottom', pos: [0, 0.0004, -0.0011] }));
  holder.add(mesh(box(fw * 0.96, 0.0012, 0.0046), M.foil, { name: 'seal-edge-bottom', pos: [0, FIN + 0.0012, 0], cast: false }));
  if (!sp.open) {
    holder.add(mesh(finGeo(fw, fh, 9, 0.0022, true), M.film, { name: 'seal-top', pos: [0, H - 0.0006 - fh, -0.0011] }));
    holder.add(mesh(box(fw * 0.96, 0.0012, 0.0046), M.foil, { name: 'seal-edge-top', pos: [0, H - FIN - 0.0012, 0], cast: false }));
  } else {
    const ty = H * 0.80;
    holder.add(mesh(tornGeo(fw, 0.0125, 7, 0.0024), M.film, { name: 'torn-edge', pos: [0, ty - 0.0040, -0.0012], renderOrder: 2 }));
  ...
  if (sp.defect === 'crease') {
    ...
    decal(holder, { map: TEX.wear({ kind: 'scratch', color: '#ffffff', seed: seed + 22, density: 1.4 }), w: 0.012, h: 0.026, pos: [-0.009, H * 0.70, zfront(0.70)], rot: [0, 0, 0.5], opacity: 0.28 });
  }
  if (sp.defect === 'faded') {
  ...
  decal(holder, { map: TEX.wear({ kind: 'dirt', color: '#d8d2c4', seed: seed + 25, density: 0.9 }), w: 0.011, h: 0.013, pos: [-0.006, H * 0.30, zfront(0.30)], rot: [0, 0, 0.4], opacity: 0.26 });
  // 下端の棚擦れ
