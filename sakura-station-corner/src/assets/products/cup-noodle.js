...
import { grp, mesh, box, cyl, sph, tor, circ, lathe, coil, tubeOf, rand, range, decal } from '../../core/kit.js';
...
const SZ = 0.095, R_MAX = 0.0470, RIM_REF = 0.0858;
...
export function build(options = {}) {
  ...
  const paperIn = MAT.paper({ color: '#f8f4ea', repeat: 4, side: THREE.DoubleSide, spec: 0.08, shadowAmt: 0.95 });
  ...
  const sleeveMat = MAT.poster({
    ...
    side: THREE.DoubleSide, uv: { offset: [0.5, 0] }, steps: 3, sat: sp.defect === 'faded' ? 0.74 : 1.06,
  ...
  foodG.add(mesh(coil(0.0150, 0.0170, 4, 10, 0.0032), nMat, { name: 'noodles-2', pos: [0.002, SOUP_Y - 0.0030, -0.001], rot: [0, 0.7, 0.10], cast: false }));
  for (let i = 0; i < 14; i++) {
  ...
  const T = {
    shrimp: (i) => {
      ...
      s.add(mesh(box(0.0056, 0.0015, 0.0076), MAT.food({ color: '#f3c8b8', spec: 0.44 }), { pos: [0.0082, -0.0010, 0], rot: [0, 0, -0.4], cast: false }));
      for (let k = 0; k < 3; k++) s.add(mesh(box(0.0011, 0.0068, 0.0070), MAT.food({ color: '#d96a52', spec: 0.4 }), { pos: [-0.0038 + k * 0.0034, 0.0026, 0], cast: false, receive: false }));
      return s;
    },
    ...
    carrot: () => mesh(cyl(0.0038, 0.0038, 0.0014, 8), MAT.food({ color: '#e07a28', spec: 0.42 }), { name: 'carrot', pos: [-0.009, SOUP_Y + 0.0028, -0.009], rot: [1.2, 0.3, 0], cast: false }),
    beef: (i) => mesh(box(0.0108, 0.0022, 0.0072), MAT.food({ color: '#6b3f2a', spec: 0.42, shadowAmt: 0.7 }), { name: 'beef', pos: [-0.007 + i * 0.013, SOUP_Y + 0.0032, 0.005 - i * 0.011], rot: [0.2, i * 1.1, 0.1], cast: false }),
    fishcake: () => mesh(box(0.0156, 0.0016, 0.0078), MAT.food({ color: '#f4efe2', spec: 0.42 }), { name: 'fishcake', pos: [0.002, SOUP_Y + 0.0032, -0.007], rot: [0, 0.4, 0.03], cast: false }),
    menma: () => {
      const s = grp('menma');
    ...
    nori: () => mesh(box(0.0194, 0.0006, 0.0126), MAT.nori({ side: THREE.DoubleSide }), { name: 'nori', pos: [-0.011, SOUP_Y + 0.0034, -0.004], rot: [0.1, 0.3, -0.16], cast: false }),
    corn: () => {
      const s = grp('corn');
      for (let k = 0; k < 8; k++) s.add(mesh(sph(0.0020, 7, 5), MAT.food({ color: '#f0c33c', spec: 0.5 }), { pos: [-0.008 + (k % 4) * 0.0050, SOUP_Y + 0.0026 + ((k / 4) | 0) * 0.0030, 0.009 + ((k / 4) | 0) * 0.0038], cast: false }));
      return s;
    },
    butter: () => mesh(box(0.0086, 0.0032, 0.0062), MAT.food({ color: '#f2dd8e', spec: 0.5, shadowAmt: 0.6 }), { name: 'butter', pos: [0.006, SOUP_Y + 0.0046, -0.007], rot: [0, 0.4, 0], cast: false }),
    bean: () => {
      const s = grp('bean');
      for (let k = 0; k < 5; k++) s.add(mesh(sph(0.0025, 7, 5), MAT.food({ color: '#4a5f38', spec: 0.4 }), { pos: [-0.009 + k * 0.0042, SOUP_Y + 0.0028, -0.013 + (k % 2) * 0.0034], cast: false }));
      return s;
    },
    squid: () => {
      const s = grp('squid');
      for (let k = 0; k < 3; k++) s.add(mesh(box(0.0080, 0.0018, 0.0058), MAT.food({ color: '#efe9dc', spec: 0.5 }), { pos: [0.010 - k * 0.004, SOUP_Y + 0.0030 + k * 0.0010, -0.006 + k * 0.005], rot: [0.1, 0.6 * k, 0.05], cast: false }));
      return s;
    },
    broccoli: () => {
  ...
  if (sp.lid === 'sealed') {
    ...
    lidG.add(mesh(cyl(0.0015, 0.0015, 0.0014, 8), MAT.paint('#4a453c', { steps: 2 }), { name: 'steam-hole', pos: [0.013, 0.0018, -0.009], cast: false }));
  } else {
    // 前半を剝いて後面へ折り返す
    ...
    fold.position.set(-0.002, 0.0016, -0.0008);
    lidG.add(fold);
    ...
    // 内側に折れたフタの影（黒ズミ）
    lidG.add(mesh(discGeo(ringR * 0.94, (t, a) => 0.0002, 20, 3, Math.PI, Math.PI * 2), MAT.metal('#8f9498', { spec: 0.4, side: THREE.DoubleSide }), { name: 'lid-shadow', pos: [0, -0.0008, 0], cast: false, receive: false }));
  }
  ...
  g.add(spoon);
  spoon.add(mesh(box(0.0580, 0.0016, 0.0082), plastic, { name: 'spoon-handle', pos: [-0.0120, 0, 0], cast: false }));
  const bowl = mesh(sph(0.0088, 12, 9), plastic, { name: 'spoon-bowl', pos: [0.0232, -0.0006, 0], cast: false });
  bowl.scale.set(1.28, 0.34, 0.86);
  spoon.add(bowl);
  spoon.add(mesh(box(0.0066, 0.0018, 0.0090), plastic, { pos: [0.0140, -0.0002, 0], cast: false }));
  const tapeMat = MAT.paper({ color: '#f6e9c8', repeat: 5, side: THREE.DoubleSide, transparent: true, opacity: 0.94, depthWrite: false });
  for (const [dx, dz, rot] of [[-0.018, 0.014, 0.30], [0.020, -0.006, -0.55]]) {
  ...
  if (sp.hook) {
    ...
    card.lineTo(-tabW / 2, 0);
    const euro = new THREE.Path();
    euro.moveTo(-0.0018, 0.0060);
    euro.lineTo(-0.0018, 0.0084);
    ...
    const geo = new THREE.ExtrudeGeometry(card, { depth: 0.0016, bevelEnabled: false, curveSegments: 6 });
    const tab = grp('hang-tab', { pos: [0, rimY - 0.0034, -0.0440], rot: [-0.05, 0, 0] });
    g.add(tab);
    tab.add(mesh(geo, MAT.poster({
      map: TEX.poster({ title: sp.name, sub: '3食パック', bg: sp.b, accent: sp.a, seed: (seed % 71) + 7 }),
      steps: 3, side: THREE.DoubleSide, sat: 1.04,
    }), { name: 'hang-card', pos: [0, 0, -0.0016] }));
    // タブとカップを留める糊代
    tab.add(mesh(box(0.0300, 0.0060, 0.0012), MAT.paper({ color: '#e5ddc8', repeat: 5 }), { pos: [0, -0.0016, 0.0018], cast: false }));
  }
