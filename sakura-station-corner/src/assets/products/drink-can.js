...
import { grp, mesh, cyl, tor, lathe, circ, sph, shape, rand, range, weather } from '../../core/kit.js';
...
function tabGeo() {
  const s = shape((k) => {
    k.moveTo(-0.0052, -0.0030);
    k.lineTo(0.0072, -0.0041);
    k.quadraticCurveTo(0.0116, -0.0041, 0.0116, -0.0002);
    k.quadraticCurveTo(0.0116, 0.0039, 0.0072, 0.0042);
    k.lineTo(-0.0052, 0.0034);
    k.quadraticCurveTo(-0.0092, 0.0034, -0.0092, 0.0004);
    k.quadraticCurveTo(-0.0092, -0.0030, -0.0052, -0.0030);
  });
  const hole = new THREE.Path();
  hole.absellipse(0.0038, 0.0002, 0.0044, 0.0022, 0, Math.PI * 2, false);
  s.holes.push(hole);
  const g = new THREE.ExtrudeGeometry(s, { depth: 0.0007, bevelEnabled: true, bevelSize: 0.00025, bevelThickness: 0.00025, bevelSegments: 1, curveSegments: 10 });
...
export function build(options = {}) {
  ...
  if (sp.defect === 'dent') bodyGeo = displace(bodyGeo, (v) => {
    ...
    return gg < 0.02 ? null : { r: -0.0046 * gg };
  });
  if (sp.defect === 'crush') bodyGeo = displace(bodyGeo, (v) => ({ r: -0.003 * Math.exp(-((v.y - 0.04) ** 2) / 0.002) * Math.max(0, Math.cos(Math.atan2(v.x, v.z))) }));
  ...
  const labMat = MAT.poster({
    map: labTex, side: THREE.DoubleSide, uv: { offset: [0.5, 0] },
  ...
  if (opened) {
    // 開口部の暗がり（内側に沈んだ底）
    const dark = mesh(circ(1, 22), MAT.paint('#241d18', { steps: 2, shadowAmt: 1, side: THREE.DoubleSide }), {
  ...
  // リベット
  topG.add(mesh(cyl(0.0022, 0.0027, 0.0018, 12), aluTop, { name: 'rivet', pos: [0, 0.1170, -0.0058] }));
  topG.add(mesh(tor(0.0025, 0.0005, 5, 14), MAT.metal('#bcc1c4', { spec: 0.82 }), { name: 'rivet-ring', pos: [0, 0.1166, -0.0058], rot: [-Math.PI / 2, 0, 0] }));
  // プルタブ（リベットで回動）
  const tabMat = MAT.canBody({ color: '#e3e7ea', repeat: 6, spec: 0.92, specPower: 210 });
  const tabPivot = grp('tab-pivot', { pos: [0, 0.1179, -0.0058] });
