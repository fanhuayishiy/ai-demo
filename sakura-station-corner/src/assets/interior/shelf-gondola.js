...
import * as P_BOTTLE from '../products/drink-bottle.js';
import * as P_CAN from '../products/drink-can.js';
import * as P_MILK from '../products/milk-carton.js';
import * as P_CHIP from '../products/chip-bag.js';
import * as P_CANDY from '../products/candy-bar.js';
import * as P_SNACK from '../products/snack-shelf-pack.js';
import * as P_BREAD from '../products/bread-set.js';
...
function stockUpright(parent, kind, o) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + shift + (r ? range(rnd, -0.013, 0.013) : range(rnd, -0.002, 0.002));
...
function stockFlat(parent, kind, o) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    const count = r === 0 ? n : Math.max(1, Math.round(n * back));
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + (r ? range(rnd, -0.009, 0.009) : 0);
...
function hookRow(parent, kind, { x0, x1, y, z, seed, rnd, variants }, mats) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    it.position.set(x + range(rnd, -0.003, 0.003), y - 0.016 - h, z + 0.048);
...
function emptySlot(parent, { x, y, w, seed, rnd }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const t = stockItem(`tag:${i % 4 === 3 ? 'new100' : 'shelf'}`, () => P_TAG.build({ seed: seed + i * 53, variant: i % 4 === 3 ? 'new100' : 'shelf' }));
...
export function build(options = {}) {
  ...
  const mats = {
    ...
    signPoster: MAT.poster({
      ...
      side: THREE.DoubleSide, steps: 3, sat: 0.98,
    }),
  };
  ...
  dust(deck, { w: 0.40, h: 0.22, pos: [len * 0.31, deckY + 0.002, -0.10], rot: [-PI / 2, 0, 0], kind: 'dirt', color: '#6b645a', opacity: 0.28, seed: seed + 202, density: 2.0, spread: 0.05 });
  {
    ...
    for (let t = 0; t < tiers; t++) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (EMPTY.some((e) => e[0] === t && e[1] === f && e[2] === i)) {
          ...
          if (t === 3 && i === 1) {
            const fallen = P_BOTTLE.build({ seed: seed + 9091, variant: 'water' });
            fallen.position.set(x1 - 0.037, y + 0.0335, Z_FACE - 0.100);
            fallen.rotation.set(0, 0.12, PI / 2);
            fallen.name = 'fallen-bottle';
            face.add(fallen);
            const laid = P_CHIP.build({ seed: seed + 613, variant: 'udon' });
          ...
          if (t === 2 && i === 2) {
            const rolled = P_CAN.build({ seed: seed + 9101, variant: 'juice' });
            rolled.position.set(cx + 0.10, y + 0.036, Z_FACE - 0.20);
            rolled.rotation.set(PI / 2, 0.55, 0);
            rolled.name = 'rolled-can';
            face.add(rolled);
            const laid2 = P_SNACK.build({ seed: seed + 619, variant: 'jelly' });
          ...
          if (tall || flat) caseStock(face, spec.k, { x0, x1, y, z: 0.155, rnd, seed: seed + t * 53 + i * 907 + f * 31, variants: spec.v }, mats);
          // レールの値札（このベイに 2〜4 枚）
          const n = Math.max(2, Math.min(3, Math.round((x1 - x0) / 0.30)));
          for (let k = 0; k < n; k++) {
            const x = x0 + (k + 0.5) * ((x1 - x0) / n);
            const tg = stockItem(`tag:${k % 5 === 3 ? 'pop' : 'shelf'}`, () => P_TAG.build({ seed: seed + Math.round(x * 977) + t * 31 + k + f * 7, variant: k % 5 === 3 ? 'pop' : 'shelf' }));
