...
import {
  grp, mesh, box, rbox, cyl, sph, tor, lathe, plane, tubeOf, coil, catenary, grill,
...
import { build as zenSets } from '../products/zen-sets.js';
...
export function build(options = {}) {
  ...
  const steamMat = MAT.decal({ map: steamTex(), opacity: 0.5, side: THREE.DoubleSide, order: 4 });
  ...
  for (let i = 1; i < 6; i++) {
    const x = BX0 + (i * TL) / 6;
    tank.add(mesh(box(0.007, 0.25, BZ * 2 - 0.03), ss, { pos: [x, FLOOR + 0.128, -0.006] }));
  ...
  for (let i = 0; i < 6; i++) {
    ...
    cellVariant[i].forEach((v, k) => {
      const [ox, oz] = spots[k];
      const item = zenSets({ seed: seed + i * 31 + k * 7, variant: v, skewer: true });
  ...
  g.add(work);
  const wL = (TW + 0.04) / 2 - (BX1 + 0.006) - 0.006;      // 作業帯の半幅
  // まな板・大根・包丁
  ...
  [['daikon', -0.028, 0.008], ['egg', 0.024, -0.012], ['chikuwa', 0.004, 0.03]].forEach(([v, px, pz], i) => {
    const it = zenSets({ seed: seed + 101 + i * 11, variant: v, skewer: true });
  ...
  for (let i = 0; i < 3; i++) {                                                                                     // 加熱照明（呼吸）
    ...
    lamp.add(noHull(mesh(sph(0.026, 12, 9), lampMat, { pos: [0, -0.006, 0], cast: false, receive: false })));
  ...
  popPos.forEach(([px, py, pz, txt], i) => {
    ...
    if (!flat) pop.add(mesh(box(0.008, 0.03, 0.008), MAT.plastic('#c9c3b2'), { pos: [0, -0.045, -0.008], cast: false }));
    g.add(pop);
  });
  ...
  hang.add(noHull(mesh(plane(0.24, 0.16), MAT.poster({ map: TEX.poster({ title: 'おでんの季節', bg: '#efe1c4', accent: '#a44f34', seed: seed + 33 }) }), { pos: [0, 0, -0.004], rot: [0, Math.PI, 0], cast: false })));
