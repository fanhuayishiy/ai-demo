...
import {
  grp, mesh, box, rbox, cyl, tor, plane, grill, finish, rand, range, row, weather, decal, pipe, stockItem,
} from '../../core/kit.js';
...
const Z_BACK = -D / 2 + WALL + 0.012;   // 庫内奥壁面
...
function fillCell(parent, spec, { cx, cellW, y, seed, rnd, back }) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + (r ? pitch * 0.5 + range(rnd, -0.011, 0.011) : range(rnd, -0.0018, 0.0018));
...
export function build(options = {}) {
  ...
  for (let t = 0; t < TIERS.length; t++) {
    ...
    for (let c = 0; c < doors; c++) {
      ...
      for (let k = 0; k < nTag; k++) {
        ...
        const tg = stockItem(`tag:${(k + t) % 5 === 2 ? 'pop' : 'shelf'}`, () => P_TAG.build({ seed: seed + t * 57 + c * 91 + k * 7, variant: (k + t) % 5 === 2 ? 'pop' : 'shelf' }));
  ...
  for (let c = 0; c < doors; c++) {
    ...
    g.add(dg);
    dg.rotation.y = range(rnd, -0.004, 0.004);   // 戸車の摩耗で少し垂れる
    // ガラス（実屈折）
    ...
    for (let k = 0; k < 3; k++) {
      ...
      dg.add(mesh(box(0.026, 0.040, 0.020), steel, { pos: [-doorW / 2 + 0.006, hy, -0.010], name: 'hinge', cast: false }));
      dg.add(mesh(cyl(0.0055, 0.0055, 0.044, 8), chrome, { pos: [-doorW / 2 + 0.006, hy, -0.010], name: 'hinge-pin', cast: false }));
    }
    // 把手（縦バー＋スタッド）
    ...
    // 下部の着霜（ガラス内面に白い帯）
    dg.add(mesh(rbox(doorW - 0.05, 0.085, 0.005, 0.02, 2), frost, { pos: [0, -doorH / 2 + 0.055, -0.006], name: 'glass-frost', cast: false }));
    // 框の褪色・ゴムの変色
