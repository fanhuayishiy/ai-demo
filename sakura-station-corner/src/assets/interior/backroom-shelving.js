...
import {
  grp, mesh, box, rbox, cyl, sph, tor, lathe, circ, plane, tubeOf, coil, catenary, grill, hoop,
  finish, rand, range, weather, decal, shadowBlob,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, memo, rr, jpText, speckle, blotches } from '../../core/textures.js';
import { PAL } from '../../core/palette.js';
import { build as drinkCan } from '../products/drink-can.js';
import { build as milkCarton } from '../products/milk-carton.js';
...
export function build(options = {}) {
  ...
  rack.add(mesh(box(L, H - 0.1, 0.01), MAT.paint('#a89e8c', { map: TEX.paper({ base: '#a89e8c' }).map, spec: 0.08, shadowAmt: 0.94, side: THREE.DoubleSide }), { pos: [0, H / 2 + 0.02, -D / 2 + 0.01], name: 'rack-backboard' }));
  ...
  crateSpec.forEach((c, ci) => {
    ...
    if (c.lid) {
      ...
      // 中身が覗く（缶・牛乳パック・布巾）
      for (let i = 0; i < 5; i++) {
        const can = drinkCan({ seed: seed + 131 + ci * 17 + i * 5, variant: ['coffee', 'cola', 'tea', 'juice', 'energy'][i % 5] });
      ...
      const mk = milkCarton({ seed: seed + 157 + ci * 9, variant: ci ? 'soy' : 'milk1000' });
  ...
  for (const [wx, wz] of [[-0.17, 0.24], [0.17, 0.24], [-0.17, -0.24], [0.17, -0.24]]) {
    ...
    caster.add(mesh(cyl(0.036, 0.036, 0.022, 14), MAT.rubber('#2f3236', { spec: 0.08 }), { pos: [0, -0.008, 0], rot: [0, 0, Math.PI / 2] }));
    caster.add(noHull(mesh(cyl(0.012, 0.012, 0.026, 10), MAT.metal('#c9cdce', { worn: 0.5, spec: 0.6 }), { pos: [0, -0.008, 0], rot: [0, 0, Math.PI / 2], cast: false })));
  ...
  for (let i = 0; i < 14; i++) {
    const a = (i / 14) * 6.284;
    mop.add(noHull(mesh(tubeOf([[Math.cos(a) * 0.02, 0.06, Math.sin(a) * 0.02], [Math.cos(a) * 0.06, 0.02, Math.sin(a) * 0.06], [Math.cos(a) * 0.07, -0.005, Math.sin(a) * 0.07]], 0.006, 8, 5), MAT.fabric({ color: i % 3 ? '#d8d2c0' : '#b8ae9a', repeat: 10 }), {})));
  }
  clean.add(mop);
  // バケツ（水・内面・取手）
  ...
  g.add(webs);
  const webMat = MAT.decal({ map: webTex(), opacity: 0.72, side: THREE.DoubleSide, order: 3 });
  [
