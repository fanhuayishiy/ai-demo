...
import {
  grp, mesh, box, rbox, cyl, finish, rand, range, weather, decal, pipe, stockItem,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, heightToNormal } from '../../core/textures.js';
import { PAL } from '../../core/palette.js';

import * as P_DAILY from '../products/daily-goods.js';
import * as P_TISSUE from '../products/tissue-pack.js';
import * as P_CANDY from '../products/candy-bar.js';
import * as P_SNACK from '../products/snack-shelf-pack.js';
import * as P_MAG from '../products/magazine-item.js';
import * as P_CUP from '../products/cup-noodle.js';
import * as P_TAG from '../products/price-tag.js';
import * as P_TRASH from '../products/trash-liner.js';
...
function stockUpright(parent, kind, o) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + (r ? pitch * 0.5 + range(rnd, -0.012, 0.012) : range(rnd, -0.002, 0.002));
...
function stockFlat(parent, kind, o) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + (r ? range(rnd, -0.010, 0.010) : 0);
...
function leanRow(parent, kind, { x0, x1, y, z, seed, rnd, variants, tilt = 0.30 }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    it.rotation.set(-tilt * (0.75 + 0.25 * ((i % 3) - 1) * 0.4), range(rnd, -0.10, 0.10), 0);
    parent.add(it);
  }
}
...
function hookRow(parent, kind, { x0, x1, y, z, seed, rnd, variants, stepMul = 0.030 }, mats) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    it.position.set(x + range(rnd, -0.003, 0.003), y - 0.015 - h, z + 0.050);
...
function shelfTag(g, { len, y, text, sub, mats, rnd, seed, side = 1 }) {
  const t = grp('shelf-tag', { pos: [side > 0 ? -len * 0.30 : len * 0.30, y + 0.30, -0.004] });
  g.add(t);
  ...
  // 支持金物・反り・色褪せ
  for (const sx of [-0.18, 0.18]) t.add(mesh(box(0.014, 0.030, 0.016), mats.alu, { pos: [sx, -0.036, -0.010], name: 'tag-arm', cast: false }));
...
export function build(options = {}) {
  ...
  for (let i = 0; i < 3; i++) {
    dust(g, { w: 0.26, h: 0.14, pos: [range(rnd, -HW + 0.4, HW - 0.4), BASE_T + 0.012 + i * 0.02, -0.020], rot: [-PI / 2, 0, range(rnd, -0.14, 0.14)], kind: 'dirt', color: '#8b8375', opacity: 0.4, seed: seed + 11 + i, density: 1.2, spread: 0.02 });
  }
  ...
  const sec = [-HW + 0.05, -HW + 1.05, -0.85, -0.10, 0.72, HW - 0.06];   // 5 区画
  for (let t = 0; t < tiers; t++) {
    ...
    dust(T, { w: 1.90, h: 0.06, pos: [range(rnd, -1.0, 1.0), y + 0.0013, Z_BOARD], rot: [-PI / 2, 0, 0], kind: 'dirt', color: '#8b8375', opacity: 0.28, seed: seed + t * 17, density: 1.6, spread: 0.03 });
    ...
    for (let k = 0; k < nTag; k++) {
      ...
      const tg = stockItem(`tag:${k % 6 === 4 ? 'sale' : 'shelf'}`, () => P_TAG.build({ seed: seed + t * 97 + k * 13, variant: k % 6 === 4 ? 'sale' : 'shelf' }));
  ...
  /* ---------- 欠番（上段の 1 区画が品切れ）＋値札のみ ---------- */
  {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const tg = P_TAG.build({ seed: seed + 601 + i, variant: i ? 'new100' : 'pop' });
  ...
  /* ---------- 床に落ちた包装・埃（棚と壁の足元） ---------- */
  {
    const wrap = P_TRASH.build({ seed: seed + 811, variant: 'wrapper' });
    wrap.position.set(HW - 0.42, 0.055, 0.235);
    wrap.rotation.set(0, 1.15, 0);
    wrap.name = 'fallen-wrapper';
    g.add(wrap);
    const crush = P_TRASH.build({ seed: seed + 823, variant: 'can' });
