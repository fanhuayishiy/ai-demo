...
import {
  grp, mesh, box, rbox, cyl, tor, lathe, circ, tubeOf, catenary, grill,
...
import { build as breadSet } from '../products/bread-set.js';
import { build as tissuePack } from '../products/tissue-pack.js';
import { build as dailyGoods } from '../products/daily-goods.js';
...
export function build(options = {}) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    bk.position.set(range(rnd, -0.008, 0.008), i * 0.049, range(rnd, -0.008, 0.008));
  ...
  const folded = dailyGoods({ seed: seed + 161, variant: 'umbrella' });
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (i === 2) {
      const br = breadSet({ seed: seed + 171 + i, variant: 'melonpan' });
  ...
  for (let i = 0; i < 2; i++) {
    const tp = tissuePack({ seed: seed + 181 + i * 9, variant: i ? 'wet' : 'tissue' });
  ...
  for (let i = 0; i < 2; i++) {
    const l = noHull(mesh(new THREE.CircleGeometry(0.018 + i * 0.004, 9), leafMat, {
