...
import {
  grp, mesh, box, rbox, cyl, grill, finish, rand, range, row, weather, decal, stockItem,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX } from '../../core/textures.js';
import { PAL } from '../../core/palette.js';

import * as P_BENTO from '../products/bento-box.js';
import * as P_ONI from '../products/onigiri.js';
import * as P_SUSHI from '../products/sushi-tray.js';
import * as P_DELI from '../products/deli-tray.js';
  ...
  for (const seg of plan) {
    ...
    for (let r = 0; r < (seg.rows || 2); r++) {
      ...
      for (let i = 0; i < count; i++) {
        const x = xs0 + i * pitch + (r ? pitch * 0.5 + range(rnd, -0.012, 0.012) : range(rnd, -0.003, 0.003));
        ...
        const push = r === 0 ? range(rnd, -0.004, 0.011) : 0;
        it.position.set(x, y, z + push);
        it.rotation.y = r === 0 ? range(rnd, -0.06, 0.06) : range(rnd, -0.30, 0.30);
...
function emptyTray(parent, { x, y, z, zf, w, seed, rnd }, mats) {
  ...
  const tg = P_TAG.build({ seed: seed + 9, variant: 'pop' });
...
export function build(options = {}) {
  ...
  const mats = {
    ...
    glass: MAT.glassLite({ color: '#e2f1f4', opacity: 0.2, spec: 1, specPower: 230, specCut: 0.05, sheen: 0.4, rim: 0.5, steps: 2, side: THREE.DoubleSide }),
    curtain: MAT.glassLite({ color: '#eaf6f8', opacity: 0.13, spec: 0.9, specPower: 200, sheen: 0.3, steps: 2, side: THREE.DoubleSide }),
  ...
  for (let t = 0; t < tiers; t++) {
    ...
    for (let k = 0; k < nTag; k++) {
      ...
      const tg = stockItem(`tag:${(k + t) % 4 === 1 ? 'new100' : 'shelf'}`, () => P_TAG.build({ seed: seed + t * 61 + k * 17, variant: (k + t) % 4 === 1 ? 'new100' : 'shelf' }));
  ...
  /* ==================================================== ④前傾ガラス＋冷気カーテン */
  {
    ...
    gp.add(mesh(rbox(len - 2 * WALL - 0.2, 0.06, 0.005, 0.02, 2), mats.frost, { pos: [0.2, Y_BOT + 0.05, cz + (Z_BOT - Z_TOP) * 0.45], rotation: [lean, 0, 0], name: 'glass-frost', cast: false }));
  }
