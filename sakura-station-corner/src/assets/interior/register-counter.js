...
import {
  grp, mesh, box, rbox, cyl, sph, tor, plane, tubeOf, catenary,
  finish, rand, range, weather, decal,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, memo, shade, rr, jpText, speckle, blotches } from '../../core/textures.js';
...
export function build(options = {}) {
  ...
  /* =============== 3. 金庫トレイ（小銭・紙幣） =============== */
  const drawer = grp('cash-drawer', { pos: [-0.46, 0.62, -0.30] });
  g.add(drawer);
  ...
  /* =============== 4. レジ袋ラック（天板下吊り・袋の束） =============== */
  const bagRack = grp('bag-rack', { pos: [0.62, 0, -0.40] });
  g.add(bagRack);
  ...
  blade.add(mesh(box(0.13, 0.006, 0.016), steelDk, { pos: [0, -0.006, 0.002] }));
  bagRack.add(blade);
  ...
  decal(div, { map: fingerprintTex(seed + 5), w: 0.16, h: 0.08, pos: [-0.08, 0.05, -0.0055], rot: [0, Math.PI, 0], opacity: 0.5 });
  g.add(div);
  ...
  for (let t2 = 0; t2 < 2; t2++) {
    for (let i = 0; i < 4; i++) {
      ...
      const b = candyBar({ seed: seed + 41 + i * 13 + t2 * 7, variant: v });
