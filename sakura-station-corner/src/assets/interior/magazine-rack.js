...
import {
  grp, mesh, box, rbox, cyl, tor, plane, tubeOf, catenary, finish, rand, range, weather, decal,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, memo, rr, jpText, speckle } from '../../core/textures.js';
import { PAL } from '../../core/palette.js';
import { build as magazineItem } from '../products/magazine-item.js';
import { build as candyBar } from '../products/candy-bar.js';
...
export function build(options = {}) {
  ...
  tiers.forEach((t, ti) => {
    ...
    if (t.mode === 'face') {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const mg = magazineItem({ seed: seed + 200 + i * 17 + ti * 41, variant: v });
        ...
        if (i % 3 === 0) {   // 後ろに 2 列目（少し高く覗く）
          const b = magazineItem({ seed: seed + 300 + i * 23 + ti * 7, variant: faceVariants[(i + 3) % faceVariants.length] });
      ...
      spineSet.forEach(([v, cnt], gi) => {
        for (let k = 0; k < cnt; k++) {
          const one = magazineItem({ seed: seed + 400 + gi * 61 + k * 13, variant: v });
      ...
      const pull = magazineItem({ seed: seed + 477, variant: 'women' });
  ...
  g.add(news);
  news.add(mesh(rbox(0.42, 0.014, 0.3, 0.004, 2), alu, { pos: [0, -0.007, 0] }));
  news.add(mesh(box(0.42, 0.04, 0.012), aluDk, { pos: [0, 0.02, 0.15] }));
  for (let i = 0; i < 3; i++) {
    const np = magazineItem({ seed: seed + 511 + i * 19, variant: 'newspaper', pack: i === 2 ? 1 : 2 });
  ...
  mTier.forEach((y, i) => {
    ...
    if (i < 2) {
      ...
      let bx = -mL / 2 + 0.05;
      for (let k = 0; k < 11; k++) {
        const one = magazineItem({ seed: seed + 611 + i * 97 + k * 11, variant: ['comic', 'comic', 'weekly'][k % 3] });
      ...
      for (let k = 0; k < 5; k++) {
        const cb = candyBar({ seed: seed + 700 + k * 29, variant: ['gum', 'mint', 'caramel'][k % 3] });
  ...
  /* ---------- 5. 落ちた雑誌 1 冊・床の埃 ---------- */
  const fallen = magazineItem({ seed: seed + 823, variant: 'weekly' });
