...
import { grp, mesh, box, rbox, cyl, tor, finish, rand, range, weather, inst } from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  for (let i = 0; i < strips; i++) {
    ...
    unit.add(mesh(box(0.26, 0.02, 4.5), tubeMat, { pos: [0, -0.008, 0], name: 'tube', cast: false }));
    ...
    for (const dz of [-1.4, 1.4]) {
      const l = new THREE.PointLight(PAL.fluorescent, 2.1, 7.2, 2);
  ...
  // 点検口（枠・蝶番・取っ手）
  const hatch = grp('hatch', { pos: [3.1, CEIL - 0.002, -1.0] });
