...
import {
  grp, mesh, box, rbox, cyl, tor, grill, finish, rand, range, row, weather, decal, pipe, stockItem,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX } from '../../core/textures.js';
import { PAL } from '../../core/palette.js';

import * as P_ICE from '../products/ice-cream.js';
import * as P_TAG from '../products/price-tag.js';
...
const PI = Math.PI;
const ICE = (P_ICE.meta && P_ICE.meta.real) || [0.065, 0.09, 0.065];
const ICE_V = (P_ICE.meta && P_ICE.meta.variants) || ['bar', 'cup', 'parfait', 'mochi', 'popsicle', 'twin'];
...
function fillBasket(parent, { y, iw, seed, rnd, back }) {
  ...
  for (let r = 0; r < 2; r++) {
    ...
    for (let i = 0; i < count; i++) {
      const x = xs0 + i * pitch + (r ? pitch * 0.5 + range(rnd, -0.011, 0.011) : range(rnd, -0.002, 0.002));
      const it = stockItem(`ice:${ICE_V[(i + r * 2) % ICE_V.length]}`, () => P_ICE.build({ seed: seed + r * 211 + i * 31, variant: ICE_V[(i + r * 2) % ICE_V.length] }));
  ...
  const tg = P_TAG.build({ seed: seed + 5, variant: 'shelf' });
...
export function build(options = {}) {
  ...
  for (let c = 0; c < doors; c++) {
    ...
    g.add(dg);
    dg.rotation.y = range(rnd, -0.005, 0.005);
    ...
    for (let k = 0; k < 3; k++) {
      ...
      dg.add(mesh(box(0.024, 0.038, 0.018), mats.steel, { pos: [-doorW / 2 + 0.004, hy, -0.010], name: 'hinge', cast: false }));
      dg.add(mesh(cyl(0.005, 0.005, 0.042, 8), mats.chrome, { pos: [-doorW / 2 + 0.004, hy, -0.010], name: 'hinge-pin', cast: false }));
    }
    // 把手（縦バー・開閉で片側だけ太さが違う）
    ...
    dg.add(mesh(rbox(doorW - 0.05, 0.13, 0.006, 0.03, 2), mats.frost, { pos: [0, -doorH / 2 + 0.075, -0.006], name: 'glass-frost', cast: false }));
    for (let i = 0; i < 10; i++) {
  ...
  /* ==================================================== ④上箱（広告・蛍光灯） */
  {
    ...
    tb.add(mesh(box(w + 0.010, 0.012, D - 0.03), mats.aluDim, { pos: [0, h + 0.006, -0.005], name: 'crown' }));
  ...
  /* ==================================================== ⑤下面（脚・コンプレッサ・ドレン） */
  {
    ...
    // 機械室の膨らみ（背面）
    bot.add(mesh(rbox(0.42, 0.24, 0.16, 0.03, 3), MAT.metalPaint('#7e8388', { worn: 0.85, repeat: 2 }), { pos: [-0.10, 0.20, -D / 2 + 0.02], name: 'compressor' }));
    bot.add(pipe([[-0.10, 0.32, -D / 2 + 0.02], [-0.02, 0.44, -D / 2 - 0.01], [0.16, 0.40, -D / 2 + 0.03], [0.26, 0.30, -D / 2 + 0.01]],
      0.010, MAT.metal('#bda06c', { worn: 0.8 }), { seg: 16, radial: 7, name: 'pipe' }));
    grill(bot, { w: 0.34, h: 0.16, nx: 2, ny: 8, bar: 0.007, mat: mats.aluDim, pos: [-0.10, 0.20, -D / 2 - 0.058], rot: [0, PI, 0] });
    // ドレンパン・ホース・水跡
    ...
    dust(bot, { w: 0.4, h: 0.2, pos: [-0.10, 0.10, -D / 2 - 0.06], rot: [0, PI, 0], kind: 'rust', color: '#8a5236', opacity: 0.42, seed: seed + 62, density: 1.6, spread: 0.05 });
    // 銘板
  ...
  // 前面下部「要冷凍」大表示（框の下・客側から見える）
  {
    ...
    g.add(b);
    b.add(mesh(rbox(0.44, 0.080, 0.008, 0.005, 2), mats.sign, { pos: [-0.10, 0.098, D / 2 + 0.003], name: 'freeze-mark' }));
    dust(b, { w: 0.4, h: 0.06, pos: [-0.10, 0.098, D / 2 + 0.009], kind: 'scratch', color: '#f2f6f2', opacity: 0.3, seed: seed + 71, density: 1.5, spread: 0.02 });
  }
