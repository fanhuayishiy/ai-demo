...
import {
  grp, mesh, box, rbox, plane, finish, rand, range, weather, decal, shadowBlob,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const runA = tape(route, { x: 0, z: 0.86, len: 1.66, dir: 'z', mat: tapeBlue });          // 南北主推動線
  ...
  const runB = tape(route, { x: 1.25, z: 0.03, len: 2.5, dir: 'x', mat: tapeBlue });         // 東（レジ前）へ
  const runC = tape(route, { x: 0.0, z: -0.72, len: 1.5, dir: 'z', mat: tapeGreen });        // 北（後場・弁当）への枝
  ...
  const runD = tape(route, { x: -0.95, z: 0.52, len: 1.7, dir: 'x', mat: tapeBlue });        // 西（雑誌・文庫）への枝
  ...
  weather(route, { w: 1.2, h: 0.1, pos: [-1.0, 0.0014 + TAPE_T, 0.52], rot: [-Math.PI / 2, 0, 0], kind: 'scratch', color: '#cfc9b6', opacity: 0.3, seed: seed + 7, spread: 0.003 });
  ...
  /* ---------- 6. 消火器設置表示 ---------- */
  const ext = grp('extinguisher', { pos: [-3.0, 0, -0.62] });
  g.add(ext);
  ...
  /* ---------- 7. 春の床ポスター ---------- */
  const poster = grp('floor-poster', { pos: [-1.0, 0, 1.16], rot: [0, 0.06, 0] });
  g.add(poster);
  ...
  curlX.add(noHull(mesh(box(0.16, 0.002, 0.6), MAT.paint('#efe8d6', { spec: 0.14, side: THREE.DoubleSide }), { pos: [0.08, 0.001, 0], cast: false })));
  poster.add(curlX);
  const curlZ = grp('curl-z', { pos: [-0.1, 0.0032, 0.3], rot: [-0.18, 0, 0] });
  curlZ.add(noHull(mesh(box(0.66, 0.002, 0.14), MAT.paint('#efe8d6', { spec: 0.14, side: THREE.DoubleSide }), { pos: [0, 0.001, 0.07], cast: false })));
  poster.add(curlZ);
  ...
  decal(g, { map: glueTex(seed + 5), w: 0.5, h: 0.1, pos: [-2.0, 0.0016, -0.28], rot: [-Math.PI / 2, 0, -0.5], opacity: 0.5, order: 3 });
