...
import {
  grp, mesh, box, rbox, cyl, sph, lathe, circ, plane, tubeOf, coil, catenary, grill,
...
import { build as coffeeCup } from '../products/coffee-cup.js';
...
export function build(options = {}) {
  ...
  wand.add(mesh(box(0.024, 0.03, 0.024), MAT.plastic('#39424a', { spec: 0.3 }), { pos: [0, -0.008, 0.01] }));
  mc.add(wand);
  ...
  stackSpec.forEach((sp, i) => {
    const cup = coffeeCup({ seed: seed + 61 + i * 7, variant: sp.s, stack: sp.n, lid: sp.lid, sleeve: false, fill: false });
  ...
  for (let i = 0; i < 6; i++) {
    lidStack.add(noHull(mesh(cyl(0.041, 0.039, 0.006, 16), MAT.plastic(['#2f3236', '#8c2f2a', '#2f4d70'][i % 3], { spec: 0.36, worn: 0.4 }), { pos: [range(rnd, -0.002, 0.002), i * 0.0065, range(rnd, -0.002, 0.002)], rot: [0, range(rnd, -0.3, 0.3), 0] })));
  }
  gan.add(lidStack);
  ...
  g.add(tray);
  const taken = coffeeCup({ seed: seed + 91, variant: 'm', lid: 'black', sleeve: true, straw: true, fill: true });
  taken.position.set(-0.24, 0.754, 0.21);
  taken.rotation.y = 0.4;
  g.add(taken);
  // 飲みかけで置いたままの 2 つ目（蓋なし・液面あり）
  const spill = coffeeCup({ seed: seed + 97, variant: 's', lid: false, sleeve: false, fill: true });
  ...
  ceilPatch.add(noHull(mesh(box(0.66, 0.006, 0.02), MAT.metal('#c8c2b2', { worn: 0.5 }), { pos: [0, -0.006, 0.32], cast: false })));
  ceilPatch.add(noHull(mesh(box(0.66, 0.006, 0.02), MAT.metal('#c8c2b2', { worn: 0.5 }), { pos: [0, -0.006, -0.32], cast: false })));
  ...
  decal(ceilPatch, { map: coffeeStainTex(seed + 3), w: 0.4, h: 0.4, pos: [0.04, -0.0075, -0.02], rot: [Math.PI / 2, 0, 0], opacity: 0.5, order: 2 });
