...
import {
  grp, mesh, box, cyl, rbox, tor, lathe, sph, hoop, finish, rand,
...
export const DEFAULT_OPTIONS = { kind: 'pipe', h: 0.7, seed: 63 };
...
export function build(options = {}) {
  ...
  if (kind === 'concrete') {
    ...
    g.add(post);
    // チェーン通し穴（上端）
    g.add(mesh(cyl(0.016, 0.016, 0.16, 12, true), MAT.paint('#8f8a80', { steps: 2, side: THREE.DoubleSide }), { pos: [0, h - 0.1, 0], rot: [0, 0, 90 * D2R] }));
    g.add(mesh(tor(0.016, 0.005, 6, 14), MAT.concrete({ base: PAL.concreteDark }), { pos: [0.078, h - 0.1, 0], rot: [0, 90 * D2R, 0] }));
    // 黄色の帯（打ち込み塗布）＋ 剥がれ
    g.add(mesh(cyl(0.075, 0.079, 0.1, 8, true), MAT.paint(PAL.markingYellow, { steps: 2, spec: 0.14, side: THREE.DoubleSide, shadowAmt: 0.8 }), { pos: [0, h * 0.62, 0] }));
    ...
    pipeG.add(mesh(cyl(R + 0.006, R + 0.006, 0.085, 18, true), MAT.paint('#f2eee2', {
      map: tape, steps: 2, spec: 0.3, specPower: 60, side: THREE.DoubleSide, shadowAmt: 0.66,
    }), { pos: [0, bandY, 0] }));
    const peel = mesh(plane(0.05, 0.04), MAT.paint('#e6e0d0', { side: THREE.DoubleSide, steps: 2, spec: 0.2 }), {
    ...
    pipeG.add(mesh(cyl(R + 0.004, R + 0.004, 0.05, 18, true), MAT.paint(PAL.markingYellow, { steps: 2, side: THREE.DoubleSide, shadowAmt: 0.8, map: tape }), { pos: [0, bodyH * 0.24 + 0.04, 0] }));
