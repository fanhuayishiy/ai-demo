...
import {
  grp, mesh, box, cyl, rbox, lathe, tubeOf, cone, sph, finish, rand, range,
  weather, decal, shadowBlob, grill, tor,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const zincPost = MAT.metalPaint(PAL.lampGreen, { worn: 0.75, repeat: 3 });
  ...
  weather(tray, { w: 0.16, h: 0.07, pos: [-0.10, 0.0695, -0.018], rot: [-Math.PI / 2, 0, 0], kind: 'moss', color: PAL.moss, opacity: 0.45, seed: seed + 12, density: 1.2, spread: 0.006 });
  ...
  frame.add(back);
  back.add(mesh(box(W - 0.034, 0.33, 0.011), zinc, { pos: [0, 0.285, -(D / 2 - 0.011)] }));
  back.add(mesh(box(W - 0.034, 0.013, 0.013), steel, { pos: [0, 0.452, -(D / 2 - 0.011)] }));
  // 雨だれ・退色・錆（経年 3・4）／内側の黒ズミ
  weather(back, { w: W - 0.12, h: 0.26, pos: [0, 0.31, -(D / 2 - 0.004)], rot: [0, Math.PI, 0], kind: 'rust', color: '#8a6a4a', opacity: 0.42, seed: seed + 21, density: 1.3, spread: 0.012 });
  weather(back, { w: 0.14, h: 0.10, pos: [-0.13, 0.15, -(D / 2 - 0.003)], rot: [0, Math.PI, 0], kind: 'moss', color: PAL.moss, opacity: 0.4, seed: seed + 22, spread: 0.01 });
  weather(back, { w: 0.28, h: 0.14, pos: [0.02, 0.20, -(D / 2 - 0.0195)], kind: 'dirt', color: '#3d3a34', opacity: 0.55, seed: seed + 23, density: 1.7, spread: 0.01 });
  ...
  function vinylLong({ tint = vinyl, bent = false, brokenRib = -1, grip = gripBlack, ribs = 8, tag = '' }) {
    ...
    u.add(noOutline(mesh(bundleGeo(bLen * 0.985, 0.0195, 0.0212, 0.0132), MAT.glassLite({ color: '#cfe3e6', opacity: 0.22, side: THREE.BackSide }), { pos: [0, yTip + 0.016, 0], cast: false, receive: false })));
    // 親骨
    ...
    if (bent) {
      ...
      u.add(noOutline(mesh(tubeOf([
        ...
        [0.0035, yTop + 0.062, 0.022], [-0.0045, yTop + 0.0655, 0.0335],
      ], 0.0086, 14, 8), grip, { cast: false })));
    } else {
      u.add(noOutline(mesh(tubeOf([
        [0, yTop - 0.006, 0], [0, yTop + 0.022, 0], [0.0015, yTop + 0.042, 0.004], [0.004, yTop + 0.0575, 0.0155],
        [0.002, yTop + 0.0625, 0.0295], [-0.002, yTop + 0.053, 0.0405], [-0.003, yTop + 0.0385, 0.0435],
  ...
  function kidUmbrella() {
    ...
    u.add(noOutline(mesh(tubeOf([
      [0, yTop - 0.004, 0], [0, yTop + 0.028, 0], [0.004, yTop + 0.052, 0.006], [0.002, yTop + 0.066, 0.022], [-0.004, yTop + 0.062, 0.034],
  ...
  for (let i = 0; i < count; i++) {
    ...
    u.position.set(xOf(i), 0, range(rnd, -0.004, 0.004));
  ...
  weather(frame, { w: 0.10, h: 0.26, pos: [-(W / 2 + 0.001), 0.20, 0.02], rot: [0, -Math.PI / 2, 0], kind: 'rust', color: PAL.rust, opacity: 0.5, seed: seed + 51, density: 1.5, spread: 0.012 });   // 経年 7：側板錆
