...
import {
  grp, mesh, box, cyl, rbox, sph, lathe, tubeOf, hoop, finish, rand,
  weather, shadowBlob,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 31, height: 4.6, kind: 'cobra' };
...
export function build(options = {}) {
  ...
  // 根元の土縁（舗装との段差埋め）
  const grind = mesh(new THREE.RingGeometry(0.2, 0.34, 22, 1), MAT.asphalt({ tone: 1, repeat: 6 }), { pos: [0, 0.004, 0], rot: [-90 * D2R, 0, 0], cast: false });
  ...
  if (kind === 'globe') {
    ...
    head.add(bulb);
    head.add(mesh(sph(gr * 0.9, 18, 12, 0, Math.PI * 2, 0, 0.72 * D2R), MAT.paint('#5f5a4e', { side: THREE.BackSide, steps: 2, shadowAmt: 1, transparent: true, opacity: 0.62 }), { pos: [0, gy, 0], cast: false, receive: false }));
    // 天笠と口金
    ...
    const lens = mesh(lathe([[0, -0.052], [0.13, -0.046], [0.2, -0.024], [0.206, -0.014], [0.13, -0.02], [0, -0.028]], 22),
      MAT.lampShade({ color: '#fff6e2', emissive: '#ffdda6', emissiveIntensity: 0.72, side: THREE.DoubleSide }), { name: 'cobra-lens', cast: false });
  ...
  g.add(mesh(cyl(0.058, 0.07, height * 0.3, 18, true), MAT.metalPaint(shadeHex(PAL.lampGreen, 1.28), { worn: 0.4, base: '#ffffff', transparent: true, opacity: 0.75, side: THREE.DoubleSide }), { pos: [0, height * 0.34, 0] }));
...
function capsuleLike(r, len) {
  return new THREE.CapsuleGeometry(r, len, 3, 7);
}
