...
import {
  grp, mesh, box, cyl, rbox, sph, cone, tor, finish, rand, range,
...
export function build(options = {}) {
  ...
  const innerPlastic = MAT.paint('#4b4642', { steps: 2, shadowAmt: 1.0, sheen: 0.02 });
  ...
  body.add(mesh(box(BW, bh, wall), redBody, { pos: [0, yb, -(BD / 2 - wall / 2)] }));           // 後
  body.add(mesh(box(wall, bh, BD - wall * 2), redBody, { pos: [-(BW / 2 - wall / 2), yb, 0] })); // 左
  ...
  const innerFaces = [
    ...
    [[-0.03, 0.30, -(BD / 2 - wall - 0.003)], [0, 0, 0]],
    [[-(BW / 2 - wall - 0.003), 0.28, 0.02], [0, Math.PI / 2, 0]],
  ...
  weather(body, { w: 0.10, h: 0.11, pos: [-(BW / 2 + 0.0025), 0.19, 0.05], rot: [0, -Math.PI / 2, 0], kind: 'moss', color: PAL.moss, opacity: 0.5, seed: seed + 22, spread: 0.006 });
  // 経年 5：貼紙残り（剥がした分別ラベル）
  ...
  for (const [x, z, tilt] of [[-0.068, 0.038, 0.13], [0.062, -0.038, -0.10], [-0.015, -0.005, 0.05]]) {
  ...
  bag.add(fold);
  const rimX = (BW + 0.014) / 2;                   // リムの外側 x = 0.227
  fold.add(mesh(rbox(0.080, 0.011, 0.195, 0.004, 2), bagMat, { pos: [-(rimX - 0.036), yRim + 0.0025, 0.012] }));      // リム天に載る折り返し（蓋と 3mm 间隙）
  fold.add(mesh(rbox(0.016, 0.020, 0.195, 0.006, 2), bagMat, { pos: [-(rimX - 0.002), yRim - 0.008, 0.012], rot: [0, 0, -4 * D2R] }));  // 縁の折れ曲がり
  fold.add(mesh(rbox(0.013, 0.285, 0.172, 0.005, 2), bagMat, { pos: [-(BW / 2 + 0.006), 0.452, 0.014], rot: [0, 0, 1.6 * D2R] }));      // 外側へ垂れる袋
  fold.add(mesh(rbox(0.055, 0.020, 0.150, 0.008, 2), bagMat, { pos: [-0.150, bagTop - 0.02, 0.014], rot: [0, 0, 9 * D2R] }));           // 内側の折り込み
  // 結び目（袋口を括った捩れ）
  const knot = grp('knot');
  knot.position.set(-(BW / 2 + 0.010), 0.308, 0.018);
  ...
  weather(bag, { w: 0.06, h: 0.16, pos: [-(BW / 2 + 0.0125), 0.42, 0.02], rot: [0, -Math.PI / 2, 0], kind: 'dirt', color: '#4a4640', opacity: 0.5, seed: seed + 32, density: 1.4, spread: 0.005 });   // 袋口の泥はね

  /* ================= 蓋（ヒンジは後面、反りあり・裏面あり） ================= */
  const lid = grp('lid', { pos: [0, yBody1 + 0.033, -(BD / 2 + 0.006)] });
  g.add(lid);
  ...
  corner.position.set(-(LW / 2) + 0.075, 0.008, LD * 0.72);
  corner.rotation.z = 2.6 * D2R;
  lid.add(corner);
  corner.add(mesh(rbox(0.15, 0.019, 0.11, 0.006, 2), orangeLid, { pos: [0, 0.002, 0] }));
  // 蓋の裏面（補強リブ格子＝開けたとき必ず見える）
  for (const z of [0.10, 0.20, 0.30]) lid.add(mesh(box(LW - 0.03, 0.014, 0.016), orangeLid, { pos: [0, -0.006, z] }));
  lid.add(mesh(box(0.016, 0.014, LD - 0.02), orangeLid, { pos: [0, -0.006, LD * 0.48] }));
  // 前縁の掴み手
  lid.add(mesh(rbox(0.19, 0.026, 0.030, 0.008, 2), MAT.hardPlastic(PAL.storeBand3, { sat: 0.7, tint: '#f2ddd2' }), { pos: [0, -0.004, LD + 0.0] }));
