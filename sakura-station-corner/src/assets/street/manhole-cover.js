...
import {
  grp, mesh, box, cyl, rbox, tor, hoop, finish, rand, range,
  weather, decal, plane, circ, inst,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { kind: 'manhole', size: 0.6, seed: 71 };
...
function leaves(parent, rnd, n, spread, y, x0 = 0, z0 = 0) {
  const geo = new THREE.PlaneGeometry(0.1, 0.075);
...
export function build(options = {}) {
  ...
  if (kind === 'manhole') {
    ...
    g.add(mesh(new THREE.RingGeometry(R * 1.02, R * 1.42, 30, 1), asphalt, { pos: [0, 0.004, 0], rot: [-90 * D2R, 0, 0], cast: false }));
    // 枠（鋳鉄フレーム：蓋より 2 mm 高い＝段差）
    const frame = mesh(new THREE.RingGeometry(R * 0.99, R * 1.06, 30, 1), iron({ repeat: 2 }), { pos: [0, 0.026, 0], rot: [-90 * D2R, 0, 0] });
    g.add(frame);
    ...
    const puddle = mesh(new THREE.CircleGeometry(R * 0.42, 22), MAT.water({ opacity: 0.42, scroll: [0.004, 0.003] }), { pos: [R * 0.18, coverY + 0.0035, -R * 0.2], rot: [-90 * D2R, 0, 0], cast: false });
    ...
    // 蓋のガタ（段付き座面）
    g.add(mesh(new THREE.RingGeometry(R * 0.9, R * 0.99, 26, 1), MAT.paint('#4f5455', { steps: 2, shadowAmt: 1 }), { pos: [0, coverY - 0.001, 0], rot: [-90 * D2R, 0, 0], cast: false }));
  }
  ...
  if (kind === 'grate') {
    ...
    g.add(mesh(rbox(W, 0.055, D, 0.008, 2), iron({ repeat: 2 }), { pos: [0, -0.005, 0], name: 'grate-frame' }));
