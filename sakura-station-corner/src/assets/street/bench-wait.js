...
import { grp, mesh, box, cyl, rbox, tor, finish, rand, range, weather, decal, shadowBlob, hoop } from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  for (let i = 0; i < SLAT; i++) {
    ...
    const slat = mesh(
      ...
      { pos: [range(rnd, -0.004, 0.004), seatY - sag, z], name: `seat-slat-${i}` },
    );
    slat.rotation.z = range(rnd, -0.004, 0.004);
    g.add(slat);
    // 留め金（2 箇所）
  ...
  for (let i = 0; i < 4; i++) {
    g.add(
      mesh(rbox(0.032, 0.004, 0.026, 0.008, 1), MAT.petal({ map: TEX.petal({ mode: 'single', tone: i % 2 }), alphaTest: 0.4, side: THREE.DoubleSide }), {
