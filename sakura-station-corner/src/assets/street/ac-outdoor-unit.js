...
import {
  grp, mesh, box, cyl, rbox, sph, tor, lathe, tubeOf, cone, hoop, finish, rand, range, row,
  weather, decal, shadowBlob, grill, coil,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const alu = MAT.metal('#c4c9cc', { spec: 0.6, repeat: 4 });
  ...
  for (const sx of [-1, 1]) {
    for (let k = 0; k < 6; k++) {
      ...
      caseG.add(mesh(box(0.006, 0.030, CD - 0.09), dark, { pos: [sx * (CW / 2 + 0.001), y, -0.008], rot: [0, 0, 0] }));
      caseG.add(mesh(box(0.010, 0.006, CD - 0.09), casePaint, { pos: [sx * (CW / 2 + 0.002), y + 0.017, -0.008], rot: [sx * 0.10, 0, 0] }));
    }
  }
  ...
  for (let k = 0; k < 4; k++) {
    ...
    blade.quaternion.copy(q).multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), 0.42));
  ...
  for (let k = 0; k < 3; k++) {
    ...
    arm.quaternion.copy(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), -a));
    fan.add(arm);
  }
  ...
  for (let k = 0; k < 8; k++) {
    ...
    sp.quaternion.copy(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), a));
    guard.add(sp);
  }
  ...
  if (wallMount) {
    ...
    mount.add(sl);
    sl.add(mesh(cyl(0.0245, 0.0245, 0.062, 14, true), MAT.plastic('#dcd7c9', { steps: 2, side: THREE.DoubleSide })));
  ...
  g.add(dirt);
  dirt.add(mesh(rbox(CW - 0.07, 0.006, CD - 0.08, 0.003, 2), MAT.paint('#a9a294', { steps: 3, map: TEX.paper({ base: '#b3ac9e' }).map }), { pos: [0.01, yTop + 0.0145, -0.006], rot: [0, 0.02, 0] }));   // 上面の埃の層
  for (let i = 0; i < 9; i++) {
  ...
  dirt.add(noOut(mesh(tubeOf([[-0.20, yTop + 0.018, 0.04], [-0.13, yTop + 0.024, 0.06], [-0.05, yTop + 0.020, 0.03]], 0.0035, 8, 5), MAT.bark({ base: PAL.trunk, repeat: 2 }))));   // 小枝
  // 松葉状のゴミ
  for (let i = 0; i < 4; i++) {
  ...
  weather(caseG, { w: 0.20, h: 0.14, pos: [0, yc - 0.10, -(fz + 0.0025)], rot: [0, Math.PI, 0], kind: 'rust', color: '#8a5236', opacity: 0.55, seed: seed + 26, density: 1.6, spread: 0.006 });   // 背面錆
  weather(caseG, { w: 0.10, h: 0.18, pos: [-(CW / 2 + 0.0025), 0.16, 0.03], rot: [0, -Math.PI / 2, 0], kind: 'rust', color: '#7d5a3a', opacity: 0.45, seed: seed + 27, spread: 0.005 });
  // ドレンの水跡（緑錆・苔の流筋）
  for (let i = 0; i < 4; i++) {
    weather(caseG, { w: 0.026, h: 0.20, pos: [-0.10 + i * 0.07, yBot + 0.13, fz - 0.0010], kind: 'rust', color: '#7f9578', opacity: 0.4, seed: seed + 31 + i, density: 0.7, spread: 0.004 });
  }
  // 塗装の黄変・退色（上半分）
  ...
  caseG.add(noOut(mesh(sph(0.034, 10, 7), casePaint, { pos: [-0.20, yTop - 0.14, fz - 0.004], scale: [1, 0.8, 0.24] })));   // へこみ（当たり跡）
  // 貼紙残り（設置年プレートの剥がれ）
