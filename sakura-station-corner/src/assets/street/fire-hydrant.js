...
import {
  grp, mesh, box, cyl, rbox, tor, lathe, hoop, sph, finish, rand,
  weather, decal, shadowBlob, plane,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 96 };
...
export function build(options = {}) {
  ...
  const ironMat = MAT.darkIron({ worn: 0.9 });
  ...
  /* ============================ 消火栓 ============================ */
  {
    ...
    hg.add(mesh(new THREE.RingGeometry(0.2, 0.235, 22, 1), MAT.marking(PAL.markingYellow), { pos: [0, 0.0665, 0], rot: [-90 * D2R, 0, 0], cast: false }));
    // 胴（テーパーする鋳物：下太・上細）
    const prof = [
  ...
  /* ============================ 防火水槽 標識柱 ============================ */
  {
    ...
    for (const [sz, ry] of [[0.009, 0], [-0.009, Math.PI]]) {
    ...
    sg.add(mesh(cyl(0.031, 0.033, 0.3, 12, true), MAT.metalPaint('#98a29d', { worn: 0.4, transparent: true, opacity: 0.8, side: THREE.DoubleSide }), { pos: [0, 0.86, 0] }));
    g.add(sg);
  ...
  /* ============================ 防火水槽のコンクリート蓋 ============================ */
  {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const lid = grp('lid', { pos: [lx, 0.055, i ? 0.012 : -0.006], rot: i ? [0.012, 0, -0.014] : [0, 0, 0.006] });
      ...
      for (const [hx, hz] of [[-0.2, 0.24], [0.2, -0.24]]) {
        lid.add(mesh(cyl(0.032, 0.032, 0.08, 12, true), MAT.paint('#4d5049', { steps: 2, side: THREE.DoubleSide, shadowAmt: 1 }), { pos: [hx, 0.002, hz] }));
