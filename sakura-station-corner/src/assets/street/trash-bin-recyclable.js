...
import {
  grp, mesh, box, cyl, rbox, sph, tor, lathe, finish, rand, range,
...
export function build(options = {}) {
  ...
  const cells = [                       // 左：かん／中：びん／右：ペット
    { x: -0.220, band: PAL.signBlue, name: 'かん', en: 'CANS', flap: 66 },        // かん口は蝶番が緩んで開きっぱなし
  ...
  weather(plinth, { w: 0.22, h: 0.05, pos: [0.20, 0.055, -(D / 2 + 0.026)], rot: [0, Math.PI, 0], kind: 'dirt', color: '#5c5347', opacity: 0.55, seed: seed + 4, spread: 0.005 });
  ...
  const innerFaces = [
    ...
    [[-0.05, 0.28, -(D / 2 - wall - 0.004)], [0, 0, 0], 0.42],
    [[-(W / 2 - wall - 0.004), 0.26, 0.03], [0, Math.PI / 2, 0], 0.28],
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    flap.add(mesh(rbox(fw, 0.010, fd, 0.003, 2), acrylic, { pos: [0, -0.005, fd / 2], cast: false, receive: false }));
    flap.add(mesh(box(fw, 0.014, 0.012), alu, { pos: [0, -0.005, fd] }));                                  // 前縁の重り
    flap.add(mesh(box(fw - 0.02, 0.012, 0.009), alu, { pos: [0, -0.005, fd * 0.52] }));                     // 補強
    ...
    // ヒンジの錆
    weather(flap, { w: 0.05, h: 0.03, pos: [0, -0.002, 0.006], kind: 'rust', color: PAL.rust, opacity: 0.6, seed: seed + 41 + i, spread: 0.004 });
  }
  ...
  for (const x of [-0.20, 0.20]) {
    net.add(mesh(rbox(0.048, 0.010, 0.040, 0.003, 2), alu, { pos: [x, yN + 0.002, -(nd / 2 + 0.014)] }));
    net.add(mesh(cyl(0.0065, 0.0065, 0.050, 8), steelDark, { pos: [x, yN + 0.002, -(nd / 2 + 0.024)], rot: [0, Math.PI / 2, 0] }));
  }
  ...
  weather(net, { w: 0.30, h: 0.06, pos: [-0.10, yN + 0.010, 0.02], rot: [-Math.PI / 2, 0, 0], kind: 'dirt', color: '#4b4f4a', opacity: 0.4, seed: seed + 51, density: 1.4, spread: 0.006 });
  ...
  for (const [x, z, tilt, col] of [[-0.268, 0.035, 0, '#c94f45'], [-0.228, 0.062, 0, '#cfd4d8'], [-0.192, 0.028, 5, '#3d76b4'], [-0.240, 0.085, -84, '#d8b04a']]) {
    const can = grp('can');
    can.position.set(x + range(rnd, -0.006, 0.006), tilt === 0 || Math.abs(tilt) < 10 ? 0.470 : 0.118, z + range(rnd, -0.006, 0.006));
    can.rotation.set(tilt * D2R, range(rnd, -30, 30) * D2R, range(rnd, -5, 5) * D2R);
    stuff.add(can);
    can.add(mesh(cyl(0.0325, 0.0305, 0.108, 16), MAT.canBody({ color: col, map: TEX.drinkLabel({ name: 'コーヒー', sub: 'BLACK', ml: '185g', a: col, b: '#f4eee2' }) }), { pos: [0, -0.020, 0] }));
    ...
    can.add(mesh(rbox(0.007, 0.0030, 0.009, 0.0012, 2), MAT.metal('#c8b6a4', { worn: 0.6, repeat: 8 }), { pos: [-0.008, 0.0358, 0.006], rot: [0, 0.9, 0] }));   // 僅かに錆びたツメ
  }
  // びん：3 本（首とキャップが顔を出す）
