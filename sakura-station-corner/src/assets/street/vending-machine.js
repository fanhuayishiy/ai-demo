...
import {
  ...
  row, grill, coil, pipe, decal, weather,
} from '../../core/kit.js';
...
function productTemplate(e) {
  ...
  if (e.kind === 'pet' || e.kind === 'petsmall') {
    ...
    p.add(noOut(mesh(cyl(0.0035, 0.0035, 0.062, 8), MAT.plastic(e.cap, { spec: 0.4 }), { pos: [0.012, H + 0.024, -0.006], rot: [8 * D2R, 0, 6 * D2R] })));
...
const Y_BODY_B = 0.088, Y_BODY_T = 1.470;        // 鋼板本体
const Y_TOP_T = 1.850;
const WIN = { x0: -0.530, x1: 0.100, y0: 0.302, y1: 1.450 };
const CAB = { x0: -0.548, x1: 0.118, z0: -0.330, z1: 0.300, y0: 0.306, y1: 1.462 };
...
export function build(options = {}) {
  ...
  const crumple = (x, y, z, ax, sgn, sc = 1, name = 'crumple') => {
    ...
    const dents = MAT.plastic(color, {
      map: TEX.metal({ base: color, worn: 0.8, repeat: 2 }).map, side: THREE.DoubleSide,
    ...
    const patt = [[0, 0, 0.058, 0.34], [0.046, 0.030, 0.034, 0.26], [-0.040, -0.024, 0.028, 0.22], [0.014, -0.046, 0.024, 0.30]];
  ...
  shellG.add(mesh(box(0.684, 0.050, 0.020), skin, { pos: [(-0.560 + 0.124) / 2, 1.466, 0.352], name: 'header-plate' }));
  ...
  decal(skirt, { map: TEX.wear({ kind: 'dirt', color: '#6b6153', seed: seed + 6, density: 1.2 }), w: 0.24, h: 0.20, pos: [-0.30, 0.16, 0.3660], opacity: 0.35 });
  // 落ち葉貼り付き跡
  ...
  for (let i = 0; i < 8; i++) seamG.add(screwHead(HW + 0.0005, 0.20 + i * 0.17, -0.30 + i * 0.02, 'x'));
  for (let i = 0; i < 8; i++) seamG.add(screwHead(-HW - 0.0005, 0.20 + i * 0.17, -0.30 + i * 0.02, 'xn'));
  // 溶接ビード（背面部）
  ...
  [-1, 1].forEach((sgn) => {
    ...
    side.add(crumple(sgn * (HW + 0.002), 1.16, -0.20, 'x', sgn, 0.62, 'dent-side2' + sgn));
    ...
    weather(side, { w: 0.28, h: 0.20, pos: [sgn * (HW + 0.010), 0.20, -0.10], rot: [0, sgn * PI / 2, 0], kind: 'rust', color: '#8a5236', opacity: 0.34, seed: seed + 11, density: 1.0, spread: 0.006 });
  });
  // 前面右下のぶつかり痕
  ...
  decal(shellG, { map: TEX.wear({ kind: 'dirt', color: '#f2ece0', seed: seed + 14, density: 0.35 }), w: 0.30, h: 0.22, pos: [-0.30, 1.8552, 0.16], rot: [-PI / 2, 0, 0.4], opacity: 0.6 });
  ...
  signG.add(mesh(box(0.262, 0.062, 0.016), tone(warm ? 'あたたか' : 'つめたい', warm ? '#f6d7cf' : '#dbeeff', warm ? '#a8382a' : '#1f5b8a'), { pos: [-0.280, 1.506, 0.384] }));
  ...
  for (const p of plan) {
    ...
    if (p.soldOut) {
      ...
      const yawJ = () => range(rnd, e.kind === 'pack' || e.kind === 'sqpet' ? -0.05 : -0.10, e.kind === 'pack' || e.kind === 'sqpet' ? 0.05 : 0.10);
  ...
  doorG.add(mesh(box(0.014, 0.190, 0.010), alu, { pos: [0.588, 0.876, -0.004] }));
  ...
  pkG.add(innerBox);
  innerBox.add(noOut(mesh(box(PKU.x1 - PKU.x0 - 0.010, 0.168, 0.014), deep, { pos: [-0.303, 0.198, -0.060] })));
  ...
  // 中の埃・落ち葉・砂
  innerBox.add(noOut(mesh(box(0.30, 0.004, 0.10), MAT.paint('#8b8071', { steps: 2, spec: 0.05, shadowAmt: 1, map: TEX.concrete({ base: '#8b8071', repeat: 3 }).map }), { pos: [-0.320, 0.122, 0.180] })));
  ...
  pkG.add(noOut(mesh(plane(0.07, 0.07), leafMat, { pos: [-0.360, 0.126, 0.190], rot: [-78 * D2R, 0.4, 0.6] })));
  // フラップ（開いた状態）＋パッキン劣化
  ...
  flapG.add(noOut(mesh(plane(0.200, 0.050), MAT.screen({ map: TEX.signboard({ text: 'おとりだし', sub: 'PUSH', bg: '#2a2e33', fg: '#ffd88a', size: 118, spacing: 0 }) }), { pos: [0, -0.100, 0.0125] })));
  ...
  weather(pkG, { w: 0.24, h: 0.06, pos: [-0.30, 0.100, 0.3665], kind: 'scratch', color: '#c9c2b4', opacity: 0.4, seed: seed + 33, density: 2.2, spread: 0.005 });
  ...
  g.add(backG);
  // 放熱グリル
  const grG = grp('grille', { pos: [-0.20, 0.42, -HD - 0.006] });
  backG.add(grG);
  grG.add(mesh(box(0.520, 0.340, 0.014), zinc, { pos: [0, 0, -0.004] }));
  for (let i = 0; i < 11; i++) grG.add(mesh(box(0.480, 0.014, 0.016), dark, { pos: [0, -0.150 + i * 0.030, 0.006], rot: [24 * D2R, 0, 0] }));
  ...
  compG.add(mesh(cyl(0.054, 0.054, 0.150, 16), MAT.darkIron({}), { pos: [0.02, 0.030, -0.002], rot: [0, 0, PI / 2], name: 'comp-hump' }));
  compG.add(mesh(cyl(0.044, 0.048, 0.018, 14), MAT.metal('#8d8f92', { worn: 0.8 }), { pos: [-0.068, 0.030, -0.002], rot: [0, 0, PI / 2] }));
  compG.add(mesh(box(0.060, 0.046, 0.028), MAT.plastic('#2f3336', { steps: 2 }), { pos: [-0.040, -0.088, -0.026] }));
  // 冷媒コイルと配管
  compG.add(mesh(coil(0.040, 0.150, 7, 12, 0.0055), MAT.metal('#b47a4c', { spec: 0.7, specPower: 120, worn: 0.6 }), { pos: [0.148, 0.020, -0.004], name: 'condenser' }));
  ...
  // 配線ダクト・電源コード・プラグ
  backG.add(mesh(box(0.050, 0.900, 0.040), MAT.hardPlastic('#c9c3b5', { repeat: 2 }), { pos: [-0.420, 0.90, -HD - 0.024] }));
  backG.add(pipe([[-0.42, 1.34, -HD - 0.040], [-0.36, 1.20, -HD - 0.056], [-0.24, 0.42, -HD - 0.052], [-0.10, 0.10, -HD - 0.044], [-0.02, 0.024, -HD - 0.038]], 0.0085, MAT.rubber('#33373b'), { seg: 26 }));
  ...
  weather(backG, { w: 0.50, h: 0.24, pos: [-0.20, 0.16, -HD - 0.006], rot: [0, PI, 0], kind: 'dirt', color: '#5d5348', opacity: 0.5, seed: seed + 37, density: 1.8, spread: 0.006 });
  ...
  groundG.add(noOut(mesh(box(0.26, 0.004, 0.09), MAT.paint('#7f7668', { steps: 2, spec: 0.06, shadowAmt: 1 }), { pos: [0.04, 0.0425, -0.290], name: 'drain-trace' })));
  // 周囲の落ち葉
  for (let i = 0; i < 4; i++) {
    groundG.add(noOut(mesh(plane(0.075, 0.075), leafMat, { pos: [range(rnd, -0.50, 0.50), 0.0452, range(rnd, 0.02, 0.38)], rot: [-PI / 2, 0, range(rnd, 0, 6.28)] })));
  }
  ...
  canG.add(noOut(mesh(sph(0.017, 10, 7, 0, PI * 2, 0, PI / 2.1), MAT.plastic('#c3c7c4', { side: THREE.DoubleSide, spec: 0.4, shadowAmt: 0.95 }), { pos: [0.028, 0.062, 0.004], rot: [0, 0, -PI / 2], scale: [0.42, 1, 1] })));
  groundG.add(canG);
