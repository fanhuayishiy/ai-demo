...
import {
  grp, mesh, box, cyl, rbox, sph, tor, lathe, plane, tubeOf, finish, rand, range,
  weather, decal, shadowBlob, grill, inst,
} from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  cage.add(mesh(tubeBent(0.11, 0.0048), wireBent, { pos: [-(hw - 0.004), 0.22, -0.05], rot: [0, Math.PI / 2, 6 * D2R] }));
  // 帯（網を押さえる横バー）
  ...
  weather(band, { w: 0.16, h: 0.06, pos: [-0.10, 0.098, hd + 0.0122], kind: 'chip', color: '#e7e2d2', opacity: 0.55, seed: seed + 8, density: 1.6, spread: 0.005 });
  ...
  stuff.add(bt);
  bt.add(mesh(lathe([[0, -0.072], [0.029, -0.070], [0.0322, -0.04], [0.031, 0.012], [0.0205, 0.045], [0.0125, 0.062], [0.012, 0.086], [0.0135, 0.094], [0.0075, 0.098], [0, 0.099]], 16),
    MAT.glassLite({ color: '#a9cfae', opacity: 0.5 }), { cast: false, receive: false }));

  /* ================= カゴ外の錆・苔 ================= */
  weather(cage, { w: 0.14, h: 0.22, pos: [-(hw + 0.002), 0.20, 0.05], rot: [0, -Math.PI / 2, 0], kind: 'rust', color: PAL.rust, opacity: 0.55, seed: seed + 21, density: 1.7, spread: 0.006 });
