...
import {
  grp, mesh, box, cyl, rbox, sph, tor, lathe, tubeOf, cone, finish, rand, range,
...
export function build(options = {}) {
  ...
  for (let i = 0; i < 5; i++) {                                                                        // 水跡（縁からの流筋）
    weather(body, { w: 0.030, h: 0.19, pos: [-0.30 + i * 0.15 + range(rnd, -0.02, 0.02), 0.225, D / 2 + 0.0026], kind: 'rust', color: '#8f9a90', opacity: 0.35, seed: seed + 11 + i, density: 0.6, spread: 0.004 });
  }
  weather(body, { w: 0.30, h: 0.14, pos: [-0.10, 0.10, -(D / 2 + 0.0022)], rot: [0, Math.PI, 0], kind: 'moss', color: PAL.moss, opacity: 0.6, seed: seed + 16, density: 1.8, spread: 0.008 });
  weather(body, { w: 0.12, h: 0.16, pos: [-(W / 2 + 0.0022), 0.12, 0.04], rot: [0, -Math.PI / 2, 0], kind: 'moss', color: '#6d8a52', opacity: 0.55, seed: seed + 17, spread: 0.006 });
  ...
  for (const bx of [-0.222, -0.128, 0.004, 0.118, 0.226]) {
    ...
    dg.add(noShadow(mesh(tubeOf([
      [bx, 0.014, -0.030], [bx + s * 0.4, 0.007, 0.070],
  ...
  for (let i = 0; i < 6; i++) {   // 枯れ茎・小石
    soil.add(noShadow(mesh(cone(0.004, 0.028 + rnd() * 0.022, 5), MAT.paint('#8b7c5c', { steps: 2 }), {
      pos: [range(rnd, -0.30, 0.30), 0.292, range(rnd, -0.08, 0.08)], rot: [range(rnd, -0.5, 0.5), 0, range(rnd, -0.5, 0.5)],
    })));
  }
  ...
  function kale({ x, z, scale = 1, tone = 0 } = {}) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      radialPlace(orient(m, -0.10, a), a, 0.013, 0.048);
  ...
  function pansy({ x, z, hue = 0, scale = 1 } = {}) {
    ...
    for (let k = 0; k < 5; k++) {
      ...
      radialPlace(orient(m, -0.50, a), a, 0.028, 0.004);
  ...
  /* ---- 植栽配置 ---- */
  kale({ x: -0.250, z: 0.030, scale: 1.00, tone: 0 });
  kale({ x: 0.012, z: -0.030, scale: 1.10, tone: 1 });
  kale({ x: 0.268, z: 0.026, scale: 0.90, tone: 0 });
  pansy({ x: -0.128, z: -0.034, hue: 0, scale: 1.0 });
  pansy({ x: 0.142, z: 0.042, hue: 1, scale: 0.94 });
  pansy({ x: -0.318, z: -0.004, hue: 2, scale: 0.86 });
  ...
  weather(tag, { w: 0.10, h: 0.03, pos: [-0.360, 0.444, 0.1218], kind: 'dirt', color: '#a8977a', opacity: 0.42, seed: seed + 31, spread: 0.004 });
  ...
  can.add(noShadow(mesh(tubeOf([[-0.020, 0.169, 0], [-0.066, 0.226, 0], [-0.128, 0.228, 0], [-0.156, 0.176, 0], [-0.148, 0.122, 0]], 0.0075, 14, 6), canBody)));
  ...
  shovel.add(mesh(lathe([[0, -0.052], [0.012, -0.048], [0.016, -0.030], [0.015, 0], [0.010, 0.012], [0, 0.015]], 10), MAT.stainless({ repeat: 5 }), { pos: [0, -0.018, 0.100], rot: [0.30, 0, 0] }));
