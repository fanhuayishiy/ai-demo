...
import {
  grp, mesh, box, cyl, rbox, tubeOf, hoop, sph, finish, rand,
...
export const DEFAULT_OPTIONS = { seed: 12, faces: 2 };
...
const visorMat = () => MAT.metalPaint('#3f4a44', { worn: 0.95, repeat: 3, base: '#6f7975', side: THREE.DoubleSide });
...
function visorGeo(r, depth, arc = Math.PI, start = Math.PI / 2) {
  return new THREE.CylinderGeometry(r, r * 1.05, depth, 18, 1, true, start, arc);
}
function faceTex(no) {
...
export function build(options = {}) {
  ...
  for (let f = 0; f < faces; f++) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const lens = mesh(new THREE.SphereGeometry(Rc, 20, 10, 0, Math.PI * 2, 0, th), lensMat(kind), {
      ...
      sec.add(vz);
      sec.add(mesh(new THREE.CylinderGeometry(r + 0.038, r + 0.038, 0.012, 18, 1, true, Math.PI / 2, Math.PI), visor, { pos: [0, 0.006, 0.128], rot: [90 * D2R, 0, 0] }));
