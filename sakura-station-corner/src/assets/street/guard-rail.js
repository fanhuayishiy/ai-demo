...
import {
  grp, mesh, box, rbox, cyl, tor, shape, finish, rand, range,
  weather, decal, shadowBlob, plane, circ,
} from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { TEX, makeCanvas, toTexture, shade } from '../../core/textures.js';
...
export const DEFAULT_OPTIONS = { len: 4, seed: 44, endType: 'middle' };
...
function beamSection() {
  ...
  return shape((sh) => {
    ...
    sh.lineTo(-lip, BEAM_H);
    sh.lineTo(-lip + BEAM_T, BEAM_H);
    for (let i = mid.length - 1; i >= 0; i--) sh.lineTo(mid[i][0] - BEAM_T, mid[i][1]);
    sh.lineTo(-lip + BEAM_T, 0);
    sh.closePath();
  });
}
function beamMesh(lenVal, mat, x, y, rz = 0) {
  const geo = new THREE.ExtrudeGeometry(beamSection(), { depth: lenVal, bevelEnabled: false, curveSegments: 8, steps: 1 });
...
export function build(options = {}) {
  ...
  weather(g, { w: len * 0.5, h: 0.02, pos: [0, BEAM_Y + BEAM_H + 0.0018, -0.006], rot: [-90 * D2R, 0, 0], kind: 'dirt', color: '#8a8171', opacity: 0.5, seed: seed + 5, count: 3, spread: 0.05 });
  weather(g, { w: len * 0.4, h: 0.018, pos: [-len * 0.1, BEAM_Y - 0.0018, -0.004], rot: [90 * D2R, 0, 0], kind: 'moss', color: PAL.moss, opacity: 0.4, seed: seed + 6, count: 2 });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    g.add(ring);
    const dish = mesh(circ(rz * 0.94, 18), MAT.metal('#8d938f', { worn: 0.9, base: '#a9afab', side: THREE.DoubleSide }), { pos: [dx, BEAM_Y + BEAM_H * CREST_T, dz + 0.0055], scale: [1, 1.55, 1] });
  ...
  bed.add(mesh(rbox(len + 0.08, 0.055, 0.3, 0.008, 2), MAT.concrete({ base: PAL.concreteDark, repeat: 3, joints: 4, cracked: true }), { pos: [0, -0.002, -0.045] }));
