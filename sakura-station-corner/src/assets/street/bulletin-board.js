...
import {
  grp, mesh, box, cyl, rbox, tor, finish, rand, range,
...
export function build(options = {}) {
  ...
  const acrylic = MAT.glassLite({ color: '#e2ecea', opacity: 0.30, side: THREE.DoubleSide });             // 黄変し気味のアクリル
  ...
  const sheet = (x, y, w, h, map, tilt = -2.0, twist = 0, lay = 0) => {
    const p = grp('poster');
  ...
  // 左列
  sheet(-0.60, caseY1 - 0.10, 0.30, 0.42, P('春の運動会', '4月12日 グランド集合', '#f6e9d2', '#d9534f', 1), -2.2, 1.5, 0);
  sheet(-0.60, caseY1 - 0.56, 0.26, 0.34, P('防災訓練', '消化器・水道点検', '#e7eef5', '#2b6fb5', 2), -1.2, -2.0, 1);
  if (!narrow) sheet(-0.62, caseY1 - 0.86, 0.22, 0.24, P('子どもらの会', '春の遠足', '#fdeaf1', '#e2685e', 3), -3.4, 2.5, 2);
  // 中央
  sheet(-0.10, caseY1 - 0.11, 0.34, 0.46, P('広報紙 4月号', '春の限定メニュー', '#eef2e4', '#3f7a52', 4), -1.6, 0.8, 1);
  sheet(-0.06, caseY1 - 0.62, 0.28, 0.38, P('健康相談', '血圧・栄養', '#f7ecd8', '#8d6fb0', 5), -2.8, -1.6, 2);
  sheet(0.04, caseY1 - 0.94, 0.20, 0.22, P('猫の譲渡会', '日曜 13時～', '#fff2d6', '#c5312c', 6), -4.0, 3.2, 3);
  // 右列
  sheet(0.50, caseY1 - 0.12, 0.30, 0.44, P('図書館だより', '春の新着書目', '#e9f0f4', '#2c6cb0', 7), -1.0, -1.0, 0);
  sheet(0.52, caseY1 - 0.60, 0.26, 0.36, P('ゴミ出し案内', '資源・可燃 週2回', '#f4f0e2', '#f0b23c', 8), -2.0, 2.2, 1);
  if (!narrow) sheet(0.46, caseY1 - 0.90, 0.24, 0.28, P('春の募集', '役員・清掃当番', '#f6dfe6', '#c2564a', 9), -3.0, -2.6, 2);
  // 古い剥がし残り（破れた下貼り）
  ...
  slot.add(flap);
  flap.add(mesh(rbox(bw - 0.07, 0.060, 0.003, 0.0015, 2), acrylic, { pos: [0, -0.030, 0], cast: false }));
  // 中のチラシ（数束、口から覗く）
  for (const [fx, fw, fh] of [[-0.10, 0.115, 0.150], [0.03, 0.100, 0.140], [0.13, 0.090, 0.130]]) {
    slot.add(mesh(rbox(fw, fh, 0.020 + rnd() * 0.012, 0.003, 2), MAT.paper({ color: fx < 0 ? '#f7f2e4' : '#eef3ee' }), { pos: [fx - 0.02, boxY - 0.010, -0.008], rot: [3 * D2R, 0, range(rnd, -3, 3) * D2R] }));
  }
  // 蓋（後ろへ開いた状態＝中身が見える）・錠前・ラベル
  slot.add(mesh(rbox(bw + 0.01, 0.010, bd * 0.55, 0.004, 2), alu, { pos: [-0.02, boxY + bh / 2 + 0.014, -(bd / 2 + 0.014)], rot: [-32 * D2R, 0, 0] }));
  ...
  weather(frame, { w: cw * 0.5, h: 0.30, pos: [-0.20, 1.06, gz + 0.0045], kind: 'scratch', color: '#ffffff', opacity: 0.20, seed: seed + 46, density: 1.6, spread: 0.006 });   // 拭き傷
  ...
  // 枠・箱全体の錆と掉漆
  weather(body, { w: 0.30, h: 0.10, pos: [-0.30, caseY0 + 0.06, fg + 0.002], kind: 'chip', color: '#cfc9b8', opacity: 0.5, seed: seed + 61, density: 1.6, spread: 0.005 });
  weather(body, { w: 0.24, h: 0.16, pos: [0.44, 1.00, -(caseD / 2 + 0.002)], rot: [0, Math.PI, 0], kind: 'rust', color: PAL.rust, opacity: 0.5, seed: seed + 62, density: 1.5, spread: 0.006 });
  ...
  weather(roof, { w: 0.12, h: 0.06, pos: [-0.30, caseY1 + 0.045, 0.02], rot: [-Math.PI / 2, 0, 0], kind: 'moss', color: PAL.moss, opacity: 0.42, seed: seed + 64, spread: 0.004 });
