...
import {
  grp, mesh, box, cyl, rbox, tubeOf, hoop, sph, plane, finish, rand, range,
...
export const DEFAULT_OPTIONS = { seed: 17, faces: 1, countdown: true };
...
export function build(options = {}) {
  ...
  // 柱の点検口（内部に端子台）
  {
    ...
    hatch.add(mesh(tubeOf([[-0.014, -0.04, -0.01], [0.01, 0.0, -0.014], [-0.01, 0.04, -0.008]], 0.004, 10, 5), MAT.rubber('#d0a94f')));
  ...
  g.add(mesh(cyl(0.066, 0.07, 0.44, 16, true), MAT.metalPaint('#9aa5a0', { worn: 0.3, transparent: true, opacity: 0.6, side: THREE.DoubleSide }), { pos: [0, 1.66, 0] }));
  ...
  for (let f = 0; f < faces; f++) {
    ...
    for (const s of sections) {
      ...
      sec.add(mesh(box(W - 0.056, secH - 0.052, 0.01), MAT.paint('#1b1e1f', { steps: 2, shadowAmt: 1 }), { pos: [0, 0, -0.008] }));
      ...
      sec.add(scr);
      // 画面前の防眩フィルター（薄い樹脂・黄変）
      const film = mesh(plane(W - 0.056, secH - 0.052), MAT.glassLite({ color: '#e8e2cc', opacity: 0.12, side: THREE.DoubleSide }), { pos: [0, 0, 0.008], cast: false });
      film.userData.noOutline = true;
      sec.add(film);
      // バイザー（上から被さる半円殼＋左右の羽）
      const vw = W - 0.03, vd = 0.1;
      sec.add(mesh(new THREE.CylinderGeometry(vw * 0.62, vw * 0.62, vd, 16, 1, true, Math.PI * 0.16, Math.PI * 0.68), caseMat, { pos: [0, secH * 0.28, vd * 0.42], rot: [90 * D2R, 0, 0], scale: [1, 1, 1] }));
