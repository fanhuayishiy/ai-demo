...
import {
  grp, mesh, box, cyl, rbox, sph, tor, lathe, tubeOf, finish, rand, range,
...
function shell(r, h, angle, mat, { seg = 18, a0 = null, inner = false } = {}) {
  ...
  const geo = new THREE.CylinderGeometry(r, r, h, seg, 1, true, start, angle);
  const m = mesh(geo, inner ? MAT.paint('#1b1d20', { steps: 2, shadowAmt: 1, side: THREE.DoubleSide }) : mat, { cast: !inner, receive: true });
...
export function build(options = {}) {
  ...
  flap.add(mesh(box(0.196, 0.010, 0.012), aluTrim, { pos: [0, -0.001, 0.002] }));                 // 蝶番側バー
  ...
  // 「郵便」標記（曲面バンド）
  const band = shell(R + 0.014, 0.050, 0.92, MAT.poster({ map: TEX.signboard({ text: '郵便', sub: 'POST', bg: '#f6f2e6', fg: '#a9382f' }), side: THREE.DoubleSide }), { seg: 18 });
  band.position.set(0, yM + 0.150, 0);
  mouth.add(band);
  // 収集時間札（下部の曲面プレート・アルミ押え付き）
  const time = shell(R + 0.013, 0.070, 1.02, MAT.poster({ map: TEX.signboard({ text: '収集 17:00', sub: '平日 1 回・土曜 2 回', bg: '#f4efe0', fg: '#3a4448' }), side: THREE.DoubleSide }), { seg: 20 });
  ...
  // 〒 標記（上部・肩の下）
  const mark = shell(R + 0.014, 0.058, 0.44, MAT.poster({ map: TEX.lightPanel({ text: '〒', bg: '#f6f2e6', fg: '#a9382f', mode: 'sign' }), side: THREE.DoubleSide }), { seg: 10 });
  ...
  weather(lock, { w: 0.05, h: 0.05, pos: [0, -0.030, 0.008], kind: 'rust', color: PAL.rust, opacity: 0.6, seed: seed + 21, density: 1.4, spread: 0.004 });
  // 収集扉まわりの錆
