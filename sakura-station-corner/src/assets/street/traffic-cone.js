...
import {
  grp, mesh, box, cyl, rbox, lathe, sph, hoop, tubeOf, finish, rand, range,
  weather, decal, shadowBlob, plane, circ,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { kind: 'cone', seed: 101 };
...
function posterBoard() {
  ...
  jpText(g, '出入りあり', { x: w / 2, h: undefined, y: h * 0.085, size: h * 0.11, color: '#fff8ef', weight: 900, spacing: 4 });
  // 手書き風の丸い吹き出しと矢印
...
export function build(options = {}) {
  ...
  if (kind === 'cone') {
    ...
    for (const [t, hh] of [[0.44, 0.1], [0.66, 0.062]]) {
      ...
      cg.add(mesh(cyl(rr, rr * 1.06, hh, 24, true), MAT.paint('#f2eee2', {
        steps: 2, spec: 0.42, specPower: 90, side: THREE.DoubleSide, shadowAmt: 0.6,
        map: TEX.metal({ base: '#efe9dc', worn: 0.5, repeat: 3 }).map,
      }), { pos: [0, y, 0], name: 'cone-tape' }));
    }
    // 反射帯の剥がれ（めくれた三角片）
    const peel = mesh(plane(0.07, 0.06), MAT.paint('#e7e1d1', { side: THREE.DoubleSide, steps: 2 }), { pos: [0.085, 0.05 + H * 0.44, 0.048], rot: [-0.35, 0.6, 0.3] });
    cg.add(peel);
    // 積み跡（重ねた時の擦過環）
    ...
    // 日焼け退色（片側だけ色抜け）
    cg.add(mesh(lathe(prof.slice(0, 9).map(([x, y]) => [x + 0.0015, y]), 20, -0.9, 1.9), MAT.plastic(shade(orange, 1.28), { transparent: true, opacity: 0.55, side: THREE.DoubleSide, steps: 2 }), { pos: [0, 0.05, 0] }));
    // 先端の欠け・潰れ
  ...
  if (kind === 'stand') {
    ...
    panel.add(poster);
    // アクリル（黄変・小傷・ネジ留め）
    const acry = mesh(rbox(W - 0.02, H * 0.8, 0.006, 0.004, 2), MAT.glassLite({ color: '#efe4bd', opacity: 0.2, side: THREE.DoubleSide }), { pos: [0, 0.006, 0.017], cast: false });
    ...
    // 下側の日除け帯（黄色反射）
    panel.add(mesh(rbox(W, 0.05, 0.014, 0.005, 2), MAT.paint(PAL.markingYellow, { steps: 2, map: bandTex(), side: THREE.DoubleSide }), { pos: [0, -H * 0.4, 0.008] }));
    sg.add(panel);
  ...
  if (kind === 'mirror') {
    ...
    const face = mesh(new THREE.SphereGeometry(Rf, 30, 16, 0, Math.PI * 2, 0, cap), MAT.chrome({ spec: 0.95, specPower: 230, base: '#cdd6d8' }), {
