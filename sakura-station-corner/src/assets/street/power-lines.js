...
import { grp, mesh, box, cyl, rbox, tor, tubeOf, catenary, coil, hoop, finish, rand, range } from '../../core/kit.js';
import { MAT } from '../../core/materials.js';
import { PAL } from '../../core/palette.js';
import { shade } from '../../core/textures.js';
...
export const POLE_ANCHORS = {
  refHeight: 8.0,
  levels: [
    { name: 'primary', wires: 'high-tension', pts: [{ x: 0.00, y: 7.55 }, { x: -0.70, y: 7.35 }, { x: 0.70, y: 7.35 }] },
...
export function anchorList(height = 8.0) {
...
export function poleRotY(dx, dz) {
...
export const DEFAULT_OPTIONS = { a: [-17.2, 6.3], b: [-13.9, 6.3], config: 'primary', sag: 0.6, seed: 5 };
...
function orient(obj, vec, axis = 'z') {
  const to = new THREE.Vector3(vec.x, vec.y || 0, vec.z);
...
export function build(options = {}) {
  ...
  for (const lv of levels) {
    ...
    for (const pt of lv.pts) {
      ...
      const jitter = range(rnd, -0.0012, 0.0012);       // 同一回路内の微細な位置ズレ（束に見せない）
      ...
      for (const [pole, P, sgn] of [[poleA, A, 1], [poleB, B, -1]]) {
        const dv = new THREE.Vector3(pole[0] - P[0], 0, pole[2] - P[2]);
        const towardPole = dv.lengthSq() > 1e-8 ? dv.normalize() : new THREE.Vector3(sgn * dx, 0, sgn * dz);
        // 線軸（杆から跨中へ抜ける方向）：バンド・針金巻きはこの軸に巻く
        const toMid = new THREE.Vector3(sgn * dx, 0, sgn * dz);
