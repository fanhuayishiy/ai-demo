...
import { grp, mesh, box, cyl, rbox, tor, cone, sph, circ, plane, extrude, shape, finish, rand, range, weather, decal, inst, catenary, pipe } from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const Z_PADDY_TOP = -2.10, Z_PADDY_END = -3.62, Z_BANK_TOP = -3.98, Z_FAR = -4.72;
  ...
  const netMat = MAT.metal('#a9b6a8', { alphaMap: netMask(9, 6), alphaTest: 0.5, side: THREE.DoubleSide, uv: { repeat: [2.3, 2.3] }, spec: 0.34, sheen: 0.06 });
  const netMatFade = MAT.metal('#87977c', { alphaMap: netMask(7, 8), alphaTest: 0.5, side: THREE.DoubleSide, uv: { repeat: [1.9, 1.9] }, spec: 0.16, sheen: 0.02, shadowAmt: 0.98 });
  ...
  for (let c = 0; c < 3; c++) {
    ...
    place(hills, scatter(n * 6, () => {
      ...
      return { x: cx + range(rnd, -1.0, 1.0), y: cb + range(rnd, 1.9, 4.3), z: cl(cz + range(rnd, -0.9, 0.9), Z_FAR + 0.05, Z_BANK_TOP), s, s2: s, rx: range(rnd, -1.4, 1.4), ry: rnd() * 6.283, rz: range(rnd, -1.2, 1.2), c: [0.9 + rnd() * 0.2, 0.97 + rnd() * 0.14, 0.82 + rnd() * 0.2] };
