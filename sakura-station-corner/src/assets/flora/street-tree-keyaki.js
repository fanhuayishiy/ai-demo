...
import { grp, mesh, cyl, tor, plane, finish, rand, range, weather, inst } from '../../core/kit.js';
...
function cardGeo(w, h, bend = 0.14, segY = 3) {
  ...
  for (let i = 0; i < p.count; i++) {
    const t = THREE.MathUtils.clamp((p.getY(i) + h / 2) / h, 0, 1);   // 浮動小数の僅かな負値で pow→NaN にならないよう固定
...
function limb(parent, pts, r0, r1, mat, { segs = 5, name = 'limb' } = {}) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    const mm = mesh(
      cyl(rq(THREE.MathUtils.lerp(r0, r1, t1)), rq(THREE.MathUtils.lerp(r0, r1, t0)), rq(len * 1.14), 6 + Math.round((1 - t0) * 4)),
...
export function build(options = {}) {
  ...
  const squash = 0.86 + rnd() * 0.16;               // 横に広がる／細身の個体差
