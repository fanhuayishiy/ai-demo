...
import { grp, mesh, box, cyl, rbox, tor, sph, circ, plane, lathe, coil, finish, rand, range, weather, decal, inst } from '../../core/kit.js';
...
export function build(options = {}) {
  ...
  const petalMat = (c, extra = {}) => MAT.paint(c, { steps: 3, spec: 0.14, specPower: 24, specCut: 0.45, sheen: 0.04, shadowAmt: 0.66, side: THREE.DoubleSide, rim: 0.22, rimColor: '#fff1f6', ...extra });
  ...
  function potDetails(pot, root, { r, h, soilY, chips = 2, waterY = 0.005 }) {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      grains.push({ x: Math.cos(a) * rr, y: soilY + range(rnd, -0.001, 0.007), z: Math.sin(a) * rr, s: range(rnd, 0.5, 1.3), rx: rnd() * 3, ry: rnd() * 3, rz: rnd() * 3, c: [0.9 + rnd() * 0.3, 0.8 + rnd() * 0.25, 0.65 + rnd() * 0.22] });
    }
  ...
  if (kind === 'conifer') {
    ...
    const trunkPts = [[0.0, 0.0], [0.018, 0.05], [-0.012, 0.115], [0.02, 0.185], [-0.004, 0.245], [0.012, 0.29]];
    const up = new THREE.CatmullRomCurve3(trunkPts.map((p) => V(p[0], p[1], 0)), false, 'catmullrom', 0.4);
    for (let i = 0; i < 9; i++) {
    ...
    for (const [yh, ang, tilt, ln] of PAD) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        seg.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dd.normalize());
        tree.add(seg);
      }
      const tip = pts[2];
    ...
    for (let i = 0; i < 2; i++) {
      ...
      f.add(head);
      for (let k = 0; k < 2; k++) f.add(mesh(bladeGeo(0.024, 0.1, { bend: 0.3, taper: 0.4, twist: 0.35 }), matLeafG, { pos: [k ? 0.008 : -0.008, 0.003, 0], rot: [0.25, k ? 1.2 : -1.2, k ? 0.2 : -0.2], name: 'leaf' }));
      plant.add(f);
    }
    // 落ちた花弁（鉢縁と受け皿）＋ 枯れ茎
