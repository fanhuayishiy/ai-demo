...
import {
  grp, mesh, lathe, sph, rbox, tubeOf, catenary, sakuraPetalGeo, sakuraPetalRestGeo, sakuraLeafGeo, tuftGeo,
  weather, inst, shadowBlob, rand, range, finish,
} from '../../core/kit.js';
...
export const DEFAULT_OPTIONS = { seed: 53, height: 4.6, spread: 4.2 };
...
function leafCardRaw(w, h, curl) {
  const g = new THREE.PlaneGeometry(w, h, 3, 3);
  const p = g.attributes.position;
  for (let i = 0; i < p.count; i++) {
    const x = p.getX(i), y = p.getY(i);
    const nx = x / (w / 2), ny = y / (h / 2);
...
function limbGeo(pts, r0, r1, o = {}) {
  ...
  for (let i = 0; i <= seg; i++) {
    ...
    for (let j = 0; j <= radial; j++) {
      ...
      const d = new THREE.Vector3(N.x * c + B.x * s, (N.y * c + B.y * s) * flat, N.z * c + B.z * s);
...
export function build(options = {}) {
  ...
  function puff(host, rr0, nn, cwv, matv, pale, curl, spin0) {
    host.add(inst(petalCard(q(cwv), q(cwv * 0.95), curl), matv, nn, (i, d, ru, col) => {
      ...
      const dir = new THREE.Vector3(Math.cos(az) * hrr, sy * (sy > 0 ? 1 : 0.7), Math.sin(az) * hrr).normalize();
      ...
      const nz = new THREE.Vector3().crossVectors(tan, ny).normalize();
      d.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(tan, ny, nz));
      d.rotateX(range(ru, -0.7, 0.7));
      d.rotateY(range(ru, -0.55, 0.55));
      d.rotateZ(range(ru, -1.0, 1.0));
