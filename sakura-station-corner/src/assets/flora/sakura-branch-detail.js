...
import {
  grp, mesh, sph, rbox, cyl, tubeOf, catenary, lathe, sakuraPetalGeo, sakuraLeafGeo,
...
export const DEFAULT_OPTIONS = { seed: 71, length: 1.2 };
...
function limbGeo(pts, r0, r1, o = {}) {
  ...
  for (let i = 0; i <= seg; i++) {
    ...
    for (let j = 0; j <= radial; j++) {
      ...
      const d = new THREE.Vector3(N.x * c + B.x * s, N.y * c + B.y * s, N.z * c + B.z * s);
...
export function build(options = {}) {
  ...
  {
    const bp = main[0];
    const tan = new THREE.Vector3(main[1][0] - bp[0], main[1][1] - bp[1], main[1][2] - bp[2]).normalize();
