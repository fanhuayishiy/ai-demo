...
```bash
npm run check        # 逐个 import + build() 全部模块，136/136
...
- 画面内**没有任何控件**：机位预设、风格切换、耗时剖析都只走 URL 参数，不是界面上的按钮。
  `?style=toon` 切三渲二、`?asm=full|slow|freeze` 调装配期渲染节流、`?trace=1` 打开耗时记录。
- 无音频、无昼夜变化、无自动巡游；场景内不放置人物与动物。
...
- 场景不模拟任何实时数据：没有交通流、没有客流、没有天气变化，电车与信号灯是循环动画而非时刻表驱动。
...
- **零 UI 的代价是必须能测**：画面不许出现调试信息，所以所有可观测性都搬到无头工具里
  （`tools/` 23 个脚本：包围盒核对、越框检测、真实拖拽帧率、逐资产消融、逐帧抖动分布、
  分辨率对照、材质/几何状态计数、LOD 阈值扫描、按形状反查杂物、启动墙钟的阶段归因、
  加载状态实拍、人为加公网往返延迟复现串行 chunk 拉取）。
...
- `npm run check`：136 个模块逐个 `import()` 并调用 `build()`，检查导出契约、几何有限性、包围盒非空。
