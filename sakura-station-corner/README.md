...
> 在线演示：<https://fanhuayishiy.github.io/ai-demo/sakura-station-corner/dist/>
...
- **零 UI 的代价是必须能测**：画面不许出现调试信息，所以所有可观测性都搬到无头工具里
  （`tools/` 19 个脚本：包围盒核对、越框检测、真实拖拽帧率、逐资产消融、逐帧抖动分布、
  分辨率对照、材质/几何状态计数、LOD 阈值扫描、按形状反查杂物）。
...
在线演示直接点上面的链接（Pages 托管的是 `dist/`，无需构建）。
...
- **「不合并网格」≠「不能实例化」**：几何数据永不改写、永不 merge；允许的是把「同几何 + 同材质 +
  ...
  真实货架本来就是同款重复摆放，所以这是更准确的建模而不是简化。全场景 27025 个 Mesh 折叠成 6456 个实例批次。
...
| 指标 | 数值 |
| --- | --- |
| 启动到 `built` | 21 s（世界装配 15 s，动效注册 6 s） |
| 场景规模 | 122 个资产实例 / 27025 Mesh / 6456 实例批次 / 11201 几何 / 3299 材质 / 835 贴图 |
| JS 堆 | ≈ 590 MB（贴图位图 455 MB） |
| 交互 | 持续拖拽 draw calls ≈ 7200，4.8M 三角/帧，fps 中位 17（8.3 µs/次的 CPU 提交是瓶颈，不是填充率） |
| 产物 | `dist/` 2.4 MB（gzip 主 chunk 292 KB），104 个文件 |
...
- 交互帧率约 17 fps（1600×900）。瓶颈是 CPU 提交次数，进一步提速需要逐实例顶点着色器风动
