...
> 在线演示：<https://fanhuayishiy.github.io/ai-demo/sakura-station-corner/dist/>（公网出画面约 9 s，本地约 8 s）
...
在线演示直接点上面的链接（Pages 托管的是 `dist/`，无需构建，公网出画面约 9 s）。
...
| 启动到「看见成品」· 公网 | **9 s**（GitHub Pages）。这里有个坑值得记：同一份产物本地 7.9 s、推上去 36 s，因为 `import.meta.glob` 把资产切成 ~100 个 lazy chunk 而装配循环逐个 `await`，公网就是 100 次串行往返；改成启动即并发预热后世界阶段 13.88 s → 5.44 s（`tools/net-lag.mjs` 给每个 chunk 加 120 ms 可在本地复现） |
