import { g as grp, M as MAT, P as PAL, T as TEX, a as sph, n as range, m as mesh, c as cyl, H as plane, E as inst, t as tor, w as weather, r as rbox, U as circ, C as CatmullRomCurve3, D as DoubleSide, q as finish, Y as memo, z as rand, Z as Euler, V as Vector3, X as cone, N as makeCanvas, Q as toTexture, _ as mulberry32 } from './index-yWEMv7O8.js';
...
function build(options = {}) {
  ...
  function layer(y, color, mask, { map, alphaTest = 0.38, opacity, w = W, d = D, name = 'layer', rough = 1, sat = 1.02 } = {}) {
    ...
    const m = mesh(plane(w / k, d / k), MAT.paint(color, {
      map, alphaMap: mask, alphaTest, side: DoubleSide, steps: 3,
  ...
  const _eu = new Euler(), _v = new Vector3();
  ...
  MAT.concrete({ base: PAL.dirt, repeat: 4 });
  ...
  if (kind === 'moss') {
    ...
    const curve = new CatmullRomCurve3(crackPath, false, 'catmullrom', 0.4);
    const wLists = [[], [], []];
    ...
    function woodSorrel(x, z) {                                  // カタバム：三つ葉＋黄花
      ...
      f.add(mesh(sph(0.0055, 6, 5), MAT.food({ color: '#e6d34a', spec: 0.24, steps: 2 }), { pos: [0.01, 0.042, -8e-3], scale: [1, 0.7, 1], name: 'ws-flower' }));
      return f;
    }
    ...
    // 側溝方向（+Z）へ流出した舌
    {
      const tongue = mesh(plane(W * 0.34, D * 0.3), MAT.paint(PAL.dirt, { map: TEX.concrete({ base: PAL.dirt, repeat: 3 }).map, alphaMap: blobMask('dirt-tongue', { blobs: 5, spread: 0.24, hard: 0.5, seed: seed + 37 }), alphaTest: 0.4, side: DoubleSide, shadowAmt: 0.94 }), { pos: [W * 0.12, 0.0035, D * 0.34], rot: [-Math.PI / 2, 0, 0.2], name: 'dirt-outflow', cast: false, receive: true });
      g.add(tongue);
    }
    // 玉石・土塊・枯れ枝
  ...
  /* ---------------- 接地感を足す接触影（薄く・大きく） ---------------- */
  const shadow = mesh(plane(W * 0.92, D * 0.92), MAT.decal({ map: blobMask(`sh${kind}`, { blobs: 6, spread: 0.34, hard: 0.35, seed: seed + 57 }), color: '#3a3325', opacity: 0.14, order: 1, side: DoubleSide }), { pos: [0, 0.0012, 0], rot: [-Math.PI / 2, 0, 0], name: 'contact-dirt', cast: false, receive: false });
  g.add(shadow);
