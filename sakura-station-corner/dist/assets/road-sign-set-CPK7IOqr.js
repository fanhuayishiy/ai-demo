 ...
 *  必须用 source-atop 画：canvas 的 alpha 通道就是贴花的 alpha 通道，
 *  直接 globalAlpha 画上去会在贴图里留下 10% 半透明的像素 ——
 *  alphaTest 0.02 挡不住它们，于是牌面会透出背后的补强筋和立柱，
 *  实拍里读成「招牌上横着几条浅色杠」。source-atop 只和已有像素合成、
 *  保留目标 alpha，褪色就只是颜色变化，不会再挖出半透明的洞。 */
...
function build(options = {}) {
  ...
  /* ------------------------------ 取付金具（バンド・ボルト） ------------------------------ */
  // バンドは「標板の裏側を回して柱を締める」金具なので、**必ず板より後ろに置く**。
  // 以前は strap を z=-0.03+0.012=-0.018 に厚み 0.078 で置いていて前縁が z=+0.021、
  // つまり反射面（z=+0.0042）より 17 mm 手前まで出ていた。標板の高さが 0.44 m を
  // 超えるとバンドが 3 本なので、正面から見て「停止」の文字を横切る白い帯が
  // 本数どおりに並んだ —— ユーザーが指した「路標上的三横」はこれ
  // （`shots/y1/sign-a.png`）。板厚 DP=0.02 より後ろ、柱（z=-0.042..-0.018）を
  // 締める範囲に収める。
  const bandN = h > 0.44 ? 3 : 2;
  const BAND_Z = -8e-3;                 // グループ（z=-0.03）から見て、板の裏側に寄せる
  for (let i = 0; i < bandN; i++) {
    const by = plateY + (i - (bandN - 1) / 2) * (h * 0.28);
    const b = grp('band', { pos: [0, by, -0.03] });
    b.add(mesh(box(w * 0.5, 0.028, 0.028), bandMat(), { pos: [0, 0, BAND_Z] }));
    b.add(mesh(box(w * 0.5 + 0.01, 0.012, 0.03), MAT.metal('#8d9491', { worn: 0.7 }), { pos: [0, 0.018, BAND_Z] }));
    for (const sx of [-1, 1]) {
      b.add(mesh(cyl(0.007, 0.007, 0.014, 6), MAT.metal('#b7bcc0'), { pos: [sx * w * 0.22, 0, BAND_Z - 0.014], rot: [90 * D2R, 0, 0] }));
    }
    g.add(b);
    weather(g, { w: 0.08, h: 0.03, pos: [w * 0.16, by - 0.022, -0.038], kind: 'rust', color: PAL.rust, opacity: 0.55, seed: seed + 31 + i, count: 1 });
  }
