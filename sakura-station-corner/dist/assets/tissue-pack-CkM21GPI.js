import { M as MAT, P as PAL, T as TEX, g as grp, n as range, m as mesh, r as rbox, w as weather, Y as memo, c as cyl, b as box, h as decal, z as rand, V as Vector3, t as tor, B as BufferGeometry, F as Float32BufferAttribute, S as Shape, R as ExtrudeGeometry, ac as Box3, N as makeCanvas, Q as toTexture } from './index-BxWjt-aN.js';
...
function endFinGeo(len, wid, teeth, th) {
  ...
  s.lineTo(0, wid / 2);
  s.closePath();
  const geo = new ExtrudeGeometry(s, { depth: th, bevelEnabled: false, curveSegments: 1 });
  geo.rotateX(-PI / 2);
...
function build(options = {}) {
  ...
  if (pack > 1) {
    ...
    for (let i = 0; i < pack; i++) {
      ...
      u.position.set((c - (n - 1) / 2) * (W * 0.965), r * (H * 0.985), r ? range(rnd, -4e-3, 0.004) : 0);
      u.rotation.set(0, range(rnd, -0.05, 0.05), range(rnd, -6e-3, 0.006));
      bundle.add(u);
    }
...
function unit(sp, M, rnd, seed, seedN) {
  ...
  const body = grp('body', { pos: [0, lift, 0], rot: [range(rnd, -4e-3, 0.004), range(rnd, -0.01, 0.010), 0] });
  u.add(body);
  ...
  /* --- 印刷（前面・背面・天面） --- */
  body.add(mesh(printPatch(sp, a, 'z', -0.09329 * 0.84, a.hw * 0.84, -a.hh * 0.52, a.hh * 0.52, 0.00040), M.front,
    { name: 'print-front', cast: false, receive: false, renderOrder: 3 }));
  body.add(mesh(printPatch(sp, a, 'z', -0.09329 * 0.84, a.hw * 0.84, -a.hh * 0.52, a.hh * 0.52, 0.00040), M.top,
    { name: 'print-back', cast: false, receive: false, renderOrder: 3, rot: [0, PI, 0] }));
  body.add(mesh(printPatch(sp, a, 'y', -0.09329 * 0.58, a.hw * 0.58, -0.049100000000000005 * 0.46, a.hd * 0.46, 0.00032), M.band,
    { name: 'print-top', cast: false, receive: false, renderOrder: 4 }));

  /* --- 両端のフィシール（ギザ） --- */
  for (const s of [1, -1]) {
    body.add(mesh(endFinGeo(0.0136, a.hd * 1.24, 6, 0.0024), M.fin,
      { name: 'fin-' + (s > 0 ? 'r' : 'l'), pos: [s * a.hw * 0.845, 0, -12e-4], rot: [0, s > 0 ? 0 : PI, 0] }));
  ...
  if (sp.lid === 'sticker') {
    ...
    o.add(mesh(rbx(a.hd * 0.34, 0.0006, a.hd * 0.52, 0.0003), M.ink, { name: 'open-slot', pos: [-0.049100000000000005 * 0.20, 0.0009, 0], cast: false }));
    for (let i = 0; i < 9; i++) {
      body.add(mesh(box(0.0026, 0.0005, 0.0012), M.lidDark, { name: 'perf' + i, pos: [-0.049100000000000005 * 0.62 + i * 0.0066, oy + 0.0005, -0.049100000000000005 * 0.26], cast: false }));
    }
    tissueTips(body, M, -0.049100000000000005 * 0.10, oy + 0.0008, 0.002, 4, a.hd * 0.44, rnd, -0.42);
  } else if (sp.lid === 'flip') {
    ...
    // 剝がれかけた楕円シール
    const st = grp('seal', { pos: [-ow * 0.30, oy + lt + 0.0012, 0], rot: [0, -0.2, 0] });
    body.add(st);
    ...
    // 紙箱：天面スリットから布が掛け出る
    body.add(mesh(rbx(a.hw * 0.60, 0.0009, 0.0060, 0.0005), M.ink, { name: 'box-slit', pos: [a.hw * 0.14, oy + 0.0004, -0.049100000000000005 * 0.04], cast: false }));
    ...
    // 天面の印刷帯・小口のかぶせ
    body.add(mesh(rbx(a.hw * 1.42, 0.0007, a.hd * 1.36, 0.0004), M.top, { name: 'box-top-print', pos: [-0.09329 * 0.10, oy + 0.0009, 0], cast: false }));
    for (const s of [1, -1]) {
  ...
  if (sp.defect === 'wrinkle') {
    decal(body, { map: TEX.wear({ kind: 'scratch', color: '#ffffff', seed: seed + 31, density: 1.6 }), w: 0.038, h: 0.014, pos: [-0.09329 * 0.42, a.hh * 0.16, zf(a.hw * 0.42, a.hh * 0.16)], opacity: 0.30 });
  }
  if (sp.defect === 'peel') {
  ...
  if (sp.defect === 'corner') {
    ...
    body.add(mesh(rbx(0.0096, 0.0058, 0.0068, 0.0015), M.tissueShade, { name: 'corner-bruise', pos: [a.hw * 0.84, a.hh * 0.60, -0.049100000000000005 * 0.70], rot: [0.3, 0.4, 0.2], cast: false }));
  ...
  decal(body, { map: TEX.wear({ kind: 'dirt', color: '#ded8c8', seed: seed + 36, density: 0.8 }), w: 0.014, h: 0.012, pos: [-0.09329 * 0.28, -a.hh * 0.14, zf(a.hw * 0.28, a.hh * 0.14)], rot: [0, 0, 0.5], opacity: 0.24 });
  return u;
}
...
function rollsAndWrap(body, M, sp, rnd, seed) {
  ...
  film.setAttribute('position', new Float32BufferAttribute(pos, 3));
...
const P_TISSUE = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  VARIANTS,
  build,
  default: build,
...
export { P_TISSUE as P, build as b };
