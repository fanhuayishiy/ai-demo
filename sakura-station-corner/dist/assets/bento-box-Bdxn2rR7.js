import { g as grp, M as MAT, T as TEX, ai as capsule, m as mesh, b as box, c as cyl, n as range, a as sph, D as DoubleSide, o as lathe, r as rbox, k as tubeOf, h as decal, t as tor, S as Shape, ab as Path, R as ExtrudeGeometry, V as Vector3, B as BufferGeometry, F as Float32BufferAttribute, z as rand, ac as Box3 } from './index-CB0Lacpv.js';
...
function frameGeo(w, d, r, t, h) {
  const outer = new Shape(); rrPath(outer, w, d, r);
  const hole = new Path(); rrPath(hole, w - t, d - t, Math.max(0.0015, r - t * 0.5));
...
function build(options = {}) {
  ...
  function baran(x0, x1, z, hgt, color = '#eaf1de') {
    g.add(mesh(sheetGeo(16, 4, (u, v) => new Vector3(
      lerp(x0, x1, u), foodY + 0.001 + v * hgt, z + 0.0018 * Math.sin(u * Math.PI * 13),
    )), MAT.paper({ color, repeat: 8, side: DoubleSide, spec: 0.08 }), { name: 'baran', cast: false }));
  }
  function ran(x, z, r, h, color = '#f2b9c4') {
    ...
    g.add(mesh(lathe(pts, 18), MAT.paper({ color, repeat: 8, side: DoubleSide, spec: 0.1 }), { name: 'ran', pos: [x, foodY, z], cast: false }));
  }
  ...
  function Leaf(cx, cy, cz, s, color, curl = 0.5) {
    g.add(mesh(sheetGeo(10, 8, (u, v) => new Vector3(
      cx + (u - 0.5) * s * 2,
      cy + curl * s * (0.5 - v) * Math.sin(u * 3.1) + 0.12 * s * Math.sin(u * 7 + v * 5) + 0.004 * Math.sin(u * 9),
      cz + (v - 0.5) * s * 1.5,
    )), MAT.leaf({ color, side: DoubleSide, spec: 0.22, rim: 0.34 }), { name: 'leaf', cast: false }));
  }
  ...
  /* ---- オカズ ---- */
  if (key === 'chicken') {
    const spots = [[-0.05, -0.02], [-0.014, -0.028], [0.024, -0.018], [-0.038, 0.014], [0.002, 0.016], [0.040, 0.008]];
  ...
  if (key === 'sashimi') {
    const cuts = [
      [-0.05, -0.016, 0.032, 0.0135, '#c2564a', true, 0.10],
      [-0.014, -0.02, 0.030, 0.0125, '#e8846a', true, -0.06],
      [0.022, -0.016, 0.028, 0.0120, '#f0a278', false, 0.05],
      [0.052, -0.018, 0.024, 0.0105, '#dbe0e4', false, -0.12],
      [-0.03, 0.014, 0.028, 0.0120, '#b9573f', true, 0.16],
    ];
  ...
  if (key === 'salad') {
    ...
    const tom = [[-0.04, 0.022], [0.008, 0.030], [0.044, -6e-3], [-0.012, -0.026]];
    ...
    cup.add(mesh(lathe([[0, 0], [0.0102, 0.0008], [0.0118, 0.0100], [0.0126, 0.0114], [0.0108, 0.0118]], 16),
      MAT.hardPlastic('#eae6db', { transparent: true, opacity: 0.6, depthWrite: false, side: DoubleSide }), { name: 'cup', pos: [0, foodY - 0.0008, 0] }));
  ...
  if (key === 'pasta') {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      m.position.set(0.008 + Math.cos(a) * rr * 1.42, foodY + 0.0078 + range(rnd, -2e-3, 0.0045), Math.sin(a) * rr);
    ...
    for (let i = 0; i < 3; i++) Leaf(0.008 + range(rnd, -0.03, 0.030), foodY + 0.0112, range(rnd, -0.022, 0.022), 0.011, '#3f7a45', 0.4);
  ...
  if (key === 'tendon') {
    ...
    for (let i = 0; i < 2; i++) {
      const x = -0.04 + i * 0.052, z = -0.014 + i * 0.010;
      g.add(mesh(capsule(0.0060, 0.0340, 8), batter, { name: 'shrimp-tempura', pos: [x, foodY + 0.0102, z], rot: [0.16, 0.4 - i * 0.7, 1.42] }));
      for (let k = 0; k < 6; k++) {
        g.add(mesh(sph(0.0028, 7, 5), batter, { pos: [x + range(rnd, -0.015, 0.015), foodY + 0.0128 + range(rnd, -3e-3, 0.003), z + range(rnd, -9e-3, 0.009)], cast: false }));
      }
    ...
    const strip = mesh(sheetGeo(8, 2, (u, v) => new Vector3(lerp(-0.022, 0.028, u), foodY + 0.0158 + 0.0016 * Math.sin(u * 6), lerp(0.002, 0.016, v))), MAT.nori({ side: DoubleSide }), { name: 'nori-strip', cast: false });
    g.add(strip);
  ...
  if (key === 'ekiben') {
    ...
    g.add(mesh(cyl(0.0086, 0.0086, 0.0032, 16), MAT.food({ color: '#f6f1e8', spec: 0.4 }), { name: 'naruto', pos: [-0.04, foodY + 0.0138, 0.024], rot: [0.2, 0, 1.5] }));
    g.add(mesh(tor(0.0038, 0.0010, 5, 14), MAT.food({ color: '#e0576c', spec: 0.4 }), { pos: [-0.04, foodY + 0.0154, 0.024], rot: [0.2, 0, 0], cast: false }));
    ...
    for (let i = 0; i < 12; i++) {
      ...
      s.position.set(range(rnd, -0.06, 0.060), foodY + 0.0150, range(rnd, 0.000, 0.046));
  ...
  if (sp.chop) {
    ...
    const zBase = onLid ? -0.028 : 0.0400;
    const wood = MAT.wood({ light: '#e6d8b6', dark: '#c3ac7d', repeat: 3 });
    const cg = grp('chopsticks', { pos: [0, 0, 0], rot: [0, -0.1, 0] });
    g.add(cg);
    for (let i = 0; i < 2; i++) {
      const y = yBase + i * 0.0042;
      cg.add(mesh(box(0.138, 0.0040, 0.0048), wood, { name: 'chopstick-' + i, pos: [-6e-3, y, zBase + i * 0.0054], rot: [0, 0.02 - i * 0.05, 0.015] }));
  ...
  if (sp.slip) {
    ...
    g.add(mesh(box(0.050, 0.0006, 0.028), MAT.paper({ color: '#f8f5ec', repeat: 4 }), { name: 'slip', pos: [-0.044, sy, 0.032], rot: [0, -0.2, 0.02] }));
    decal(g, { map: TEX.adStrip({ text: 'お取り扱い注意', bg: '#f8f5ec', seed: seed + 7 }), w: 0.040, h: 0.022, pos: [-0.044, sy + 0.0005, 0.032], rot: [-Math.PI / 2, 0, -0.2], opacity: 0.95 });
  }

  /* ---- 蓋（PET ドーム／印刷ペーパー蓋／紙スリーブ） ---- */
  const lidY = TRAY_H + 0.0012;
  const pet = MAT.glassLite({ color: '#eff8f9', opacity: 0.20, spec: 1, specPower: 250, specCut: 0.05, sheen: 0.45, rim: 0.5, side: DoubleSide, steps: 2 });
  if (sp.lid === 'dome') {
  ...
  if (sp.lid === 'paper') {
    ...
    }), { name: 'paper-lid', pos: [0, lidY + t / 2, -0.0314], rot: [0.020, 0.02, -0.01] }));
    g.add(mesh(box(shellW + 0.005, 0.0012, lz * 0.98), MAT.paper({ color: '#e2d6bc', repeat: 6 }), {
      name: 'paper-lid-underside', pos: [0, lidY - 0.0004, -0.0314], cast: false,
    }));
    decal(g, { map: TEX.wear({ kind: 'dirt', color: '#c9bb98', seed: seed + 61, density: 1.1 }), w: 0.060, h: 0.024, pos: [0.020, lidY + t + 0.0004, -0.044], rot: [-Math.PI / 2, 0, 0.02], opacity: 0.30 });
  }
  if (sp.sleeve) {
    // 紙スリーブ（前後左右の 4 面＋前面印刷）
    const sm = MAT.poster({ map: TEX.drinkLabel({ name: sp.name, sub: sp.sub, ml: '内容量 約320g', a: sp.accent, b: sp.bg, kind: 'bento' }), steps: 3, side: DoubleSide, sat: 1.04, spec: 0.14 });
  ...
  if (sp.defect === 'sauce') {
    decal(g, { map: TEX.wear({ kind: 'dirt', color: '#8a4326', seed: seed + 54, density: 1.5 }), w: 0.046, h: 0.024, pos: [-0.02, foodY + 0.0016, -0.028], rot: [-Math.PI / 2, 0, 0.3], opacity: 0.45 });
...
const P_BENTO = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  VARIANTS,
  build,
  default: build,
