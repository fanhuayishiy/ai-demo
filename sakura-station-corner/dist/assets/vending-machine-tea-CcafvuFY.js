import { P as PAL, g as grp, M as MAT, T as TEX, m as mesh, b as box, A as row, r as rbox, i as grill, w as weather, H as plane, h as decal, c as cyl, a as sph, n as range, t as tor, W as pipe, q as finish, D as DoubleSide, z as rand, o as lathe } from './index-ROCNX270.js';
...
function productTemplate(e) {
  ...
  if (e.kind === 'bottle') {
    ...
    p.add(noOut(mesh(cyl(0.0034, 0.0034, 0.058, 8), MAT.plastic(e.cap, { spec: 0.4 }), { pos: [0.011, H + 0.022, -5e-3], rot: [8 * D2R, 0, 7 * D2R] })));
  } else {
    const rim = H - 0.004;
    const pts = [
...
const WIN = { x0: -0.49, x1: 0.190, y0: 0.420, y1: 1.400 };
const CON = { x0: 0.214, x1: 0.556 };
const HTW = { x0: -0.556, x1: -0.492 };                    // ヒーター表示タワー
const PKU = { x0: -0.47, x1: 0.170, y0: 0.140, y1: 0.400 };
const CAB = { x0: -0.508, x1: 0.208, z0: -0.33, z1: 0.300, y0: 0.412, y1: 1.412 };
const ROWS = 5, COLS = 3;
const SHELF_Y = [0.432, 0.624, 0.816, 1.008, 1.200];
const COL_X = [-0.387, -0.149, 0.089];
const DIV_X = [-0.268, -0.03];
...
function build(options = {}) {
  ...
  const crumple = (x, y, z, ax, sgn, sc = 1, name = 'crumple') => {
    ...
    c.rotation.y = sgn > 0 ? PI / 2 : -PI / 2;
    const dents = MAT.plastic(color, { map: TEX.metal({ base: color, worn: 0.8, repeat: 2 }).map, side: DoubleSide, spec: 0.32, specPower: 62, shadowAmt: 0.72, steps: 3, tint: '#f2e2c4', sat: 0.96 });
  ...
  shell.add(mesh(box(1.112, 0.360, 0.700), skin, { pos: [0, 1.620, -0.02], name: 'head-block' }));
  shell.add(mesh(box(0.014, 0.330, 0.660), skinDeep, { pos: [-0.3, 1.620, -0.02], name: 'head-seam-l' }));
  shell.add(mesh(box(0.014, 0.330, 0.660), skinDeep, { pos: [0.300, 1.620, -0.02], name: 'head-seam-r' }));
  ...
  fpl(0.620, 0.060, -0.15, 0.418);                                                   // ガラス下見切り
  ...
  base.add(vent);
  vent.add(mesh(box(0.660, 0.062, 0.014), dark, { pos: [0, 0, -6e-3] }));
  for (let i = 0; i < 12; i++) vent.add(mesh(box(0.046, 0.042, 0.010), aluDk, { pos: [-0.3 + i * 0.055, 0, 0.002], rot: [22 * D2R, 0, 0] }));
  // リベット・ネジ
  ...
  [-1, 1].forEach((sgn) => {
    ...
    shell.add(sd);
    sd.add(mesh(box(0.008, 1.20, 0.012), skinDeep, { pos: [sgn * (HW + 0.001), 0.80, -0.1], name: 'seam' }));
    sd.add(mesh(box(0.006, 0.460, 0.340), skinDeep, { pos: [sgn * (HW - 0.004), 1.120, -0.14], name: 'vent-recess' }));
    grill(sd, { w: 0.300, h: 0.420, nx: 4, ny: 5, bar: 0.007, mat: aluDim, pos: [sgn * (HW - 0.001), 1.120, -0.14], rot: [0, sgn * PI / 2, 0] });
  ...
  g.add(fascia);
  fascia.add(mesh(box(1.130, 0.420, 0.110), skin, { pos: [0, 0, -0.01], name: 'fascia-slab' }));
  fascia.add(mesh(box(1.136, 0.016, 0.118), aluDim, { pos: [0, -0.216, -0.01], name: 'fascia-sill' }));
  ...
  decal(signG, { map: TEX.wear({ kind: 'dirt', color: '#b09a78', seed: seed + 43, density: 0.9 }), w: 0.78, h: 0.15, pos: [-0.1, 0.082, 0.0655], opacity: 0.24 });
  weather(fascia, { w: 0.42, h: 0.05, pos: [0.30, -0.1, 0.0565], kind: 'chip', color: '#c9b79a', opacity: 0.5, seed: seed + 44, density: 1.5, spread: 0.005 });
  // 「あたたかい／つめたい」二層 LED
  ...
  ledRow('つめたい', '#0f1a22', '#8fd0ff', '#2b6fb5', -0.16);
  // クローン（円筒天板）＋後方ホッパー
  ...
  g.add(topG);
  topG.add(mesh(box(0.320, 0.036, 0.220), skinDeep, { pos: [0.28, 1.818, -0.18], name: 'hopper-bump' }));
  topG.add(mesh(box(0.160, 0.024, 0.150), zinc, { pos: [-0.32, 1.812, -0.19], name: 'service-hatch' }));
  topG.add(mesh(box(0.090, 0.020, 0.090), aluDk, { pos: [-0.32, 1.828, -0.19], name: 'hatch-knob' }));
  weather(topG, { w: 0.56, h: 0.26, pos: [0.0, 1.8060, -0.15], rot: [-PI / 2, 0, 0], kind: 'dirt', color: '#8b8171', opacity: 0.5, seed: seed + 46, density: 1.6, spread: 0.004 });
  weather(topG, { w: 0.32, h: 0.20, pos: [-0.18, 1.8100, -0.06], rot: [-PI / 2, 0, 0], kind: 'dirt', color: '#eee6d6', opacity: 0.55, seed: seed + 47, density: 0.5, count: 2, spread: 0.004 });
  for (let i = 0; i < 3; i++) row(topG, 3, 0.085, (k, x) => screwHead(x, 1.8020, -0.32 + i * 0.090, 'z'), { x0: -0.3 });
  ...
  for (const p of plan) {
    ...
    if (p.soldOut) {
      ...
      const yaw = () => range(rnd, e.kind === 'pack' ? -0.05 : -0.1, e.kind === 'pack' ? 0.05 : 0.10);
  ...
  conG.add(noOut(mesh(plane(0.090, 0.045), MAT.screen({ map: TEX.lightPanel({ text: 'かざす', bg: '#0d1a16', fg: '#7fe8c0', mode: 'led', spacing: 0 }) }), { pos: [cxm - 0.064, 0.730, 0.3875] })));
  {
    ...
    for (let i = 0; i < 4; i++) for (let j = 0; j < 4; j++) {
      ...
      qr.add(noOut(mesh(box(0.009, 0.009, 0.0018), MAT.plastic('#e9f6f2', { steps: 2, shadowAmt: 0.5 }), { pos: [-0.0135 + i * 0.009, -45e-4 + j * 0.009, 0.0014] })));
    }
  }
  ...
  ch.add(noOut(mesh(box(pw - 0.010, 0.250, 0.014), deep, { pos: [pxm, 0.268, -0.04] })));
  ...
  [-0.01, 0.010].forEach((s) => ch.add(noOut(mesh(cyl(0.0062, 0.0062, 0.040, 8), chrome, { pos: [pxm + s * (pw / 2 - 0.012), 0.344, 0.070], rot: [PI / 2, 0, 0] }))));
  // フラップ
  ...
  flapG.add(mesh(rbox(pw - 0.008, 0.252, 0.014, 0.005), MAT.hardPlastic('#3d3428', { repeat: 2 }), { pos: [0, -0.13, 0.004], name: 'flap' }));
  flapG.add(mesh(box(pw - 0.050, 0.026, 0.008), MAT.plastic(red, { spec: 0.3, steps: 3 }), { pos: [0, -0.2, 0.012] }));
  flapG.add(noOut(mesh(plane(0.220, 0.055), MAT.screen({ map: TEX.signboard({ text: 'おとりだし', sub: 'PUSH', bg: '#2a241d', fg: '#ffd88a', size: 118, spacing: 0 }) }), { pos: [0, -0.16, 0.0135] })));
  ...
  const bk = grp('back');
  g.add(bk);
  bk.add(mesh(box(0.520, 0.400, 0.014), zinc, { pos: [-0.2, 0.52, -HD - 0.010] }));
  grill(bk, { w: 0.460, h: 0.340, nx: 7, ny: 3, bar: 0.010, mat: dark, pos: [-0.2, 0.52, -HD - 0.022] });
  ...
  bk.add(pipe([[-0.44, 1.30, -HD - 0.032], [-0.4, 0.90, -HD - 0.044], [-0.36, 0.30, -HD - 0.040], [-0.3, 0.030, -HD - 0.036], [-0.1, 0.020, -HD - 0.040]], 0.0085, MAT.rubber('#33302a'), { seg: 24 }));
  ...
  gr.add(mesh(box(0.880, 0.014, 0.170), MAT.rubber('#3d3a34'), { pos: [-0.15, 0.037, 0.366], name: 'mat' }));
  decal(gr, { map: TEX.fabric({ repeat: 6 }).map, color: '#4a463e', w: 0.84, h: 0.13, pos: [-0.15, 0.0455, 0.366], rot: [-PI / 2, 0, 0], opacity: 0.5 });
  ...
  for (let i = 0; i < 4; i++) {
    gr.add(noOut(mesh(plane(0.08, 0.08), leafMat, { pos: [range(rnd, -0.5, 0.50), 0.0452, range(rnd, -0.1, 0.34)], rot: [-PI / 2, 0, range(rnd, 0, 6.28)] })));
  }
  // 空き瓶 1 本（温蔵機の横は瓶が転がる）
  ...
  const bg2 = grp('litter-bottle', { pos: [-0.3, 0.0700, 0.420], rot: [0, range(rnd, -0.08, 0.08), PI / 2 - 0.06] });
  bg2.add(productTemplate(bE).clone());
  gr.add(bg2);
  gr.add(noOut(mesh(plane(0.20, 0.10), MAT.paint('#6b6355', { steps: 2, spec: 0.05, shadowAmt: 1, map: TEX.wear({ kind: 'dirt', color: '#4b4438', seed: seed + 67, density: 1.4 }) }), { pos: [-0.3, 0.0455, 0.420], rot: [-PI / 2, 0, 0.2] })));
  ...
  weather(shell, { w: 0.30, h: 0.10, pos: [-0.3, 0.150, HD + 0.0015], kind: 'rust', color: '#8a5236', opacity: 0.45, seed: seed + 70, density: 1.4, spread: 0.005 });
