import { g as grp, M as MAT, P as PAL, m as mesh, b as box, n as range, c as cyl, r as rbox, A as row, h as decal, T as TEX, t as tor, U as circ, w as weather, E as inst, q as finish, D as DoubleSide, x as PlaneGeometry, a as sph, z as rand } from './index-C4-XtFer.js';
...
function build(options = {}) {
  ...
  MAT.concrete({ base: '#8f887a', repeat: 1 });
  ...
  const petalMat = (col) => MAT.paint(col, { steps: 3, spec: 0.12, specPower: 20, specCut: 0.5, sheen: 0.03, shadowAmt: 0.66, side: DoubleSide, rim: 0.2, rimColor: '#fff0f6', sat: 1.04 });
  ...
  function brickRun(parent, axis, fixed, count, layer) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const y = layer * brickH + brickH / 2 + range(rnd, -4e-3, 0.006);
  ...
  function mkPansy(mat, o = {}) {
    ...
    for (let i = 0; i < 3; i++) {          // 黒班（面向かい側 3 弁の基部）
      const s = mesh(gLidPansy, MAT.paint(o.blotch || '#3b2f4a', { steps: 2, spec: 0.06, shadowAmt: 0.9, side: DoubleSide }), { pos: [0, 0.0035, 0], name: 'pansy-blotch' });
  ...
  for (const sx of [-0.085, 0.085]) {
    ...
    sign.add(mesh(tor(0.008, 0.0025, 4, 8), matWire, { pos: [sx, 0.196, -4e-3], rot: [Math.PI / 2, 0, 0], name: 'plate-rivet' }));
  }
  g.add(sign);
