import { g as grp, m as mesh, M as MAT, T as TEX, b as box, ao as DodecahedronGeometry, E as inst, h as decal, H as plane, P as PAL, w as weather, o as lathe, r as rbox, c as cyl, ah as ConeGeometry, n as range, q as finish, N as makeCanvas, Q as toTexture, O as jpText, z as rand } from './index-ROCNX270.js';
...
const DEFAULT_OPTIONS = { len: 5, seed: 88 };
...
function build(options = {}) {
  ...
  const top = mesh(box(W, 0.012, len), MAT.ballast({ repeat: 8 }), { pos: [0, -1e-3, 0], receive: true, cast: false, name: 'ballast' });
  g.add(top);
  // 砂利の寄せムラ（小石を inst で撒く）
  const stoneGeo = new DodecahedronGeometry(0.028, 0);
  ...
  /* ------------------------------ 雑草（境界と白線の隙間） ------------------------------ */
  {
    ...
    const clumpGeo = new ConeGeometry(0.055, 0.13, 7, 1, true);
    const spots = [];
    for (let i = 0; i < 16; i++) {
      const edge = rnd();
