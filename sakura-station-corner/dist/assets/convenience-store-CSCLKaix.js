import { g as grp, M as MAT, P as PAL, T as TEX, m as mesh, b as box, w as weather, c as cyl, t as tor, n as range, r as rbox, h as decal, q as finish, z as rand } from './index-DD_JJZx9.js';
...
function build(options = {}) {
  ...
  /* ---------------- 1. 布基礎・鉄板水切 ---------------- */
  // 布基礎は「壁の下を通る帯」であって「建物いっぱいの slab」ではない。
  // 以前は footing が y ∈ [-0.06, 0.40]、base-top が [0.39, 0.49]、damp-course が
  // [0.47, 0.53] で、いずれも W+0.1 / D+0.06 / W+0.05 = **店舗全面を埋める板**だった。
  // 室内地坪は FY=0.12 なので、基礎が自店の床より 0.41 m も突き出て店全面を覆い、
  // 大ガラスの下半分がコンクリートの白い帯で塞がれていた
  // （`tools/pick.mjs --px=600,555` が打つ最初の不透明体が foundation/damp-course
  //   だった＝「橱窗が起雾」の正体のうち、ガラスではない半分）。
  // 天端をすべて FY=0.12 以下に下ろし、床に覆われるようにする。
  const fnd = grp('foundation');
  fnd.add(mesh(box(W + 0.1, 0.46, D + 0.1), M.foundation, { pos: [0, -0.11, 0], name: 'footing' }));
  fnd.add(mesh(box(W + 0.06, 0.1, D + 0.06), MAT.concrete({ base: '#b6b0a4', repeat: 1 }), { pos: [0, 0.07, 0], name: 'base-top' }));
  // 鉄板水切（基礎と立ち上がりの取り合い）
  fnd.add(mesh(box(W + 0.05, 0.055, D + 0.05), M.aluDark, { pos: [0, 0.1, 0], name: 'damp-course' }));
  // 打継目地・沈下・コケ
  for (let i = 0; i < 5; i++) {
