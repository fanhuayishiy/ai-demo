import { g as grp, M as MAT, P as PAL, r as rbox, n as range, m as mesh, b as box, c as cyl, w as weather, k as tubeOf, T as TEX, E as inst, K as extrude, J as shape, t as tor, W as pipe, h as decal, D as DoubleSide, a as sph, aj as BackSide, q as finish, V as Vector3, z as rand } from './index-Dv-C_8Uh.js';
...
const DEFAULT_OPTIONS = { len: 12, seed: 601 };
...
const POST_Z = -0.4;              // 柱列（世界 z -11.2 の基礎石上）
const POST_W = 0.14;
const POST_TOP = 2.28;
...
function build(options = {}) {
  ...
  POSTS.forEach((px, i) => {
    ...
    const h = POST_TOP + range(rnd, -5e-3, 0.005);
  ...
  for (let i = 0; i <= nRaf; i++) {
    ...
    const lift = i % 7 === 0 ? range(rnd, -7e-3, 0.007) : 0;   // 垂木の反り
  ...
  for (let i = 0; i < 5; i++) {                                  // 野地板 5 枚（目地 4mm）
    ...
    roof.add(mesh(box(BL, 0.016, (SH * 2 - 0.01) / 5 - 0.005), deckWood, { pos: [0, -9e-3 - bow, zw], name: 'deck-board-' + i }));
  }
  const sw = BL / 3;                                             // トタン 3 枚継ぎ（反り・色違い）
  for (let i = 0; i < 3; i++) {
    const sx = x0 - CX + sw * (i + 0.5);
    const curl = range(rnd, -45e-4, 0.0045);
    roof.add(mesh(box(sw - 0.02, 0.013, SH * 2 + 0.02), i === 1 ? zinc : zincOld, {
      pos: [sx, 0.0065 + curl, range(rnd, -7e-3, 0.007)], rot: [range(rnd, -5e-3, 0.005), 0, curl], name: 'totoan-' + i,
    }));
  ...
  for (let i = 0; i < nLamp; i++) {
    ...
    L.add(mesh(cyl(0.13, 0.072, 0.072, 18, true), MAT.lampShade({ color: '#fff6e2', emissive: '#ffdca6', emissiveIntensity: 0.6, side: DoubleSide }), { pos: [0, -0.25, 0], cast: false, name: 'lamp-shade' }));
    L.add(mesh(cyl(0.126, 0.126, 0.006, 18), MAT.lampShade({ color: '#fff8ea', emissive: '#ffe1b0', emissiveIntensity: 0.85, side: DoubleSide }), { pos: [0, -0.286, 0], cast: false }));
    L.add(mesh(sph(0.03, 10, 8), MAT.bulb({ color: '#fff4d8' }), { pos: [0, -0.252, 0], cast: false }));
    L.add(mesh(cyl(0.122, 0.07, 0.06, 16, true), MAT.paint('#6b6355', { side: BackSide, steps: 2, spec: 0.02, shadowAmt: 1, transparent: true, opacity: 0.72 }), { pos: [0, -0.247, 0], cast: false, receive: false }));  // 内側スス
  ...
  g.add(inst(rbox(0.05, 0.003, 0.04, 0.003, 1), MAT.petal({ map: TEX.petal({ mode: 'single', tone: 1 }), alphaTest: 0.4, side: DoubleSide }), 44, (i, d, r) => {
    d.position.set(range(r, x0 + 0.4, x1 - 0.4), -5e-3, range(r, -1.3, 1.3));
