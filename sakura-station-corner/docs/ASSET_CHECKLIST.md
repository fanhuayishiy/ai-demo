...
## 阶段 34：「保留画质还能省多少」—— 三条一起推，一条留下、两条撤回

用户问：不动画面还能省多少。先量预算，再逐条推。

### 34.1 提交预算（新增 `tools/call-budget.mjs`）

| 项 | 实测 |
| --- | --- |
| 静止 hero 主场景 | 5234 calls / 51.3 ms |
| 拖拽 4 s | 5118 calls / 88.5 ms，期间阴影 pass 重画 279 次 |
| 阴影 pass 单次 | **9 次提交 / 0.1 ms** ← `autoUpdate=false` + 只在镜头停下才刷，已经没有油水 |
| 后期链 | 1–3 次全屏 quad，忽略 |
| 视锥内透明件 | 513（其中同资产×同材质×同序 ≥3 的只有 132 件） |
| 摆动子树里的实例批次 | **1028 → 去重后 246 桶**，是剩余最大的一块 |

结论：还能动的只有「摆动件合并」「透明件合并」「合并档数」三处，乐观合计 −18%。

### 34.2 先定价，再动手（新增 `tools/sway-merge-price.mjs`）

合并会把花冠批次合成横跨整棵树的大包围球 → LOD 再也剔不掉它们。
不必真做蒙皮就能量出这个代价：把所有摆动子树里被 LOD 藏掉的批次强制可见。

```
hero     基线 5234 calls / 4.94M tris / 53.0 ms → 强制全可见 +874 批 / +39,400 tris / +1.5 ms
store    +652 批 / +6,884 tris / +1.3 ms
interior +573 批 / +4,376 tris / −4.5 ms（噪声）
```

省下的提交 782 × 8.8 µs ≈ 6.9 ms，代价 1.3–1.5 ms → **值得做**。
（这一步是本该先做的：阶段 33 的 bands 就是没定价直接改，白跑一轮。）

### 34.3 摆动件合并（`src/core/sway-gpu.js`）—— 留下

链深实测 2 层为主（1914 批里 1628 批在两层摆动组下），本来以为要做实例化蒙皮。
但用**世界矩阵增量**就不需要：设某组静态世界矩阵 `W_s`、当前 `W_a`，令 **B = W_a·W_s⁻¹**，
则顶点静止世界位置 `x = W_s·R·p` 摆动后就是 `B·x` —— 恒等式，与链深无关，
因为祖先的运动本来就叠在 `W_a / W_s` 里，**每个实例只需一个骨骼号**。
动画仍留在 CPU（257 个组、每帧 257 次矩阵乘），搬进 GPU 的只是「把变换应用到顶点」。

- B 含平移（绕枝根的旋转＝旋转+平移），且缩放自己约掉（`W = T·R·S ⇒ W_a·W_s⁻¹` 仍是纯旋转），
  所以法线不需要逆转置；
- 骨骼矩阵走一张 `4N×1` 的 RGBA32F DataTexture，在 `scene.onBeforeRender` 里上传
  （three 在 `scene.updateMatrixWorld()` 之后调它 → 读到的一定是当帧姿态，无延迟）；
- 合并体按「树 × 几何 × 材质」成组并挂到 `world` 下，所以每棵树仍是自己的剔除单元；
- 逐实例属性只能挂在几何上，而几何是缓存共享的 → **每个桶克隆一份几何**，
  否则两个桶共用几何时后写的 `aSwayBone` 会覆盖前一个；
- 阴影走同一条变换（`customDepthMaterial` + 同一个补丁），否则树影会停在静止姿态；
- 逐实例颜色（花瓣的色差来自这里）必须随实例一起搬走，不然整棵树变成一块均匀的粉。

### 34.4 等价性证明（新增 `tools/sway-equiv.mjs`、`engine.setAnimTime`、`shoot --time=`）

三步各查一类错误：

1. **数值**：贴图第 gi 格必须等于「该组 live 世界矩阵 × 合并时烘的静态逆阵」——
   槽位错位 / 行列写反会以「米」暴露。实测 3144 个采样实例，**最大误差 1.9e-15 m**。
2. **静止姿态逐像素**（把 `branch-sway` 的 targets 全部 amp=0 后截图对比）：
   meanAbsDiff **0.23**，工具噪声底是 0.13–0.33。
3. **钉住时间逐像素**（`setAnimTime(3.4)` 后截图）：**0.22**。
   差异图只有三块树冠的细碎斑点，建筑、地面、树影全黑 —— 即位置 / 颜色 / 材质 / 阴影都对，
   残差来自 LOD 粒度与帧间时间差。

顺带修掉一个测量陷阱：**UV 流光和摆动都是墙钟驱动的**，不钉住时间，任何两次运行的
吊牌 / 水面区域都对不上。我一度把这种时序噪声当成「透明件合并把吊牌印刷膜画到了玻璃后面」，
据此加了一整套守卫 —— 钉住时间重测才发现两个版本都是 0.01–0.08。
**跨运行的逐像素对比必须先钉时间**，`shoot.mjs --time=` 就是为此加的。

### 34.5 透明件合并 —— 撤回

实测两版：

| 版本 | store calls | 画面 | 装配耗时 |
| --- | --- | --- | --- |
| 只查「同桶内互不重叠」 | −700（5943→5442→4742） | 钉时间后 0.02 | +1.0 s |
| 再查「与任何其它透明件都不重叠」 | −75（5367） | 钉时间后 0.02 | +1.2 s |

严格守卫是对的（跨材质互相叠着的件一旦换了排序 z 就会翻面），但它把收益从 −700 压到 −75；
而为 7430 个透明零件克隆+拼接几何要多花约 1.2 s 装配。**2% 的提交换 1.2 s 启动 + 一类新的排序风险，不划算**，
整条通道删掉，判据留在 `merge-static.js` 的注释里。

### 34.6 bands 4 → 1 —— 撤回

少 38 次提交（−0.7%），但合并体按档内最大零件判定 → 多 1.3% 三角形，
`renderer.render` 中位反而 49.3 → 52.0 ms。**方向是错的**，默认保持 4 档。

### 34.7 最终指标（1600×900 / hi 档 / A-B-A-B 交错、每次之间静置 60 s）

| 视口 | 阶段 33 末 | 阶段 34 末 | Δ |
| --- | --- | --- | --- |
| hero | 5234 calls / 4.94M tris | **4492** / 4.98M | −14.2% |
| store | 5943 | **5442** | −8.4% |
| interior | 6008 | **5610** | −6.6% |
| top | 3532 | **3282** | −7.1% |
| 场景 Mesh | 21630 | 20044 | 1914 批 → 328 批 |

拖拽交互帧率（同一次测量里两组交替）：

| | run1 | run2 | draw calls/帧 |
| --- | --- | --- | --- |
| 摆动合并关 | 中位 **17.7** | **17.5** | 4303–4522 |
| 摆动合并开 | 中位 **19.9** | **19.5** | 3794–4184 |

**+11.6% 帧率**，两组各自重复性 0.2–0.4 fps（不是热漂移）。
从本会话起点（7641 calls / 12.7–13.6 fps）累计：**calls −41%，交互中位 +48%**。

### 34.8 体检与一处如实记录的回退

`npm run check` 142/142 ✓；`check:placed` 缺席 18 条与阶段 33 **逐条相同** ✓；
`check:boot` 无 pageerror、越界丢弃 0 ✓；`weather-check` 五档 + 控件 ✓；
钉时间后 hero/store/interior 对阶段 33 基线的差异仍在 0.1–0.4 区间。

**频闪的「方向翻转率」从 0 变成 0.7**（`flicker.mjs`，`?sway=off` 复跑仍为 0，可重复）。
幅度没有变：相邻帧亮度差 mean 0.06 / max 0.08（0–255 标度，工具自己给的胶片颗粒基线是 1–3，
即比可见颗粒小一个数量级以上）。所以画面看不到闪，但符号开始逐帧交替，
成因未定位 —— 记在这里，下次动 `sway-gpu` 或后期链时先复测这一项。
**→ 该项已在阶段 35 结案：是指标坏了，画面没有问题。**

### 34.9 还剩什么

保留画质的空间基本见底：剩下的 4492 次里 3282 次是「一个看得见的不同形状 = 一次提交」的下界，
透明件 513 次动不得（排序），rbox 805 件不能归一（圆角不随尺寸缩放）。
再往下必须少画东西：`?q=lo` 的装配期细节预算（少摆实例 / 跳过最小件），
那是与 §27「不简化细节」冲突的更大例外，仍未做。

---

## 阶段 35：频闪「翻转率 0 → 0.7」结案 —— 画面没问题，指标坏了

阶段 34 把这项读数记成「成因未定位的回退」。查清了：**不是场景的回归，是 `flicker.mjs` 的度量本身失效**，
摆动件合并只是碰巧把它踩了出来。

### 35.1 先读指标定义，再猜成因

`flicker.mjs` 的翻转率是「均值序列一阶差分的符号变化比例」，而它把均值 `toFixed(1)` 量化到了 **0.1 级**。
两个事实撞在一起：

- 真实的全帧均值波动只有 **0.015–0.023 / 255**（=0.009%），是量化步长的 1/5；
- `Math.sign(0) === 0`，而 `0 !== +1` 被判成「翻转」。

于是序列被压成一串相等的台阶：**40 帧里 38 个样本的差分恰好为 0**，被旧代码当成有效样本参与投票，
真正有信息的只剩 2–3 个 —— 比值由三五个抛硬币决定。单调漂移时全为 0 差 → 报 0；
有动画时出现零星 ±0.1 台阶 → 报 0.7~1.0。**读数与稳定性无关，只与采样窗口内画面动了多少有关。**

### 35.2 四条件对照（新增 `tools/flicker-where.mjs`）

把「动画本身造成的正常变化」和「合并引入的逐帧不稳定」分开：

| 条件 | 均值全精度极差 | 相邻帧 \|Δ\| mean | 翻转率(全精度) | 翻转率(toFixed(1) 口径) |
| --- | --- | --- | --- | --- |
| A 合并开 · 动画 | 0.019 | 0.0036 | 0.53 | **1.0**（可用样本 3/38） |
| B 合并开 · 摆动冻结 | 0.014 | 0.0031 | 0.58 | 0（零差 38/38） |
| C 合并关 · 动画 | 0.019 | 0.0036 | 0.53 | 0 |
| D 合并关 · 摆动冻结 | 0.014 | 0.0030 | 0.58 | 0 |

**A 与 C 逐项相同**（合并前后各 0.019 / 0.0036 / 0.53）→ 摆动件合并没有改变任何稳定性。
全精度翻转率四组都在 0.53–0.58，即**随机游走**（真频闪应接近 1 且有可见幅度）。

3×3 分格定位了「变化发生在哪」：采样框右下格逐帧变化像素占比 0.75–0.81%（其余格 0.02–0.43%），
冻结摆动后降到 0.37% —— 那格本来就覆盖信号灯 / 落樱 / 呼吸灯光，变化来自一直在动的系统，
与合并无关，且 A(0.81) 与 C(0.75) 仍然一致。

### 35.3 修指标，并让它自带判据

`flicker.mjs`：均值改为**全精度**；零差样本剔除后再算比值并**报出可用样本数**；
可用样本 < 5 直接输出 `n/a`；新增「均值全精度极差」和「路径/净位移」；
最后加一行**先于比值的判据**——极差 < 0.05/255 时画面根本没有可测量的明暗摆动，
此时任何翻转率都是在读随机数。修完两档读数：

```
sway=on  极差 0.0149  路径/净位移 11.81  翻转率 0.59（可用样本 22）
sway=off 极差 0.0226  路径/净位移  8.75  翻转率 0.55（可用样本 22）
=> 判定：无频闪。整体明暗极差 0.0149/255（<0.05 判据），翻转率此时无意义。
```

### 35.4 这一条为什么值得单独记

我自己在阶段 34 把它写成「可重复的回退」（复跑确实是 0 和 0.7 —— 可重复的假象），
如果顺着去查 `sway-gpu` 的每帧上传、查骨骼精度、查 LOD 滞回，会烧掉好几轮而一无所获。
**「可重复」不等于「有效」**：读数稳定地反映的是采样窗口内的动画量，不是稳定性。
教训与阶段 34 那条「跨运行对比必须先钉住动画时间」同源 ——
**先确认指标在测什么，再拿它当证据。**

---

## 阶段 36：右上角帧率读数（第三次 UI 让步），顺带揪出两个「看着合理的错数字」

用户点名要右上角显示帧数。这是「成品画面零 UI」的第三次让步，也是第一次让步的对象是
**纯诊断信息**：前两次一个是装配期临时层（首帧后摘掉）、一个是功能控件（天气条）。
所以它的硬约束是——读数绝不能干扰被读的东西。

### 36.1 实现（`src/core/fps-hud.js` + `index.html` 的 `#fps`）

- `pointer-events: none` —— 右上角正是拖拽取景最常经过的地方，读数不能挡住操作；
- **4 Hz 刷新**而不是逐帧写 `textContent`：逐帧改 DOM 会参与布局，把要测的东西测成噪声；
- 数字全部取自 `engine.stats`（引擎自己的 0.5 s 滚动窗口），不在 HUD 里另起一套计时 ——
  两处计时迟早给出两个「帧率」，然后有人拿错的那个做判断；
- 随天气切亮/暗配色（与天气条同一判据 `weather.dark`），低于 25 fps 时主数字变粉；
- `?fps=0` 关掉，且 **`tools/shoot.mjs` 默认就带这个参数**：常驻覆盖层一旦进画面，
  每一次像素基线都会多出一块恒定差异（阶段 33 在天气条上记过同样的教训）。
  `--fps=1` 才带着读数拍；
- 配色可读性要按**叠乘之后**算：容器 `opacity: .82` × 次要字 `alpha .45` ≈ 0.37，
  夜景下第一版的 ms / draw 两段基本糊掉。新增 `tools/hud-zoom.mjs` 把那一小块按 3× 裁出来看
  —— 整屏缩略图上的「看不清」是缩略图的错，不是配色的错。

### 36.2 第一个错数字：`stats.calls` 一直是 1

HUD 第一版显示 `1 draw`。来源是 `renderer.info` 默认 `autoReset = true` ——
**composer 每跑一个 pass 就调一次 `renderer.render`，每次调用开头都把 info 清零**，
于是帧末读到的是最后一个全屏 quad 的 1 次提交，不是整帧的。

修法在 `engine.js`：在**场景那次 render 结束的瞬间**取数（`scene.onAfterRender`，
three 在该场景全部绘制完成后才调），0.5 s 窗口只负责 fps。该口径不含后面 SMAA / 输出
那 2–3 次全屏 quad，作为「画面里有多少东西要画」反而更干净。

交叉验证：修完 HUD 在 hero 读 `4.5k draw`，与 `merge-check.mjs` 独立测到的 **4492** 一致，
`perf.mjs`（自己关 autoReset）测到的 3794–4184 也在同一量级。三个来源一致才算数。

### 36.3 第二个错数字：`img-diff` 的「像素占比」其实是通道占比

`pctPixelsOver24` 的分子在 RGB 通道循环里 `++`，分母却是像素数 —— 于是它恒为真实像素占比的
约 3 倍，一块完全相同的像素能算出 **300%**（判夜景配色时撞上）。已改成按像素判定。

**历史读数一律 ÷3 解读**：阶段 33 记的「4.68% 像素超 24 级」实际 1.56%，
阶段 34 的「基线自差 0.26%」实际 0.087%。它们彼此仍可比较（同一个错公式），
只是标签一直虚高 3 倍；`meanAbsDiff` 不受影响。

### 36.4 验证

`npm run check` 143/143 ✓；dev 与 `dist/` 产物双路 `check:boot` 无 pageerror、越界丢弃 0 ✓；
`shoot`（默认 `fps=0`）对阶段 34 基线 meanAbsDiff **0.10** —— 画面没被这次改动带走；
`--fps=1` 时只有右上角 diff 出恒定块，其余区域干净 ✓。
顺手清掉几个会话遗留的 `vite preview` 进程（4173/4179/4181/4183）——
残留进程会污染启动与帧率测量，这条纪律早就记过。

---

## 阶段 37：加载还能提速吗 —— 首帧在画一张没人看的整场景

「8 s 能不能再快」这种问题不能靠感觉答。先把那 8 s 逐帧摊开，再逐个假设定价。
新增三件量具：`engine.frameLog`（装配期逐帧账本：毫秒 / 提交数 / 是否 bake 阴影 / 已上传的
几何与贴图数）、`tools/boot-feed.mjs`（读账本并按成分归因）、`tools/boot-compile.mjs`
（built 后暂停渲染循环，手动拆首触帧）、`tools/cpu-profile.mjs`（CDP 行级采样）、
`tools/zoom-hitch.mjs`（拉近时的补传尖峰，瞬移 + 真实滚轮两种）。

### 37.1 先量：8.77 s 花在哪（dev 800×500，丢弃冷启动后中位）

```
加载 0.18 ｜ engine 0.07 ｜ world 4.79 ｜ lod 0.12 ｜ motion 3.37 ｜ weather 0.02 ｜ built→成帧 0.22
```
`motion` 那 3.37 s 不是动效代码（8 套动效同步耗时合计 0.05 s），是装配期间插进来的
27 个帧。账本里第 25 帧：**4866 ms，绘制 23336 次，同帧上传 4114 份几何 + 1097 张贴图**。
另外 `?asm=slow|full|freeze` 三档成帧 8.77 / 8.89 / 8.54 —— 装配期的渲染节流确实只是「挪」，
这条老结论成立。

### 37.2 修的是顺序，不是内容：LOD 安装时立刻剔一遍（`core/lod.js`）

`installLod` 把剔除挂在 `onUpdate` 上，要等 `acc` 攒满 0.12 s 才跑第一次；
而「装配完成后的第一帧」在这之前就到了 —— 于是那一帧画的是**未剔除的整场景 2.3 万次提交**，
画面还被加载层盖着，没有任何人看见。把剔除本体抽成 `pass()`，安装时先跑一遍
（`?lodtick=off` 回到历史行为）。

| 指标（成对，构建产物 1600×900） | lodtick=off | lodtick=on |
| --- | --- | --- |
| 首触帧 | 4866 ms / 23336 次 | 2721 ms / 3188 次 |
| built→成帧总账 | 7.45 s | **6.15 s（−18%）** |
| dev 800×500 成帧 | 9.35 s | **7.25 s（−22%）** |
| GPU 资源（built 时） | 4127 几何 / 1111 贴图 | 1653 / 333 |

隐藏零件的几何与贴图不再于启动时上传，改在第一次露出时补 —— 所以必须查这一条：

### 37.3 省下的时间有没有搬到「拉近卡一下」

`tools/zoom-hitch.mjs`（1600×900，稳态帧先跑热）：

| 路径 | lodtick=off | lodtick=on |
| --- | --- | --- |
| 瞬移到 store（工具路径，最坏情况） | 峰值 140 ms | 峰值 247 ms，补传 +483 几何 / +118 贴图 |
| 真实滚轮推进到 store（26 格 × 45 ms） | 均值 60 / p90 64 / 峰值 102 | 均值 63 / p90 64 / 峰值 137 |
| 真实滚轮推进到 interior | 均值 66 / p90 75 / 峰值 105 | 均值 67 / p90 76 / 峰值 103 |

用户走得到的路径上多出来的是一次性的 ~35 ms 峰值，均值与 p90 不动；
瞬移那条只有工具会走（成品画面没有跳转控件）。判为可接受。

**画面等价**：钉住动画时间三机位逐像素 meanAbsDiff 0.03–0.10、超 24 级 0.03–0.15%，
而**同一配置重复拍摄**的噪声地板就是 0.12% —— 差异全在花丛与地面的单像素散点（花瓣粒子落点），
没有整块零件多出来或少掉。交互稳态：`perf` 中位 24.0 vs 24.3 fps，draw calls 与三角形数一致。

### 37.4 三个「看着该省、实测不省」的假设（都已撤回）

- **`renderer.compileAsync` 提前编译**：在世界刚挂上时开编，让链接与剩下的 JS 并行。
  实测 `lod` 阶段 +0.34 s（遍历 2 万个对象），成帧无改善 → 撤。链接不能提前"用完"，
  GL 在使用一个程序前必须等它链接完成，挪不动。
- **关掉 `debug.checkShaderErrors`**：CPU 采样榜首是 three 的 `onFirstUse` 自耗时 3.5 s，
  像是元凶（它每个程序首次使用都同步取 `getProgramInfoLog`）。关掉后成帧 6.04/6.03 vs
  6.05/6.36 —— 无差别。**采样器把「阻塞在驱动里等链接」记成该函数的自耗时，那是等待不是 CPU 工作**；
  profiler 榜首 ≠ 可回收成本。生产构建因此保留着色器错误自检。
- **贴图预算 `?texcap=8`**（1191 份位图全压到 8 px，画面必然变糊，只用于定价）：
  成帧 5.85 → 5.36 s。整条程序化贴图线（绘制 + 上传）只值 0.5 s，再动它就是拿画面换半秒。
  开关留在 `core/style.js` 里作为常设消融口。

### 37.5 现在的账与剩下的三条路（构建产物 1600×900）

`0.10 加载 + 0.06 engine + 4.10 world + 0.10 lod + 1.44 motion(GPU 首触) + 0.01 weather + 0.25 成帧 = 6.15 s`

推送后公网（GitHub Pages，800×500，A-B 交替四组）：开 **7.45 / 7.19 s**，关 9.13 / 8.92 s ——
`motion` 那一段从 3.00 s 掉到 1.22 s，与本地同构。**但公网的尾风险不在装配而在 chunk 拉取**：
同一份产物读到过一次「加载」阶段 28.8 s（其余六个阶段全正常），历史更极端的 36 s。
一次抖动就能让整页翻倍，所以公网数字要按「中位 + 尾部」两个量看，只看中位会低估它。

`world` 那 4.1 s 就是 123 个资产模块 + 10 个地图模块的建模 JS 本身，行级采样里没有
单个热点超过 0.4 s（textures.js 合计 1.4 s：`heightToNormal` 0.36 / `jpText` 0.30 /
`speckle` 0.27 / `fitW` 0.22；three 侧 `_Object3D` 0.15 / `RoundedBoxGeometry` 0.14 /
`generateSegment` 0.10）。再往下只剩三条：
① 把建模与贴图生成搬进 Worker（架构级改动，收益上限约 1.5 s，且 242 MB 位图要跨线程传）；
② 构建期细节预算（与 §2.3「不简化细节、不压缩精度」正面冲突，需明确授权）；
③ 已经在卖的 `?q=lo`（只少 23% 提交，且是拿画面换）。
**免费的部分已经拿完了。**

### 37.6 又一次跨会话 fps 不可比

本轮 dev 与构建产物成对复测都是 **23.5–24.2** 中位，而阶段 34 记的是 19.5–19.9，
两次的 draw calls 构成几乎一致（3771–4175 vs 3794–4184）。同一场景、同一提交数、
不同会话差 20% ⇒ 机器热状态与后台负载。**跨会话比较用 draw calls 与三角形数，fps 只在
同一次成对测量内部比**（AGENT.md §8 已按这条改写）。

---

## 阶段 38：60 fps 目标可行性 —— 把「每次提交 10.3 µs」这条账算到底

目标被明确抬到 60 fps（此前交互中位是 23.5–24.2 fps）。先说结论：**在当前画面内容量下，
1600×900 的 60 fps 靠调度、状态优化与合批都到不了**；能到的是 ~30–40 fps，
再往下必须少画东西。下面是把这句话变成数字的过程。

### 38.1 帧时间的线性模型（`tools/frame-anatomy.mjs`，hero 机位，五个 LOD 阈值采样点）

```
ms ≈ 10.26 µs × 提交数 + 0.31 ms × 百万三角形 + 3.4 ms 固定项      （残差 ≤ 0.3 ms）
```
采样点：px=6/26 → 4492 次 51 ms；px=90 → 3308 次 39 ms；px=150 → 2980 次 35 ms；px=260 → 2651 次 32 ms。

**推论一**：三角形几乎免费（0.31 ms/百万 → 全场 5M 三角只值 1.5 ms）。
**推论二**：60 fps 的 16.6 ms 预算里，扣掉固定项与着色，只容得下 **约 1100 次提交**，
而当前任何一个机位都在 2600–4500 次之间。

### 38.2 那 10.3 µs 是不是「状态」？不是（`tools/draw-cost.mjs`）

| 档位 | 帧时间 | 说明 |
| --- | --- | --- |
| A 基线（4492 次） | 55 ms | |
| C 摘掉全部 map / normalMap | 55 ms | 贴图绑定 = **0 ms** |
| B 全场共用 1 个材质 | 52 ms | 材质与 uniform 换绑 = **3 ms（5%）** |

`tools/shade-cost.mjs` 再补三刀：各向异性 8→1 省 1 ms、关掉 380 个材质的 alphaTest 反而 +1 ms、
把所有贴图换成 1×1 省 0 ms。`tools/multidraw-price.mjs` 里「4 种属性布局交替」与「统一布局」同为
2.3 µs/次 —— **换 VAO / 换输入布局也不是成本所在**。所以省材质、做图集、统一顶点布局、降各向异性
这些常见药方，在本场景全部没有肉。

### 38.3 唯一还没被排除的机制：把提交本身合掉（multi-draw）

`WEBGL_multi_draw` 在本机 Chrome/ANGLE 可用，three r186 的 `BatchedMesh` 会走
`multiDrawElementsWEBGL`：合成基准里 **3000 份独立几何 = 1 次提交 = 1 ms**
（同样的东西拆成 3000 个 Mesh 是 4–12 ms）。也就是说机制成立。

问题在「谁能进同一批」。`tools/draw-classes.mjs` 按四种同批定义量了视锥内的真实分布（hero 4187 次）：

| 键 | 类数 | 覆盖 ≥2 件的绘制 | 每批 ≤128 → 提交数 |
| --- | --- | --- | --- |
| K1 只看状态 | 126 | 98% | 152 |
| K2 + 贴图身份 | 555 | 90% | 577 |
| K3 + 逐件标量（颜色除外） | 671 | 88% | 690 |
| K4 + 颜色（≈现在的材质数） | 1484 | 78% | 1488 |

**颜色是主要障碍**：3717 个材质里绝大多数只因颜色不同而不同。K4（不需要任何逐件数据）只能把
4187 压到 1488 → 按 38.1 的模型 ≈ 15.3 + 1.5 + 3.4 ≈ 20 ms 提交项，加着色与后期约 39 ms → **26 fps**。
要吃到 K3 的 690 批，必须把逐件颜色搬进着色器（BatchedMesh 自带的 `setColorAt` 在
`color_fragment` 里仍要求 `USE_COLOR`，与我们的 `onBeforeCompile` 改造冲突，得自建逐件属性贴图）。
而即便如此，38.1 的模型给出的上限是 **~30 fps**：multi-draw 只省 CPU 侧的排队，
GPU 仍要按 4492 个对象各自的光栅化设置走一遍。

`tools/batch-probe.mjs` 的两组实测印证了风险：
- 只按「同材质同色」合批（保守档）：吸收 118 件、省 92 次 → 帧时间 0～+1 ms（收益淹没在噪声里）。
- `--price` 档（键里去掉颜色、并把实例批次全部拆开重排）：吸收 64117 件、只剩 421 批 →
  **帧时间 51 → 88 ms**。因为合并体包围球横跨全场，视锥与 LOD 的剔除全部失效。
  这条与阶段 33/34 记录的「跨资产合并让 interior 反而多 656 次」是同一件事的两次独立复现。

### 38.4 结论与剩下的选项（按代价排序）

1. **少画东西**：这是唯一能真正到 60 fps 的路，且必须是**装配期细节预算**
   （少摆实例 / 跳过最小件），与 §2.3「不简化细节、不压缩精度」正面冲突 —— 需要单独拍板。
   已备好量尺：`?lodpx=N` 单独拨投影像素阈值（不动 dpr / 阴影 / 描边距离），
   实测 26 → 46 px 得 +7%、→ 90 px 得 +30%、→ 260 px 得 +37%，代价是远景小件提前消失。
2. **逐件颜色 + 按类 multi-draw 合批**：不改画面，预计 24 → 30 fps 左右，
   要动 `BatchedMesh` 装配、LOD 的逐件可见性、着色器逐件属性贴图、启动耗时（+1~2 s）与全部像素回归。
   投入产出比在本场景不划算，且到不了目标 —— 未做，探针与数据都留在 `tools/` 里。
3. **窗口 / DPR**：三角形与片元都不是瓶颈（38.1、38.2），降内部分辨率几乎不省，别指望这条。

另外记一条测量纪律：本轮同一份构建在不同时刻读到 18.8 与 24.0 fps 中位（draw calls 相同），
再次确认 **fps 绝对值跨会话不可比**，所以阶段 38 的所有判断都建立在「同一次运行内的比值」上。

### 38.5 收尾的那个数：还能合批的东西只剩 382 / 4408

把探针的候选条件全部放开（`--price --override --noexplode --min=2`，两档都在
`scene.overrideMaterial` 下测，把着色固定成常数）之后：**视锥内 4408 次提交里，
只有 382 件是「可再合批的散件」**，其余约 4000 次分别是实例批次（2358）、已烘好的静止合并体、
透明件、带逐件动画标记的件、非索引几何。也就是说 autoInstance + 形状归一 + 提交层合并
已经把「冗余」榨干了 —— 合批需要重复，而这个画面按规格就是「每个物件独立建模、独立材质」。

按 38.1 的模型算：把这 382 件并成 18 批只省 287 次提交 ≈ 2.9 ms，
而实测因为合并体变大丢掉视锥裁剪，反而 +2 ms。**结论：合批这条路在本场景已到收益边界，
60 fps 只能来自「少画东西」这一条。**

---

## 阶段 39：拍板 `lodpx=90` —— 以及为什么必须配一张「钉住名单」

用户从阶段 38 给的三档里选了 90 px。直接把全局阈值抬上去，画面是这样塌的
（`tools/lod-where.mjs` 按资产对账，hero 机位 26 → 90 共少画 1168 次提交，涉及 109 个资产）：

| 资产 | 少了多少件 | 看到的现象 |
| --- | --- | --- |
| sakura-a1/a2/b1 | 171 | 樱花树掉空内层花簇，右边那株直接变稀 |
| bike-*（7 辆） | 240 | 自行车只剩两个轮和一根撑子 |
| train-single | 42 | 车厢窗与门缝淡成一片 |
| store-signage | 20 | **屋顶招牌「サクラ・マート」只剩白底板** |
| street-lamp / road-sign / 信号机 | 数十 | 灯杆没了灯头，止まれ 与黄色菱形牌消失 |
| drainage / manhole / ac / bin / 电线杆铁件 | 其余 | 路面格栅、井盖、空调外机风扇消失 |

**判据不是「重不重要」而是「拆件可读性」**：全局阈值砍的是零件，
而这些物件少一个零件就读不出来（一辆只剩轮子的自行车不是"简化版自行车"，是画错）。
反过来，**整件消失是无害的**（远处一个井盖看不见 ≠ 画面坏了）。
所以拆成两半：

- 全局阈值 26 → **90 px**（`core/quality.js` 的 hi；mid/lo 同步只增不减：110 / 140）。
- `IDENTITY` 名单（`world/placement.js`）里的资产单独钉在 **26 px** —— 也就是
  抬阈值之前那批物件逐件不变。名单是量出来的，不是猜的。

### 39.1 代价与收益（成对 A/B，1600×900 持续拖拽，同一轮内比较）

| 档位 | draw calls/帧 | fps 中位 | 画面 |
| --- | --- | --- | --- |
| 26 px（旧默认） | 4012–4431 | 13.8 / 15.1 | 基线 |
| 90 px 裸抬 | 2916–3291（−28%） | 22.0 / 22.4（**+30%**） | 招牌掉字、自行车只剩轮、灯没头 |
| 90 px + 钉住名单 | 3569–3961（−11%） | 17.1 / 18.0（**+21~24%**） | 只剩路面杂物变稀 |

钉住名单交回约 450 次提交，换掉一半的帧率收益 —— 这笔账按 §2.1「质量优先」是必须付的。

### 39.2 像素对账（钉住版 vs 26 px 基线，钉住动画时间 `--time=3.4`）

hero 1.92%、store 1.13%、interior 0.66%、top 2.43%、夜景 hero 1.26% / store 0.86% 的像素超过 24 级；
裸抬 90 时同一组数字是 4.47% / 1.84% / 0.86% / 3.74%。
**剩下的差异全部是「远处小件不再画」**：道路排水格栅、井盖、空调外机风扇、远处垃圾桶与电线杆铁件。
拉近即逐件回归（阈值按投影像素判定，物体靠近后必然越过 90 px）。

### 39.3 顺手补的机制说明

`?lodpx=N` 只拨全局阈值，资产自报的 `userData.lodPx` 优先于它 —— 所以
`?lodpx=26` 这一档测出来就是「旧画面」，是干净的对照组。名单与阈值都写在代码注释里，
`tools/lod-where.mjs` 可以随时重算「某个资产值多少提交」。

**如果用户回头说井盖/格栅不能少**：把 `misc/manhole-cover`、`map/drainage` 加进 `IDENTITY` 即可，
代价按 39 的表读（drainage 40 件 ≈ 0.4 ms）。
