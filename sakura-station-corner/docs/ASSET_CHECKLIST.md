...
- [ ] 24.5 **限速标牌面仍然半透明**（`shots/sign4.png`）：能看到背后的色带、货架与补强筋。
  排除了三个假设：不是 air-haze（藏掉后依旧）、不是 `fadeWear`（改完依旧）、
  不是标板单面（DoubleSide 后依旧）。查 `sign-plate` 时它 `visible:false` —— 那是 LOD 在远景
  正常剔除，不是原因。**下一步该查 `S.face()` 画出来的白色圆盘本身 alpha 是多少**，
  以及 `MAT.decal` 的 `depthWrite:false` 与标板渲染顺序的关系。
