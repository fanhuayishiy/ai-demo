...
| [sakura-station-corner](./sakura-station-corner/) | 樱花街角 · 电车与便利店微缩三维沙盘（Three.js r186，102 个独立资产模块，画面零 UI） | 可交互 |
...
- 画面不许出现调试信息，所以可观测性全部搬到无头工具里：`tools/` 21 个脚本覆盖包围盒核对、越框检测、真实拖拽帧率、逐资产消融、逐帧抖动分布、分辨率对照、材质/几何状态计数、LOD 阈值扫描、按形状反查杂物、启动墙钟的阶段归因
- 26553 个 Mesh 折叠为 6220 个实例批次（同几何 + 同材质 + 同渲染状态才合并，每件变换仍独立，几何永不 merge）
- 启动 7.9 s 出画面（中位，1600×900）：慢的原因几乎全在贴图的生成与 GPU 上传，不在模型精度——平涂会剥掉的
  贴图不再画、印刷内容按屏上 texel 密度定尺寸、几何缓存键量化到 0.5 mm；装配期渲染限到 ~8 fps（`?asm=`），
  到 `built` 从 7.9 s 压到 4.8 s，且成帧时间的方差从 ±0.9 s 收到 ±0.06 s
...
npm run check:boot   # 无头启动：确认真正建完且无 pageerror
